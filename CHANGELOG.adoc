...
NOTE:: *How this file is maintained.* A section is frozen once its release has **shipped** - `0.5.x` and below are hand-written legacy from before the fork, and are finished. The section for the release currently being cut is **generated at release time from the commit log**, replacing whatever text is under it, and is frozen in turn once that release ships. **That includes `0.6.0.0` below**: it has not shipped, so what you are reading there is working text and not the notes that release will publish. Consequently a PR never adds an entry here and never opens an `== Unreleased` section; the only edit a PR may make is correcting a factual error in text that is already present. The judgement applied when generating (what earns an entry, and how long it may be) is in `docs/releasing.md`.
...
First release of the community fork of https://github.com/confluentinc/parallel-consumer[confluentinc/parallel-consumer] (upstream is no longer actively maintained). Source compatible with upstream 0.5.x apart from one removed accessor — two changes are required of every codebase: the Maven groupId, and the Java packages every import names; plus that one method, if you called it (see Breaking below). Includes upstream fixes and features merged after upstream's last release (0.5.3.3) that were never published there.

=== Breaking

* Maven coordinates rebranded from `io.confluent.parallelconsumer` to `bz.stub.parallelconsumer` (https://github.com/astubbs/parallel-consumer/pull/55[#55]), and the Java packages moved with them — `io.confluent.parallelconsumer` to `bz.stub.parallelconsumer`, with the library's shared internal utilities folded in under `bz.stub.parallelconsumer.internal.utils`. Update your dependency declarations. Rewrite your imports; apart from the removal of `WorkManager.getSuccessfulWorkListeners()`, no signature changed, so nothing else in your code moves. The README's Upgrading section carries the one-line sed and that method's replacement.
...
** *Self-hosted high-CPU lane* - the full suites, including chaos and performance, run per PR on real hardware. Advisory only; the required merge gate stays on GitHub-hosted runners. See link:docs/self-hosted-runner.md[docs/self-hosted-runner.md].
+
