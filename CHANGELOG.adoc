...
* Mutation testing (PIT) runs advisory on the GitHub-hosted PR build (`bin/ci-mutation-test.sh`), scoped to the core classes changed vs the PR base so it fits the 2-core runner - and that is now the *only* per-PR mutation lane. It had been running up to four times per PR (twice on highcpu, plus the hosted and `local` lanes), three of them the same scoped computation; the `local` copy also checked out shallow, which silently promotes a scoped run into the full sweep. **The full `internal.*` sweep is now manual-only** (`mutation-full-sweep.yml`, with `target-classes`/`target-tests` inputs): it has never once completed - 42+ min on CI, 83+ min locally with minions dying on `MEMORY_ERROR` - and a sweep that never finishes scores *zero* mutants while holding a 16-core runner for its full timeout on every push. It is deliberately not nightly either: scheduling a job that has never finished only moves the waste to a quieter hour. **A single flaky test no longer switches mutation testing off repo-wide:** PIT refuses to score anything while any test is unstable *without* mutation, so one unrelated flake meant zero mutants everywhere (twice: #101, #110). `skipFailingTests` now makes it drop that test's coverage instead of aborting - it must be set in the pom, since the mojo parameter declares no `property` and `-DskipFailingTests` is silently ignored. Finally, every run writes a GitHub job summary saying which outcome it was - scored (with the totals), *skipped* because nothing changed, or died before scoring - because a green mutation tick means "nothing to mutate" at least as often as it means "all mutants killed". Analysis and the remaining work (retargeting off the concurrency core, where mutants hang by construction and survivors are unfalsifiable) in link:docs/plans/2026-08-03-002-mutation-testing-plan.md[the mutation testing plan] (https://github.com/astubbs/parallel-consumer/pull/111[#111]).
