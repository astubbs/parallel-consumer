...
NOTE:: *How this file is maintained.* Everything up to and including `0.6.0.0` is hand-written and frozen. From the next release on, each section is **generated at release time** from the `Changelog:` trailers of the commits that shipped in it, then re-assessed and rewritten as a set before being frozen in turn - so do not add entries here in a feature PR. What earns a trailer, and how to write one, is in `AGENTS.md` -> Changelog. In short: an entry must be observable by a user or operator (API, behaviour, performance, logs and metrics, or the published artifact), and it is one sentence of about 25 words naming what a reader would have *seen* - not how the bug worked.
