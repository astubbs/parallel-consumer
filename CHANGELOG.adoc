...
NOTE:: *How this file is maintained.* Everything up to and including `0.6.0.0` is hand-written and frozen. From the next release on, each section is **generated at release time from the commit log** and then frozen in turn - so do not add entries here in a feature PR, and do not open an `== Unreleased` section. The judgement applied when generating (what earns an entry, and how long it may be) is in `AGENTS.md` -> Changelog.
