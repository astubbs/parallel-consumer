 ...
 * harness therefore drives every binding: {@link CoreBinding} hands it an in-JVM {@link Client}, a client
 * library that constructs Parallel Consumer for itself is handed the mock Kafka clients through
 * {@link #startEmbeddedClient}, and anything that speaks the production gRPC protocol - in any language -
 * reaches the same fixture through {@link #startEngine()}. Each binding's run reduces to "connect, process a
 * record, report" against a fixture that already exists - the {@link HarnessScenario} names are that shared
 * conformance vocabulary.
 * <p>
 * <b>THE THREE LANES ARE MUTUALLY EXCLUSIVE PER INSTANCE</b>, and each owns the engine differently:
 * {@link #start} owns the engine and the client is a function; {@link #startEmbeddedClient} lends the mock
 * Kafka clients to a client that brings its own engine; {@link #startEngine()} owns the engine and the client
 * is a foreign process speaking gRPC at it.
 * <p>
 * <b>This class is the reconciliation astubbs/parallel-consumer#387 left for the engine rung, and it is
 * done.</b> That rung cut this file out of {@code feats/proxy-requirements}'s {@code ProxyHarness} with the
 * engine lane <em>removed</em>, because the sidecar on that stack hosts no engine and answers every session
 * {@code UNIMPLEMENTED} (astubbs/parallel-consumer#384) - and it recorded, in this header, in the module pom
 * and in a deferred inflight note, that whoever landed the engine had to merge the two rather than keep both.
 * The engine lane below is that restoration. <b>The NAME is the extracted rung's and the MODULE is the god
 * branch's</b>, and that split is the engine lane deciding, which is what that rung said would settle it:
 * {@code ConformanceHarness} says what the class is for, and the sidecar module is the only place it can
 * live, because the lane needs {@code ConfigureHandler} and {@code ProxyProcessor} and because
 * {@code TestModeMain} - the process a spawned foreign runner actually starts - sits in this same test tree
 * and would otherwise import the conformance module and close a dependency cycle.
 * {@code parallel-consumer-proxy-conformance} reaches it through this module's test-jar, which its pom
 * already declared for exactly this reason.
 * <p>
...
public class ConformanceHarness implements AutoCloseable {
    ...
    // --- the byte[] mock pair, shared by the engine lane and the embedded-client lane. Separate from the pair
    // above because neither of those deserializes (their record types are byte[]), while the in-JVM lane above
    // speaks Strings; the lanes are mutually exclusive per harness instance. ---
     ...
     * {@code bz.stub.parallelconsumer.proxy.testmode.TestModeMain} is the process entry point that calls this
     * on behalf of a spawned, non-JVM test, and {@link ConformanceDriver#spawnAgainst} is what spawns it.
     * <p>
     ...
     * private: the mock consumer whose commit history {@link #lastCommittedOffset()} reads. A test that built
     * its own mocks instead would be asserting about a fixture the harness cannot see, so every scenario
     * written against the harness would have to be written a second time for it.
     * <p>
