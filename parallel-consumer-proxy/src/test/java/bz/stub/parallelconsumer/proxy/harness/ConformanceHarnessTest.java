...
class ConformanceHarnessTest {
    ...
    void aProcessedRecordAdvancesTheCommittedOffset() throws Exception {
        try (var harness = new ConformanceHarness(HarnessScenario.A_PROCESSED_RECORD_ADVANCES_THE_COMMITTED_OFFSET)) {
    ...
    void anUnreportedRecordHoldsBackTheCommit() {
        var harness = new ConformanceHarness(HarnessScenario.AN_UNREPORTED_RECORD_HOLDS_BACK_THE_COMMIT);
    ...
    void aFailedRecordIsRedeliveredWithItsFailureHistory() throws Exception {
        try (var harness = new ConformanceHarness(HarnessScenario.A_FAILED_RECORD_IS_REDELIVERED_WITH_ITS_FAILURE_HISTORY)) {
    ...
    void recordsSharingAKeyShareAShardAndDistinctKeysRunConcurrently() throws Exception {
        try (var harness = new ConformanceHarness(
            ...
            assertWithMessage("the shared key's head record reached the client")
                    .that(sharedHeadInFlight.await(ConformanceHarness.CONVERGENCE_BUDGET.toSeconds(), SECONDS))
                    .isTrue();
            ...
            var sharedShardOffsets = harness.deliveries().stream()
                    ...
                    .map(ConformanceHarness.Delivery::offset)
                    .toList();
    ...
    private static List<Long> deliveredOffsets(ConformanceHarness harness) {
        return harness.deliveries().stream().map(ConformanceHarness.Delivery::offset).toList();
    }
}
