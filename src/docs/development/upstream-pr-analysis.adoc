...
* *confluentinc#843 is not caused by confluentinc#909.* That defect *drops* the fresh container
  ...
  One rebalance-gated route to the reported symptom did exist - the stale sweeps evicted a
  container without asking whether it was in flight, so the same instance could start the
  re-delivered key beside its still-running worker - and is fixed on the fork; see
  `docs/solutions/logic-errors/a-revoke-sweep-freed-a-key-whose-worker-was-still-running-2026-09-18.md`.
