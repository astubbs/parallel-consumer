...
----
<1> Generated records per second, per source topic, with each route's declared type filled with realistic random data.
<2> Stop generating after ten seconds and close, draining first. Without a bound the run ends when you close the handle.
<3> How many distinct keys the generator draws from. Keys have to repeat for key ordering to have anything to order, and they have to repeat rarely for a topic to look like a real one.
<4> The one line that differs from a run against a broker.
<5> The bound closes the instance, so this returns when the run ends.
...
*A driver that publishes for you* -- for a soak or a demo, where the point is volume rather than particular records: it fills each route's declared value type with realistic random data, encodes it with that route's own serialiser, paces it at a rate you declare, and takes an optional bound after which it settles and closes the run itself. That is what a sandbox does unless `handPublished()` says otherwise.
