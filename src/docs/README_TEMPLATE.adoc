...
Committed offset metadata written by any earlier version is still read, so an existing consumer group can be upgraded in place without resetting or migrating offsets.
This version does, however, write denser metadata formats that older versions cannot read: during a _rolling_ upgrade, a not-yet-upgraded instance that reads a newer instance's commit falls back to the committed offset - safe, but it can redeliver work that was in flight beyond that offset, which matters to non-idempotent processors.
The exposure ends when the rolling upgrade completes (see <<offset_map,the offset map>>).
...
Counter `pc.offsets.encoding.usage{subsystem=offsetencoder, codec="BitSet|BitSetCompressed|BitSetV2|BitSetV2Compressed|RunLength|RunLengthCompressed|RunLengthV2|RunLengthV2Compressed|DeltaList|DeltaListCompressed"}`
...
.. the offset map is serialised and string-encoded - Base64 for small payloads (below 22 bytes, keeping them readable by older releases), the denser Z85 from 22 payload bytes up, where it is always shorter - and added to the commit message metadata.
...
The offset map is encoded in parallel using several different techniques - run length encoding, bitmap encoding, and a sparse delta-list for maps where the incomplete offsets are few and far apart.
The sizes of the encoded maps are then compared, and the smallest chosen for serialization.
Any encoding whose result is significantly large is also tried `zstd`-compressed, and if that results in a smaller serialization then the compressed form is used instead.
...
** encodes into a `BitSet`, a `RunLength` and a sparse delta-list - each with a zstd-compressed twin when large enough to be worth compressing - then uses the smallest and tags as such in the encoded String
** Which is smallest can depend on the size and information density of the offset map
** Measured sizes for every encoding across a scenario corpus - and the reasoning for the set - are in link:docs/offset-encoding-density-benchmark.md[the offset-encoding density benchmark report]
