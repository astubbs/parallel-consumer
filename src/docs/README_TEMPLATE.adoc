...
* Pluggable handling of terminally failed offset commits - fail fast (the default) or continue past the failure under canned bounded policies - see <<commit-failure-handling>> section for more details
...
=== Handling Terminal Commit Failure

When an offset commit keeps failing until its retry budget (the `offsetCommitTimeout` option) is spent, the default is to shut down (fail fast) - the historical behaviour.
The `commitFailureHandler` option makes that decision pluggable ({issues_link}/317[astubbs#317], {base_confluent_url}/issues/833[confluentinc#833]): the handler receives a `CommitFailureContext` - the failure, the offsets it was for, and history such as consecutive exhausted budgets and time since the last successful commit - and returns `SHUT_DOWN` or `CONTINUE`.
Continuing leaves the offsets dirty, so they are retried with the next commit cycle.

Canned policies live in `CommitFailurePolicies`:

* `shutDown()` - always fail fast. The default.
* `continueBounded()` - continue until a bound trips, then shut down; the recommended way to opt in. The default bounds are 5 consecutive exhausted budgets, 5 minutes without a successful commit, or 20 exhaustions within a 30 minute rolling window.
* `continueUnbounded()` - never shut down for a commit failure; explicit opt-in only.

While continuing, the `commitFailureContinueMode` option governs record intake: `KEEP_PROCESSING` (the default) keeps working optimistically, while `PAUSE_INTAKE` stops taking on new work until a commit succeeds again, bounding the replay an eventual rebalance would cause.
In the transactional mode `KEEP_PROCESSING` is always coerced to `PAUSE_INTAKE` - EoS cannot keep producing past a failed transaction - and the in-flight transaction is recovered (completed if the broker in fact accepted it, otherwise aborted) before the offsets are committed again.

WARNING: On that abort lane, only the input side is recovered.
Records that already completed work had produced into the aborted transaction are never visible to `read_committed` consumers and are *not* re-produced - completed work has no replay machinery, so its source offsets simply stay dirty and commit with the next transaction, carrying no output with them.
Continuing across an aborted transaction is therefore at-most-once for those outputs; recovery prefers completion precisely to keep that window small.
If it is unacceptable, keep the default `shutDown()` - shutting down leaves those offsets uncommitted, so a restart redelivers the records, reruns the work and produces its output again.

IMPORTANT: The handler operates in the `PERIODIC_CONSUMER_SYNC` and `PERIODIC_TRANSACTIONAL_PRODUCER` commit modes only.
It does not operate in `PERIODIC_CONSUMER_ASYNCHRONOUS` (the default mode): asynchronous commits carry no retry budget and report failures only through a later callback, so there is no exhaustion event for a handler to act on.
Configuring either option in that mode is rejected when the options are validated.

A continuing-but-failing instance stays visible: four meters in the `committer` subsystem track exhaustions, the consecutive-failure streak, time since the last successful commit, and the seam's state - see <<metrics>>.

.Opting in to bounded continue
[source,java,indent=0]
----
include::{project_root}/parallel-consumer-examples/parallel-consumer-example-core/src/main/java/bz/stub/parallelconsumer/examples/core/CoreApp.java[tag=commitFailurePolicy]
----

<1> The handler needs a commit mode with a retry budget - sync (shown) or transactional, not the default async mode.
<2> Continue past terminally failed commits until a bound trips, then shut down.
<3> Optional: pause intake of new work while commits are failing, instead of processing optimistically.
...
**Commit Failure Exhaustions**

Counter `pc.commit.failure.exhaustions{subsystem=committer}`

Total number of exhausted offset-commit retry budgets. Each increment is one terminal commit failure consulting the configured commitFailureHandler - sync, transactional and escalated rebalance-deferral failures all count here

**Commit Failure Consecutive Exhaustions**

Gauge `pc.commit.failure.consecutive.exhaustions{subsystem=committer}`

Number of commit retry budgets exhausted in a row with no intervening successful commit. Resets to 0 on a successful commit and on a fresh partition assignment

**Commit Time Since Last Success**

Gauge `pc.commit.time.since.last.success{subsystem=committer}`

Seconds since the last successful offset commit. While no commit has succeeded in the current assignment yet, measured from the assignment's start (before the first assignment: from instance construction), so the gauge always has an epoch and never reports an absent-state sentinel

**Commit Failure Seam State**

Gauge `pc.commit.failure.seam.state{subsystem=committer}`

Commit-failure seam state, reported as number with following mapping - 0:HEALTHY, 1:FAILING_CONTINUING, 2:FAILING_PAUSED
