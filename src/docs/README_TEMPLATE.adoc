...
It is a *drop-in replacement*: the Java API and the Java package names (`io.confluent.parallelconsumer.*`) are unchanged from upstream 0.5.x, so your imports and your code are unaffected.
The Maven coordinate is the one edit: the `groupId` changes from `io.confluent.parallelconsumer` to `bz.stub.parallelconsumer`.
See <<upgrading>> for the upgrade steps.
...
0.6.0.0 is the first release of this community fork, and it changes the Maven coordinates.
Upstream's last release was published as `io.confluent.parallelconsumer`; this fork publishes as `bz.stub.parallelconsumer`.
...
The Java package names (`io.confluent.parallelconsumer.*`) and the API are unchanged, so no import or source edit is needed.
Committed offset metadata is also unchanged, so an existing consumer group can be upgraded in place without resetting or migrating offsets.
