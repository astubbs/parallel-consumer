...
CAUTION: These numbers come from this project's *chaos test suite*, which stops and restarts fleet
members every few hundred milliseconds for a solid minute while deliberately slow records are in
flight. It is built to be far more hostile than production, precisely so that effects like this are
visible. *Do not read these as rates you should expect* -- a normally-operating deployment rebalances
rarely, and would see a small fraction of this. Treat the table as showing the *relative* difference
between the four configurations, which is the part that carries over.
