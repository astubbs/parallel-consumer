...
The Java API is unchanged from upstream 0.5.x, but the Maven `groupId` and the Java packages *move*.
Your imports change; nothing you call does.
See <<upgrading-from-upstream,Upgrading from upstream 0.5.x>> for the one-line change.
...
=== From upstream 0.5.x to this fork

Two things move, and neither changes what you call.
The Maven `groupId` becomes `bz.stub.parallelconsumer`, and the Java packages move with it.

// The lines below MUST keep the old spelling: they are the instruction for getting off it, and a
// migration step that names only the destination tells the reader nothing. rename-packages.sh would
// otherwise rewrite them into a package moving to itself and a sed that does nothing - which passes
// every check the script has, because the sweep hunts old spellings that SHOULD have been rewritten.
// rename-packages: freeze-begin(upgrade-instructions)
Update your dependency declaration to the new `groupId`, then rewrite your imports:
...
The library's shared internal utilities moved with it, into `bz.stub.parallelconsumer.internal.utils` -- they were always plumbing, they are not part of the published API, and they are now named as such.
