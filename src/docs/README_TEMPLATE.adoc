...
NOTE: The `JStream*` variants of these processors (which return a `java.util.stream.Stream` of results instead of
taking a callback) are *deprecated for removal*. They buffer every result in an unbounded queue that only drains when
you consume the stream, and the returned `Stream` is single-shot - it terminates the first time the queue is
momentarily empty and cannot be restarted, so results produced after that accumulate until the JVM runs out of memory.
Use the callback form shown above. See link:https://github.com/astubbs/parallel-consumer/issues/122[astubbs#122].
