...
Handing PC a finished `Producer` instance works as it always has; it is the one path PC cannot rebuild a producer on, so `producerFactory` and the recovery in <<producer-recovery>> need `producerConfig`, and a factory beside an instance is refused at validation.
...
==== Moving from a producer instance to producer configuration

The `producer` option - a finished `Producer` instance - stays supported.
`producerConfig` (plus, optionally, `producerFactory`) is the path PC owns, and the only one that recovers when the broker invalidates the producer (see <<producer-recovery>>), so this is how to move when you want that:
...
*Known gap, not fixed:* the commit-interval default is applied by comparing your configured value against the default by reference identity, so explicitly setting `commitInterval(Duration.ofSeconds(5))` - the same value as the non-transactional default - is treated as "not set" and silently replaced with 100ms.
Any other explicit value survives.
See `docs/inflight/bug-commit-interval-identity-check.md`.
...
Supplying a finished `Producer` instance still works for every flow it worked for, and stays supported: PC cannot read a finished producer's configuration back out, so it can never build another from it - which is why anything that needs PC to build the producer refuses an instance at validation (`producerFactory` beside `producer` fails, naming both) and why recovery does not run on that path.
