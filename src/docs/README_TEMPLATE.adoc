...
*Fixed:* "was a commit interval explicitly set?" used to be answered by comparing your configured value against the default by reference identity, which had one narrow failure mode - explicitly passing back the `DEFAULT_COMMIT_INTERVAL` constant object itself was indistinguishable from never calling `commitInterval(...)`, and got silently replaced with 100ms.
Any other explicit value, including one merely equal in value to the default, was and still is kept.
See `docs/solutions/logic-errors/commit-interval-identity-check-was-narrower-and-inverted-2026-09-02.md`.
