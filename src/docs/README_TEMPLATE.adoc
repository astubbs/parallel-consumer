...
In the transactional commit mode PC sets `transactional.id` itself - an id derived from the consumer's `group.id`, unique per instance and reused by every replacement producer - so leave it out of that configuration; a value set there is replaced, and the replacement is reported at WARN. The <<producer-recovery,producer recovery>> section has the derivation and the ACL prefix it fits.
