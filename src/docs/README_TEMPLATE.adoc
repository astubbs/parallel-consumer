...
<4> Producer configuration, only required if using the `produce` flows: PC builds the producer from it through the factory on the next line (the default is `new KafkaProducer<>(config)`; override it to wrap or instrument the producer, keeping every key it is given) - see <<producer-ownership>>.
...
Nothing forces the move in this release; the configuration path is the one PC owns, and what PC does with a producer from here on is built on it - see <<producer-ownership>>.
To move:
