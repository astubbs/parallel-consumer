...
*Behaviour* is a drop-in replacement for upstream 0.5.x: the API is source-compatible apart from one removed accessor, and the committed offset format is unchanged, so an existing consumer group upgrades in place without resetting or migrating offsets.
...
One method was removed, and it is the only source edit the rename does not cover: `WorkManager.getSuccessfulWorkListeners()`.
Registration is now `wm.addSuccessfulWorkListener(listener)`, replacing `wm.getSuccessfulWorkListeners().add(listener)`.
The getter handed out the live internal list, so any caller could clear or reorder another component's listeners, and registering while the consumer ran could stop it outright.
It was an undocumented Lombok accessor on a field marked "useful for testing", so most codebases will not have called it -- but if yours did, it is a compile error rather than a silent behaviour change.

Beyond that, the API is source-compatible, so past the import lines no source edit is needed, and most codebases reference only a handful of types.
