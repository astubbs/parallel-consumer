...
CAUTION: *One guarantee is currently recorded as refuted, on one path.* "No record is produced whose source offset is excluded from the transaction being committed" holds for the periodic commit made by the control loop, which drains its completed-work mailbox before collecting offsets. It does *not* hold for the commit made while partitions are being revoked, which does not drain first -- so a rebalance can commit a transaction whose offsets omit a record that transaction already produced. The output is committed and its input offset is not, and the next owner of the partition reprocesses that input and produces the output again: on that path you get *at-least-once*, not exactly-once. The failure is deterministic and is pinned by a quarantined test with a passing control arm. It is not yet fixed, because the correct fix is a thread-ownership change at the commit seam rather than a local one. If you rely on exactly-once across rebalances, make your output sink idempotent until this is closed.
