...
----
<1> Connection properties in. You construct no Kafka clients; the library builds them, and sets the serialisers itself because each route declares its own.
<2> One topic bound to one function, with its own types -- `json(Order.class)` resolves to the JSON deserialiser already on your classpath, and `context.value()` is an `Order`.
<3> A *filtered* outcome: the record is finished with, commits like a success, and is counted separately. No exception, no sentinel return.
<4> A normal return is success.
<5> A second topic with a different value type. Two routes, two functions -- not one handler switching on `record.topic()`.
<6> After two further attempts, two hundred milliseconds apart, this record *parks in place*: it stays incomplete in the offset map, holds no worker, and the partition commits past it. Nothing was configured to make that possible and no dead-letter topic exists.
<7> Told once, when a record gives up.
<8> Your own client. An exception here is a retry.
...
----
<1> A handle out, usable with try-with-resources. Its close drains.
...
<2> Stop publishing after ten seconds and close, draining first. Without a bound the run ends when you close the handle.
...
* A <<fluent-api,fluent API>> (incubating) beside the classic options-builder one: connection properties in, one typed route per topic with its own function and policy, and a handle out
** <<park-in-place,Park in place>> - a record that runs out of attempts stays where it is, holding no worker, while the partition commits past it, so a dead-letter topic is a relief valve rather than a requirement
...
The *fluent API* is the one the example at the top of this page uses: connection properties in, one typed route per topic with its own function and its own policy, and a handle out.
...
Here the map already commits past an incomplete record, so nothing has to be copied anywhere, and *a definition that declares no dead-letter destination is complete*.
...
| `PARTITION`
| Its partition, in full. Partition ordering means records are processed in offset order, so a parked record at the head stops the partition. Park serves key and unordered processing; a partition-ordered instance should declare a dead-letter destination instead.
|===

Three more consequences, none of them defects, all of them things an operator will see:

* *Consumer-group lag reads as stuck at the oldest parked record.* The committed offset cannot advance past an incomplete one, so `kafka-consumer-groups` shows the distance from that offset even while everything after it is being processed and committed in the metadata. The honest figure is on the handle -- the count of unresolved records -- and in the parked-count metric.
* *A parked record is reported as slow work in this release.* Parking is, over today's engine, a retry with a far-future delay, so the shard scan counts every parked record against its partition's slow-records meter and names its topic in the periodic "records in the queue have been waiting longer than" warning. It is noise rather than a symptom. Removing it is a small engine change, and a test pins today's behaviour so the change shows up rather than happening in silence.
* *A close that drains waits for records queued behind a parked key.* The handle closes draining by default, and a drain waits for the work the shards still hold. Under `KEY` ordering, records queued behind a parked key are work the engine can never take, so such a close waits out its whole drain timeout and then reports the timeout. Either close without draining, or keep in mind that the wait is bounded by `drainTimeout` and nothing is lost by it.
...
`resume` and `dlq` on the handle refuse in this release and say so -- they need engine commands that do not exist yet, and they arrive in the next milestone.
...
*A route whose parked records must survive a restart parks with a dead-letter destination or an age bound.*

[[parked-set]]
=== Dead-lettering, and the parked set

A store nobody can see is a leak, so the parked set is queryable per route from the handle.
...
----
<1> One route's parked set, across every partition it is assigned. `handle.topic(...)` names a route by any of its topics, and a topic nothing routes is refused rather than answered with an empty set.
...
The same figures are published through the <<metrics,metrics integration>>, under the `routes` subsystem: outcome counts tagged by topic and outcome, and the parked count and oldest parked age tagged by topic and partition.

*Copying a record to another topic is called `dlq` here, and it is a verb.*
It is the relief valve for park rather than the first resort: `dlqTo` names the destination, `dlqWhenOffsetPayloadReaches` fires when a partition's offset-map payload reaches a percentage of Kafka's commit-metadata cap, `dlqOlderThan` fires before the topic's retention could delete a parked record, and `dlqImmediately` is the classic dead-letter queue -- export on the dispatch after exhaustion, with no park in between.

NOTE: *Dead-lettering does not run in this release.* A definition that declares a destination is refused at start, naming the topic, rather than accepted and silently doing nothing -- which is the one outcome worse than a refusal. The API is documented here because the shape is settled; what is missing is the engine accessor the payload trigger reads. Until it lands, park in place is the whole of the story, and for the ordering modes above it is enough.

An exported record will carry the original key bytes, value bytes and headers unchanged, plus provenance headers naming the source topic, partition, offset, timestamp, attempt count, and the last failure's time, class and message.
...
<3> *Dead-letter* -- when the record must leave the topic rather than sit in it. That is a capacity or a retention question, not a severity one: the offset map is bounded, and a parked record only survives a restart if it has somewhere to go.
<4> *Stop* -- when a record running out of attempts means the deployment is wrong rather than the record is. A schema nobody deployed a reader for, a downstream that will never come back: continuing would burn through the topic parking everything.

The three are not a severity ladder. Park is about *this record*, dead-letter is about *capacity*, and stop is about *the deployment*.
...
*A route binds one topic to one function.* Registering a second route for a topic already routed is refused at definition time, as is naming one of your own routed topics as a dead-letter destination, or supplying a deserialiser in the connection properties -- every check runs before a connection is opened, and every message names the topic or the setting.
...
*Almost nothing is instance-wide.* The engine has one consumer, one commit and one transaction, so the commit mode is a property of the instance; everything else is a per-route setting with an instance default, and the two are named apart -- instance-wide settings plain, per-route defaults with a `default` prefix.
...
[source,java,indent=0]
----
include::{project_root}/parallel-consumer-examples/parallel-consumer-example-core/src/main/java/bz/stub/parallelconsumer/examples/core/FluentApiSnippets.java[tag=fluentHandle]
----
...
<3> An instance can end in three ways, and the handle says which: closed by you, stopped by a route, or failed. This is the second.
...
* *Per-route policy* -- retry limit, delay, concurrency, dead-letter destination.
...
On the *fluent API* a route declares a retry limit -- ten attempts by default -- and what happens when a record reaches it: <<park-in-place,park in place>>, dead-letter, or stop the instance.
...
* Dead-lettering itself -- the API is settled and refused at start until the engine accessor it reads exists (<<parked-set,Dead-lettering, and the parked set>>).
* `resume` and `dlq` as commands on the handle, so a parked record can be acted on without a restart.
...
**Route Parked Records**

Gauge `pc.route.parked.records{subsystem=routes, topic="topicName", partition="partitionNumber"}`

Number of records parked in place right now on this partition - the size of the set an operator can act on, which is not the parked counter above

**Route Parked Oldest Age**

Gauge `pc.route.parked.oldest.age{subsystem=routes, topic="topicName", partition="partitionNumber"}`
