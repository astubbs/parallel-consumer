...
<5> Distinguishes this instance's `pc_*` series from another instance's.
Stable, and not a random UUID per restart: a fresh value on every deploy churns the whole series set for no benefit.
...
.The same question from Parallel Consumer's own side, for the panel beside it
[source,promql]
----
...
See the link:{project_root}/parallel-consumer-examples/parallel-consumer-example-metrics/src/main/java/io/confluent/parallelconsumer/examples/metrics/OrderFulfilmentApp.java[order fulfilment example] and its link:{project_root}/parallel-consumer-examples/parallel-consumer-example-metrics/src/test/java/io/confluent/parallelconsumer/examples/metrics/integrationTests/OrderFulfilmentAppMetricsTest.java[test], which scrapes the endpoint with a real Prometheus and asserts that the two in-flight meters agree, that the latency series really are buckets, and that no order id ever became a tag value.
