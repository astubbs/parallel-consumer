...
* A <<sandbox,sandbox>> that runs any definition - fluent or classic - with no broker, no Docker and no test environment, against generated records of the definition's own types
...
*A route binds one topic to one function.* Registering a second route for a topic already routed is refused at definition time -- a second function over the same topic is a second definition with its own consumer group, which is Kafka's own model for it and gives each function its own progress, retries and parked set -- as is naming one of your own routed topics as a dead-letter destination, or supplying a deserialiser in the connection properties -- every check runs before a connection is opened, and every message names the topic or the setting.
...
`parallel-consumer-sandbox` runs *any* definition with no broker, no Docker and no test environment, against records it generates itself: each route's declared value type filled with realistic random data, encoded with that route's own serialiser, paced at a rate you declare, with an optional bound after which the run closes itself.
...
Two things follow from that. It is the *demo*: a new reader can run a real definition against real records without provisioning anything. And it is the *broker-free test kit*: a test drives the same sandbox with its own records and asserts on outcomes and the parked set, which is how this project's own primary build signal works. It serves the classic API too, so an existing options-builder application runs broker-free with its start call changed and nothing else.
