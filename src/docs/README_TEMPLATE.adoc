...
*A driver that publishes for you* -- for a soak or a demo, where the point is volume rather than particular records. You give it a function from a record's index to its value, per topic (`feeding("orders", index -> anOrder(index))`); it encodes with that route's own serialiser, paces at a rate you declare, and takes an optional bound after which it settles and closes the run itself. That is what a sandbox does unless `handPublished()` says otherwise.

The sandbox has *no third-party dependencies*: it is the engine plus the mock clients that already ship with it. It knows how to pace and stop, and nothing about what an order looks like -- so a routed topic that nothing has said how to fill is refused when a driven sandbox starts, naming the topic.
