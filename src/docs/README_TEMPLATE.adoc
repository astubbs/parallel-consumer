...
| `parallel-consumer-streams-spike`
| 8
...
Maturity belongs to a module, not to one project-wide version number. Reliability and API stability are separate:
0.6.0.0 ships only once every known defect in the stable-module release scope is resolved and evidenced, while their public APIs remain pre-1.0. An
alpha artifact is a separate, explicit opt-in and cannot change an application that does not depend on it.

This makes v6 a special stability release. Virtual threads, micro-batching and a dead-letter queue are intentionally
deferred capabilities, not exceptions to the known-defect gate. The claim does not deny that unknown defects can exist;
it says the known defect ledger is cleared before the release is cut.
...
| `parallel-consumer-core`
| Stable
| Release-gated for all known defects
...
| `parallel-consumer-streams-spike`
| Alpha / experimental
| Module-specific evidence only; does not inherit stable confidence
| Volatile while the experiment is evaluated
| You want to evaluate the Kafka Streams integration and can tolerate changing behaviour and APIs.

| `parallel-consumer-connect`
| Alpha / experimental
| Module-specific evidence only; does not inherit stable confidence
| Volatile while the experiment is evaluated
| You want to evaluate running supported Kafka Connect sink tasks in-process.
|===

The experimental artifacts are opt-in leaves. Their rows in link:docs/data/module-maturity.yaml[the
module-maturity data] carry what is and is not claimed for each, and should be read before depending on
either one. For the reliability evidence and release-time limits behind the stable
module claim, see link:docs/data/testing-evidence.yaml[the testing evidence data] and
