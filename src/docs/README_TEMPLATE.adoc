...
CAUTION: *Two guarantees are currently recorded as refuted, both on the same path.* "No record is produced whose source offset is excluded from the transaction being committed", and "a source offset and its produced records will be committed as an atomic set", both hold for the periodic commit made by the control loop, which drains its completed-work mailbox before collecting offsets. It does *not* hold for the commit made while partitions are being revoked, which does not drain first -- so a rebalance can commit a transaction whose offsets omit a record that transaction already produced. The output is committed and its input offset is not, and the next owner of the partition reprocesses that input and produces the output again: on that path you get *at-least-once*, not exactly-once. The failure is deterministic and is pinned by a quarantined test with a passing control arm. It is not yet fixed, because the correct fix is a thread-ownership change at the commit seam rather than a local one. If you rely on exactly-once across rebalances, make your output sink idempotent until this is closed. The front-page promise that results exist exactly once under failure is *not* recorded as refuted: the route from an omitted offset to a duplicated result is sound but has not been observed, and that register deliberately separates what was seen from what was argued.
...
One thing did change there: an `InvalidPidMappingException` on the produce path still closes the instance, but the batch is now failed and its offsets are not committed, where previously the close swallowed the exception and offsets were published for output that was never produced.
...
Counter `pc.producer.recoveries{subsystem=producer, condition="exceptionSimpleName"}`

Total number of times the transactional producer was replaced after the broker reported it invalid (fenced, stale epoch, expired producer id, lost generation), tagged by the condition that triggered it. Distinct from ordinary commit failures, which are retried on the same producer

**Producer Consecutive Recoveries**

Gauge `pc.producer.consecutive.recoveries{subsystem=producer}`

Producer recoveries since the last successful commit - non-zero for a moment after any recovery, rising while the instance is alive but not progressing
