...
*Verified against a real broker, each with a negative control that was observed to fail:* all parallel workers share one bulk transaction; an open transaction is invisible at `read_committed` while a `read_uncommitted` reader sees it; an aborted or timed-out transaction never becomes visible; a crash before commit leaves neither the results nor the source offset, and the replay commits them together with each result present exactly once; no record reaches the broker whose source offset is excluded from the transaction being committed; processing is blocked for the duration of the commit lock; and the commit interval is reduced to 100ms only when you have not set one.

Also verified: if one `send` in a `pollAndProduceMany` result set fails terminally - for example a record over `max.request.size` - none of that result set becomes visible.
That case used to leave a *partial* result set visible to a `read_committed` consumer, silently, because a producer callback threw and pre-empted the Kafka client's own transition of the transaction into an abortable state.
