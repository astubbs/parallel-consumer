...
*Two documented guarantees do NOT currently hold, and you should design around them:*

* If one `send` in a `pollAndProduceMany` result set fails terminally - for example a record over `max.request.size` - the records already accepted stay in the transaction and the next commit publishes them. A `read_committed` consumer then sees a *partial* result set for that source offset. The instance does not fail or shut down.
* Consequently "all records produced from a given source offset will either all be visible, or none" is not true in that case either.

Both have the same cause: a producer callback that throws, which pre-empts the Kafka client's own transition of the transaction into an abortable state.
Until that is fixed, do not rely on all-or-none for multi-record result sets where a send can fail terminally.
