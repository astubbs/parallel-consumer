...
* `JStreamParallelStreamProcessor` -- *deprecated for removal*, see <<callbacks-vs-streams>>
* `JStreamVertxParallelStreamProcessor` -- *deprecated for removal*, see <<callbacks-vs-streams>>
...
Use the callbacks. The `Streaming` versions of the API, which hand back a `java.util.stream.Stream` of results, are *deprecated for removal*:

* `JStreamParallelStreamProcessor`
* `JStreamVertxParallelStreamProcessor`

WARNING: They buffer every result in an unbounded queue that only drains when you consume the stream, so a slow or absent consumer grows it until the JVM runs out of memory -- and the returned `Stream` is single-shot, terminating the first time the queue is momentarily empty and unable to restart, after which everything produced accumulates behind a consumer that has already finished.
Closing does not rescue what you have not read: it discards the queue, and `closeDrainFirst()` drains the queued *processing* rather than the stream.
There is no bounded version to move to -- see link:https://github.com/astubbs/parallel-consumer/issues/122[astubbs#122] for why -- so take results through the callback forms shown above instead.
...
<2> Called once the request has been dispatched
<3> Called with the response - handle each result here, as it completes
