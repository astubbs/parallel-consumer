...
Two identifiers do change in 0.6.0.0, and both are mechanical: the Maven `groupId`, and the Java package names, which move from `io.confluent.parallelconsumer.*` to `bz.stub.parallelconsumer.*`.
See <<upgrading>> for the steps.
