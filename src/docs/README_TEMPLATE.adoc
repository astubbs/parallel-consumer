...
<4> Producer configuration, only required if using the `produce` flows: PC builds the producer from it, and rebuilds it if the broker invalidates it - see <<producer-recovery>>.
