...
Supplying a finished `Producer` instance still works for every flow it worked for, is deprecated, and does not recover: PC cannot read a finished producer's configuration back out, so it cannot build a replacement, and on that path the broker-reported conditions keep their earlier behaviour.
