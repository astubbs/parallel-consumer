...
*Behaviour* is a drop-in replacement for upstream 0.5.x for most users: the committed offset format is unchanged, so an existing consumer group upgrades in place without resetting or migrating offsets, and the API changes are the short list under Breaking in the link:CHANGELOG.adoc[changelog].
