...
*A route naming a topic the cluster does not have fails the start, by name.* This is the one check that does reach the cluster: `start()` asks it once, before the instance joins the group, and refuses a definition it cannot satisfy. What it replaces is silence -- Kafka's consumer auto-creates a topic it subscribes to unless the broker forbids it, so a misspelled name became an empty topic with a partition count nobody chose, and where the broker did forbid it the route simply processed nothing. Declare `whenTopicMissing(MissingTopic.CREATE)` to have the start create what is missing, with the broker's own partition count and replication factor, or `whenTopicMissing(MissingTopic.IGNORE)` to start anyway with the missing topics named in a warning.
