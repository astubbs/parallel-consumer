...
`resume` and `dlq` on the instance refuse in this release and say so -- they need engine commands that do not exist yet, and they arrive in the next milestone.
...
It is the relief valve for park rather than the first resort: `dlqTo` names the destination, `dlqAtOffsetPayload` fires when a partition's offset-map payload reaches a percentage of Kafka's commit-metadata cap, written `percentOf(...)` so the unit is on the value rather than in the reader's head, `dlqOlderThan` fires before the topic's retention could delete a parked record, and `dlqImmediately` is the classic dead-letter queue -- export on the dispatch after exhaustion, with no park in between.

Those four qualify a record that parks.
`afterRetries(dlq("payments.dlq"))` is the reaction itself, beside `park()` and `stop()`: the record is copied out on the dispatch that exhausts it and never enters the parked set, so there is nothing left for a trigger to qualify and declaring one beside it is refused.
Reach for it when the record has to leave the topic rather than sit in it; reach for a parking policy with a trigger when it may sit there until something forces the issue.
...
* `resume` and `dlq` as commands on the instance, so a parked record can be acted on without a restart.
