...
<4> *What a record contains is yours*: a function from the record's index to its value. The sandbox knows how to pace and stop; only your application knows what an order looks like. A test that wants particular records skips the driver entirely and publishes them itself -- see <<sandbox,the sandbox>>.
...
* A <<sandbox,sandbox>> that runs any definition - fluent or classic - with no broker, no Docker and no test environment: publish your own records, settle, and assert, or let it make up records of the definition's own types
...
*Publish, settle, assert* -- for a test, which knows its own data:
...
try (ParallelConsumerInstance instance = pc.start(sandbox)) {
    sandbox.publish("orders", "cust-1", new Order("o-1"));     // <1>
...
----
<1> Encoded with the route's own serialiser -- you do not supply one, because the route already declared both halves of its format. Published from your thread, and placed by the key, so key ordering means here what it means against a broker.
