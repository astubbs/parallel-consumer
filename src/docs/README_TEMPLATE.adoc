...
In the core module this is the size of the worker pool: a fixed `ThreadPoolExecutor` whose core and maximum size are both `maxConcurrency`, its platform threads created on demand as work arrives and kept once created.
A value far above the core count is right for IO-bound functions, because a thread parked on an HTTP call or a database write costs stack memory and little else; for a CPU-bound function it is context-switch cost, so set it near the core count instead.
It also sizes the buffer: the in-flight target is `maxConcurrency * batchSize * loadFactor`, so it decides how many records are held in memory as well as how many are processed at once (`messageBufferSize` fixes that number independently, see <<ordered-by-partition>>).
In the Vert.x, Reactor and Mutiny modules it is a ceiling on outstanding asynchronous work, not a thread count.
...
IMPORTANT: `batchSize` is a maximum, not a target. A batch is whatever work is eligible at the moment it is
assembled, sliced into chunks of at most `batchSize`; the library never waits for a batch to fill. A consumer that
keeps up with its input therefore sees small batches, often of one record, and only sees full batches once lag
builds. With key or partition ordering the slice is drawn from what the shards can offer at that instant, so it is
bounded by eligible work, not by the topic's backlog. Waiting until N records or T elapses (micro-batching) is a
deferred capability, not a setting - see <<roadmap>>.
...
[[several-instances]]
=== Several instances in one application: isolation versus sharing

An instance is the isolation boundary. Each `ParallelStreamProcessor` owns its `KafkaConsumer` and `group.id`, its worker pool sized by `maxConcurrency`, its buffer and its commit cadence, and pauses its own poller when its own buffer fills. Nothing is shared between two instances in one JVM unless you share it.

So for a service with more than one workload -- a fast command topic and a slow, heavy event topic, say, or the stages of a fan-out pipeline -- run one instance per workload, each with its own consumer group. Each stage's lag is then its own signal, back-pressure is per stage, and a slow workload cannot starve a fast one, because they do not share a pool. The `pcinstance` metrics tag (see <<metrics>>) keeps their meters apart in one registry.

The reasons to share one instance across topics are fewer: one group to operate, one set of threads. The costs are what sharing always costs here -- interference between workloads, lag and back-pressure that look global, and no in-library scheduler to make one instance fair across its topics. Prefer separate instances unless the workloads are alike.

Two things not to share: a `KafkaConsumer` between instances, ever; and a `Producer` between instances in the transactional commit mode, where each instance needs its own `transactional.id`. Two instances in the *same* `group.id` on one JVM only split the partitions between them and isolate nothing.

Thread cost is the sum of `maxConcurrency` across instances, in the core module a fixed pool of platform threads per instance, so size each stage for what its downstream can take rather than for speed. Limiting one stage by another's capacity is not built in: today it is that stage's `maxConcurrency`, or `pauseIfRunning()` and `resumeIfPaused()` driven by a monitor of the downstream group's lag. Group-wide rate limiting and self-tuning concurrency are on the <<roadmap>>.
...
Maximum number of records queued in any single shard (the most-loaded shard) - useful for spotting hot-key shards under KEY ordering
...
Building from source needs *JDK 17* - not the Java 8 the library runs on, and not a newer JDK either.

The parent `pom.xml` sets `source.version` to 17 and `release.target` to 8: the sources are Java 17, and the command-line build compiles them to Java 8 bytecode with the https://github.com/bsideup/jabel[Jabel] javac plugin (`--release 8`, see the `maven-compiler-plugin` configuration).
That is why the jar runs on Java 8 while the code that produced it does not compile there.

* *JDK 17, specifically.* The Maven enforcer's `requireJavaVersion` rule rejects anything older than `source.version`, and JDK 21 or newer fails in the other direction: Jabel 1.0.0 bundles a Byte Buddy that refuses class-file version 65, so `javac` cannot load the plugin at all.
CI builds on Temurin 17, and that is the toolchain this repository is tested on - the measurements are in link:{project_root}/docs/solutions/build-errors/error-prone-jabel-and-the-jdk-that-satisfies-neither-2026-08-25.md[docs/solutions/build-errors/error-prone-jabel-and-the-jdk-that-satisfies-neither-2026-08-25.md].
* *The Mutiny module is the exception.* SmallRye Mutiny is itself compiled for Java 17 and implements `java.util.concurrent.Flow`, which a release level of 8 hides, so `parallel-consumer-mutiny` overrides `release.target` to 17 and emits Java 17 bytecode - the same floor it needs at runtime, see <<java-version-per-module>>.
Core, Vert.x and Reactor stay at Java 8.
* *IntelliJ IDEA does not use Jabel.* Importing the project activates the `intellij-idea-only` Maven profile, which compiles at `source.version` for `source`, `target` and `release` alike and drops Jabel from the annotation processors, because the IDE's compiler cannot run it.
Plain `mvn` on the command line uses Jabel with `--release 8` as above.
* *Tests run on the JDK that launched Maven.* Surefire and Failsafe fork `${jvm.location}/bin/java`, which defaults to `${java.home}`.
The `jvm8-release` and `jvm9-release` profiles point it at a `jvm8.location` / `jvm9.location` property instead, for running the suite on the target JVM before a release; the pom marks `jvm8-release` as currently broken.

The first command in a fresh clone, and the generated-sources trap behind it, are in <<Maven targets>> and link:{project_root}/docs/building.md[docs/building.md].
