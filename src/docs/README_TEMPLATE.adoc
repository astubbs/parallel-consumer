...
Two guarantees are attributed rather than re-proved here, and are marked as such in the register: the commit-lock timeout failing fast, and the produce-lock timeout returning the record for retry.
Both are covered by existing tests, but no negative control was observed for either, so neither is recorded as proved.
