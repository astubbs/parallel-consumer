...
* Adaptive concurrency (experimental): rather than trusting a static `maxConcurrency` guess, measure how the workload responds and discover the sustainable in-flight target - with an observe-only mode that reports what it would do without acting, see the <<adaptive-concurrency>> section
* Global rate limiting (experimental): declare a resource and its rate once, and every instance of a consumer group respects it collectively - each instance's share is the fraction of the partitions it holds, so a rebalance re-divides the rate with no controller and nothing on the record path, and a waiting record can say which resource it waits on and when its next credit mints, see the <<global-rate-limiting>> section
...
WARNING: *Experimental.* This surface may change in a minor release, and enforcement is *unvalidated* - see <<adaptive-concurrency-status>> before turning it on. It is designed to be adopted observe-first.

`maxConcurrency` is a compile-time guess about a runtime quantity.
The right value depends on the capacity of whatever your processing function talks to, on the data currently in this instance's assigned partitions, and on the time of day - so one static number is wrong in both directions.
Too low silently strands throughput that nobody files an issue about; too high floods the downstream system.
From the outside, the two look much the same: a plateau, with no way to tell whether the cap, the host, the broker or the downstream service is holding it there.

Adaptive concurrency measures how service time responds to the work currently in flight and moves the *admission target* - the number of records allowed in flight at once - between a floor of one slot and a ceiling it may never exceed.
The thread pool is not resized; admission is the control variable, and it is the same variable that already governs dispatch quantity and broker back-pressure.
...
| `DISABLED`
| The default. The controller does not exist, and concurrency behaves exactly as it always has - `maxConcurrency` is both the pool size and the in-flight target.

| `OBSERVE`
| The controller measures, decides, and reports what it *would* do - through metrics and a rate-limited log line - while changing no runtime behaviour at all. This is the safe first step, and the one to adopt today.

| `ENFORCE`
| The controller's target actually steers dispatch and broker intake, between one slot and the ceiling.
|===
...
Options are immutable, so the mode is fixed at construction: *promoting `OBSERVE` to `ENFORCE` is a restart*, not a runtime call.

The mode also defaults from the `pc.adaptiveConcurrency` system property, so an existing deployment can be put into observation without touching its source.
*The property can select at most `OBSERVE`*: a property value of `ENFORCE` resolves to `OBSERVE` and logs a WARN.
Enforcement changes what your application does to somebody else's system, so it has to be a deliberate line of code rather than a launch flag.

`adaptiveConcurrencyInitialTarget` optionally seeds the starting target - useful when you already know roughly where the sustainable point is and do not want to spend the deploy window ramping into it.
Left unset, the controller starts from the target today's static configuration derives, so at startup admission is never above what you run now.
The seed is validated against the floor and the ceiling, and setting it while the mode is `DISABLED` is a configuration error rather than a silent no-op.

In `ENFORCE`, `maxConcurrency` is the ceiling the controller may never exceed - with one documented exception.
Leaving `maxConcurrency` at the library default of 16 selects a higher adaptive default ceiling of 64 instead, because keeping the hand-tuning default while asking for adaptive behaviour is a contradiction.
Set `maxConcurrency` explicitly whenever you want a specific ceiling.

=== Where it does not apply

The controller serves the core engine's pre-loaded queue path only.
The engines that dispatch into an external runtime - Vert.x, Reactor and Mutiny - and the measurement-only direct-pull engine cannot serve it, because the concurrency an admission controller would steer lives outside anything this library sizes.
Setting `adaptiveConcurrencyMode` on any of them *fails construction*, naming the engine and why: you asked for something that will not happen, and a warning you never read gets you to "this feature is broken" rather than "I configured it wrong".
The one exception is a mode picked up from the `pc.adaptiveConcurrency` system property instead of set in your code - that is a JVM-wide ambient default, not a request, so it keeps the older behaviour of one WARN at construction and running static.
It is never silently ignored.

=== What to watch

All four meters are in the `processor` subsystem - see the <<metrics>> section for the full listing and setup.

`pc.admission.target`:: The live target, in slots. Under `OBSERVE` this stays at the static `maxConcurrency`-derived value; it moves only under `ENFORCE`.
`pc.admission.would.be.target`:: The target the controller *would* publish. This is the whole product of `OBSERVE`: watching it against the live target tells you how wrong your static guess is, and in which direction, before you let anything act on it.
`pc.admission.constraint`:: Which constraint bound the most recent sample window, as an enum-valued gauge. *This is the meter that answers "why is it not moving".*
`pc.admission.movements`:: How many times the target actually changed value. A window that closes on a hold, or on a clamp landing on the same slot count, does not count - so a flat movement count next to a busy constraint gauge means the controller is deciding and finding nothing to do.

The constraint values are `APP_LIMITED` (too few samples to judge, or the application is not filling the target - the window says nothing about downstream capacity), `BACKOFF` (an admission was dropped for overload), `FAILURE_LIMITED` (the non-success fraction froze growth - a fast-failing downstream *lowers* measured service time, which a naive controller would read as headroom), `PROBING` (a bounded step taken to test a suspicion), `ADAPTING` (ordinary gradient adaptation), `AT_CAP` and `AT_FLOOR` (the clamps bound), and `COOLDOWN` (a rebalance discarded the measurement history).
Zero means no window has closed yet.
The numeric mapping is published in the meter description, and the values are hand-assigned rather than ordinals so that adding a reason never renumbers one your dashboard is already keyed on.

[[adaptive-concurrency-ordering-starvation]]
=== Where admission is not the binding constraint

Adaptive concurrency can only help a workload whose throughput is bounded by *how much is in flight*.
Under `KEY` or `PARTITION` ordering with a skewed key distribution, it is not: the shards serialise, and the instance can sustain a single record in flight of a configured twenty-four while every slot sits empty and service time stays flat.
No admission controller can raise throughput there, because admission is not what is holding it back.

The controller is built to recognise that signature rather than mistake the idle slots for headroom - sustained in-flight far below the target, with a narrow spread and flat service time.
It takes one bounded probe upward (`PROBING`) to check that it is not looking at starvation it manufactured itself, and otherwise holds and reports `APP_LIMITED`.
So on an ordering-starved workload the honest outcome is a *diagnosis*, not a speed-up: the fix is the ordering mode, the key distribution, or the buffer sizing described under <<ordering-guarantees>>, not the concurrency number.

[[adaptive-concurrency-status]]
=== Status: observe-first, and enforcement unvalidated

The measuring harness that would establish what `ENFORCE` is worth lands separately, and *no benchmark result has been published yet*.
Until one is, this documentation describes enforcement; it does not recommend it.
`OBSERVE` is the mode to adopt: it is a diagnostic that costs nothing to run, and it answers a question - how far the sustainable target is from your configured one - that you currently cannot answer at all.

[[navigator]]
[[global-rate-limiting]]
== Global Rate Limiting (the Navigator)

WARNING: *Experimental.* The navigator surface is under active development and may change in a minor release - the `resourceTags` option carries the same warning on the field itself.

A rate limit belongs to the service being called, not to the process calling it.
Ten instances sharing one 100/s vendor API each need a tenth of it, and that tenth changes every time an instance starts, stops or dies.
The usual answers are to divide the number by hand - wrong the first time an instance fails - or to stand up a shared cache and coordinate through it, which is a second distributed system in the path of every record.

The navigator does neither, because the consumer group has already done the division.
*Your share of a resource is the fraction of the subscription's partitions you currently hold*: an instance holding three of four partitions takes three quarters of the declared rate, the instance holding the fourth takes a quarter, and when one instance dies the group's own rebalance grows the survivors' shares to match.
Credit is minted locally from the assignment the instance already has, so there is no controller to elect, no control topic, and nothing distributed on the record path.

You declare two things on the options, and nothing inside your processing function.
`resourceContracts` registers a resource - its name, its rate, the quantum credit mints on, and its burst.
`resourceTags` names the resources this instance's function consumes.
`allocationStrategy` chooses how the share is decided, and defaults to `PARTITION_SHARE`, the strategy described above; `IN_PROCESS` shares one application-supplied allocator between instances in a single JVM, and `CUSTOM` takes an allocator of your own.
The `allocationStrategy` and `AllocationStrategy` javadoc carry the rest, including the combinations that deliberately fail validation rather than silently ignoring what you configured: an allocator supplied alongside the default strategy would never be used, so it fails naming both fields, and selecting `IN_PROCESS` or `CUSTOM` without one fails naming the omission.

[source,java]
----
ParallelConsumerOptions.<String, String>builder()
        .consumer(consumer)
        .resourceContracts(Collections.singletonList(
                new ResourceContract("api-x", 2.0, 2, Duration.ofSeconds(1))))
        .resourceTags(Collections.singletonList("api-x"))
        .build();
----

*What the fleet is promised is stated in one place and deliberately not repeated here:* the `ResourceContract` javadoc.
It covers credits per quantum, burst as one fleet-wide budget divided by share, the no-overshoot rule under a stable assignment, the bounded overshoot across a rebalance together with the domain that bound was measured over, the utilisation property, the per-consumer-group scope, and the convergence clock after an instance leaves or joins.

Waiting for credit is explained, not silent.
`PollContext.getNavigatorView()` gives the processing function a read-only view of why its own work is held: `blockingResourceDeferrals()` returns each resource currently withholding work paired with the instant its next credit mints - "waiting on api-x, next credit at T" - beside the per-shard deferred counts and this instance's current share.
The same facts reach a dashboard as meters under `pc.navigator.*` - the deferral reason and episode counts, the conservation counters, and `pc.navigator.share.fraction` and `pc.navigator.share.credits.per.quantum` for the share itself - each carrying its own description; see the <<metrics>> section for registry setup.

The proof is a measurement rather than a claim.
`NavigatorPartitionShareIT` runs the storyline across real JVM process boundaries, and `NavigatorChurnLadderIT` churns a ladder of instances under both assignment protocols with clock skew injected per process, publishing the measured overshoot curve as a dated record written by the run itself and committed under link:docs/test-hardening/[`docs/test-hardening/`] with the CI run that produced it.
`bin/demo-navigator.sh` shows the same storyline for human eyes; link:docs/demos.md[`docs/demos.md`] explains why a demo is never the evidence.
...
Gauge `pc.admission.target{subsystem=processor}`

Adaptive concurrency - the LIVE admission target in slots, the number of user function invocations that may be in flight at once. Moves only under adaptiveConcurrencyMode=ENFORCE; under OBSERVE it is the static maxConcurrency-derived value

**Admission Would Be Target**

Gauge `pc.admission.would.be.target{subsystem=processor}`

Adaptive concurrency - the target the controller WOULD publish in slots. The whole product of adaptiveConcurrencyMode=OBSERVE, where it moves while the live target stays static; under ENFORCE it equals the live target

**Admission Constraint**

Gauge `pc.admission.constraint{subsystem=processor}`

Adaptive concurrency - which constraint bound the most recent closed sample window, reported as number with following mapping - 0:NONE, 1:APP_LIMITED, 2:BACKOFF, 3:FAILURE_LIMITED, 4:PROBING, 5:ADAPTING, 6:AT_CAP, 7:AT_FLOOR, 8:COOLDOWN

**Admission Movements**

Counter `pc.admission.movements{subsystem=processor}`

Adaptive concurrency - number of times the admission target actually changed value. A window that closes on a hold, or on a clamp landing on the same slot count, does not count
