...
| `maven.yml` | The main build: unit, integration and performance suites, static analysis (Infer and SpotBugs, batched as `static: analysis`), and duplicate detection, dependency review and the whole-tree CVE scan (batched as `scan: repo`). PR-scoped PIT is `Mutation Tests (PIT, PR-scoped)` - advisory, and its own job: a required check must not wait on a non-gating lane whose runtime is bimodal. |
