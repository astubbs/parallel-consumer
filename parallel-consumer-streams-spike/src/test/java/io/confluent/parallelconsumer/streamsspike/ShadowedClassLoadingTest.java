...
/**
 * Proves the classpath-shadowing premise this whole spike rests on: the classes we generate and compile win
 ...
 * nothing at all. That is the single most likely way this spike produces a false positive, so it is asserted
 * directly rather than inferred from behaviour.
 *
 * @see io.confluent.parallelconsumer.streamsspike.integrationTests.ShadowedStreamsControlTest
 */
@Slf4j
class ShadowedClassLoadingTest {

    /**
     * The classes listed in {@code spike.patched.classes} in this module's pom. Keep in sync: a class that is
        ...
        for (Class<?> generated : GENERATED) {
            ...
            assertThat(location.toString())
                    .as("%s must load from this module's compiled output, not the kafka-streams jar. "
                                    ...
                                    + "spike is measuring stock Kafka Streams - every downstream result would be a "
                                    + "false positive.",
                            generated.getName())
                    .doesNotContain(".jar")
                    .contains("/classes/");
    ...
    void unGeneratedSiblingsStillComeFromTheJar() {
        ...
        assertThat(location.toString())
                .as("%s is not in spike.patched.classes, so it must still come from the jar. If it does not, "
