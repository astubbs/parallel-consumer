 ...
 *
 * - **AK core** - kafkajs, this language's own Kafka client, one record at a time. Always spelled
 *   "AK core"; bare "core" reads as `parallel-consumer-core` (`CONCEPTS.md`).
 * - **typescript-grpc** - the application as a *foreign client*: this module's client library
...
import { AK_CORE, SIDECAR_ARM, type ArmResult } from "./report";
...
/**
 * Counts towards a target and resolves once, so an arm can wait for "the backlog is done" without
 * polling. It never rejects: the budget below is what turns a stall into a failure.
 */
class Countdown {
  private count = 0;
...
export async function akCore(
  ...
  await withinBudget(AK_CORE, countdown, target, countdown.reached);
  const elapsedMs = performance.now() - startedAt;
  await consumer.disconnect();
  return finished(AK_CORE, elapsedMs, countdown.processed);
}
...
export async function typescriptGrpc(
  ...
  const client = await ParallelConsumerClient.open({
    ...
    ordering: "unordered",
    kafkaProperties: broker.consumerProperties(groupId(SIDECAR_ARM)),
    instanceTag: `${SIDECAR_ARM}-demo`,
  ...
  try {
    ...
    // The race may be decided by the countdown, leaving this promise's rejection with nobody to
    // hear it. This handler is what stops that becoming an unhandled rejection.
    void ended.catch(() => undefined);

    // The user's function. It takes no parameter because it does not need one - the workload is
    // the same simulated wait every arm runs, so an arm that read the record would be doing work
    // the AK core arm does not. Returning nothing is a success; throwing would be a failure.
    client.poll(async () => {
      await work(options.delayMs);
      countdown.hit();
    });
    ...
    const outcome = await withinBudget(
      ...
      countdown,
      target,
      Promise.race([countdown.reached.then(() => "reached" as const), ended]),
    );
    const elapsedMs = performance.now() - startedAt;
    if (outcome === "ended" && countdown.processed < target) {
      throw new Error(
        `${SIDECAR_ARM} ended early at ${countdown.processed} of ${target} - the session closed ` +
          `before the backlog did`,
      );
    }
    return finished(SIDECAR_ARM, elapsedMs, countdown.processed);
  } finally {
...
async function withinBudget<T>(
  ...
  const budget = new Promise<never>((_resolve, reject) => {
    timer = setTimeout(() => {
      reject(new Error(`${arm} stalled at ${countdown.processed} of ${target}`));
    }, ARM_BUDGET_MS);
  });
  try {
...
function finished(arm: string, elapsedMs: number, processed: number): ArmResult {
  process.stdout.write(
    `=== ${arm} finished: ${processed} records in ${Math.round(elapsedMs)}ms ===\n`,
  );
  return { arm, elapsedMs, processed };
}
