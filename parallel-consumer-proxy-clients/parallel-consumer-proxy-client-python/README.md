...
`make test` spawns the real sidecar (`Main`), which lives in the proxy
...
- **Reaching the test phase needs `-Dpc.foreignClients`, not `-P foreign-clients`.** Both activate
  the module, but the `python-sidecar-harness` profile - which pulls the proxy module into the reactor
