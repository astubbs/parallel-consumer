...
cpp` and the same script with the test flag. The `foreign-clients` profile in the clients aggregator
([`../pom.xml`](../pom.xml)) binds them to `compile` and `test`, and decides whether the module is
in the reactor at all:

```bash
./mvnw compile -P foreign-clients -pl :parallel-consumer-proxy-client-cpp -am   # build in the image, extract
./mvnw test    -P foreign-clients -pl :parallel-consumer-proxy-client-cpp -am   # ...and run the artifacts on the host
```
...
- **`ctest` and the static analysis happen inside `compile`**, as layers of that image build, so a
  red test or a cppcheck finding fails the *compile* phase. Maven's `test` phase is the different
  assertion: it runs the extracted binaries **on this host**, where the dynamically linked control
  is expected to fail. A run where both work proves nothing, and the script says so and fails.
- **`clean` needs no configuration and gets none.** The container route writes exactly one host-side
  path, `target/container`, inside `${project.build.directory}` - so Maven's default clean already
  removes the binaries and `link-report.txt`. The pom records that measurement, so the empty
  configuration cannot be mistaken for an oversight.
- **`clean` is close to free, and that is the caching working.** Everything expensive is outside
  this directory: the `debian:trixie` base image, the BuildKit layer cache and the apt cache mounts.
  Measured here: `mvn clean` then a rebuild re-exported all four artifacts in about three seconds,
  every image layer `CACHED`.
- **Never prune the BuildKit cache to tidy up after this module.** It is shared with every worktree
  and agent on the machine - 17GB of it here, nearly all reported reclaimable, which is what makes
  `docker builder prune` tempting - and it is this language's `~/.m2`, which `mvn clean` does not
  empty either.
