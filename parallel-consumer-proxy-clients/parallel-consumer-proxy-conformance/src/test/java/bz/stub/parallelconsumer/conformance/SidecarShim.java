 ...
 * results protocol is to hold the harness in the asserting JVM. So the harness binds its gRPC server
 * here, on an ephemeral loopback port, and the runner has to reach that port.
 * <p>
 * <b>Nothing in this module spawns a runner yet, and this is here anyway.</b> {@link ConformanceHarness}
 * has no engine lane on this stack - the sidecar hosts no engine (astubbs/parallel-consumer#384) - so
 * the driver's spawn half is the engine rung's to bring. What that half needs from here is exactly these
 * four lines, and they are the half that can be proven <em>today</em>: {@code SidecarShimTest} runs a
 * written shim and watches it announce a port and then hold its stdin until it is closed, which is the
 * whole of the spawning contract as a client observes it. Landing it proven beats landing it with the
