...
public final class ConformanceRunnerPrebuild {
    ...
    public static void main(String[] args) {
        // The runners behind the bindings this run selected, and never a runner whose binding the matrix
        // will not drive: a prebuild that built more than the matrix runs would make a CI row install a
        // toolchain to compile something nobody then executes, and one that built less would leave the
        // matrix to build it at scenario time, which is the race this step exists to remove.
        var selected = ConformanceBindings.selected().stream().map(ConformanceBinding::name).toList();
        var runners = LanguageRunners.all().stream()
                .filter(runner -> selected.contains(runner.language()))
                .toList();

        if (runners.isEmpty()) {
            // Not a failure: -Dpc.conformance.language=core, or a selection of JVM clients only, is a
            // legitimate run with nothing to shell out for - and on this rung it is EVERY run, because no
            // foreign binding is registered until there is an engine for one to reach (ConformanceBindings
            // carries that call). A selector that matches NOTHING has already thrown out of selected().
