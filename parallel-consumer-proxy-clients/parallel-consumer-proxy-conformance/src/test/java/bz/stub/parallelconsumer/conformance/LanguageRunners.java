 ...
 * <p>
 * <b>The registry is complete on this rung; what is missing is the thing that RUNS an entry.</b> Every
 * language below has a client library and a conformance runner in this tree, and every entry names where
 * that runner is built and where its binary lands. What no entry can do here is execute a scenario, because
 * that needs an engine behind the sidecar and there is none - {@link LanguageRunner}'s own header carries
 * that call, and {@link TheEngineArrivingMustBringTheForeignCellsTest} is what stops it staying true by
 * accident. Which languages a run drives is {@link ConformanceBindings}' - the selector lives there, with
 * the core control arm.
 *
