...
/// <summary>
/// Locates the JVM-side sidecar so a .NET test can spawn it as an ordinary sidecar binary.
/// </summary>
/// <remarks>
/// <para>The sidecar is <c>parallel-consumer-proxy</c>'s production <c>Main</c>, which is a
/// classpath invocation rather than a binary - so "the sidecar binary" for a test is the JVM
/// launcher and the classpath is an argument, and everything awkward about that lives here rather
/// than in each test.</para>
/// <para>WHAT THIS BUILD'S SIDECAR DOES. It hosts no Parallel Consumer engine: it binds, announces
...
/// begin. The dispatch scenarios above are the shared conformance suite's identities, deferred
/// until an engine exists to run them against; nothing here stands in for one.</para>
/// </remarks>
internal sealed record ConformanceHarness(string Path, IReadOnlyList<string> Arguments)
{
    ...
    /// <summary>Written by the <c>dotnet-sidecar-harness</c> profile in this module's pom.</summary>
    ...
    /// <remarks>
    /// ONE ROUTE, AND IT THROWS RATHER THAN GUESSING. The <c>dotnet-sidecar-harness</c> profile in
