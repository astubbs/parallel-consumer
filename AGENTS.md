...
| **`docs/offset-encoding-density-benchmark.md`** | Generated measurement of how many bytes and metadata chars each offset encoding costs across a scenario corpus - incumbents and unshipped candidates - plus the back-pressure engagement points and the ship/case-against verdict per candidate (`bin/offset-encoding-density-report.sh`, `--check` fails when stale) | The design reasoning behind the encoders, which belongs with the code in the offsets package javadoc |
