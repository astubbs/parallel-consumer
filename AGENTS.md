...
| **`docs/inflight-vision.md`** | The same contract for the **agent harness** corpus: the generative laws behind `bin/inflight.mjs`, the `docs/inflight/` corpus and the hooks that deliver it, and the connections between those notes | Any fact a note owns, and anything the topic docs own - `docs/agent-harness.md` has the layer map, `docs/inflight-tool.md` the worked examples, `docs/inflight/AGENTS.md` how a note is written. It links rather than restates, and **the note wins** where the two disagree |
| **`docs/inflight/process-open-research-questions.md`** | What *this project* believes and has not checked - claims with no falsifier, constants that were reasoned rather than measured, contradictions between notes, and open decisions blocking others. Every entry names the check that would settle it | Prior art - that is the adjacent-systems registers below. An entry with no proposed check, which is an opinion |
| **`docs/inflight/*-adjacent-systems-register.md`** | The living prior-art registers - which of this project's claims are already occupied and by whom, on three evidence marks (verified / surveyed / claimed). One for the engine, one for the harness tooling | Findings themselves - the dated sweeps under `docs/plans/` own those verbatim and may not be rewritten. The register owns only what changes, and never restates what a sweep holds |
