...
| **`docs/test-hardening/`** | Dated audits of tests that do not run, do not assert, or were never written - per-test evidence, the disabling commit, and what coverage is actually lost | A live registry (it is a point-in-time record, not generated); triage goes in `refactoring.md` |
