...
- **Length: one sentence, then the link.** Symptom, what changed, PR link. The diagnosis, the experiment, the rejected alternatives and the postmortem belong in the commit message, the PR body and `docs/solutions/` - one click away for whoever wants them. An entry that runs to a paragraph is being written for its author, not its reader.
