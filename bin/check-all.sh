...
NEEDS_ARGS="check-ossindex-audit.sh check-execution-mode.sh"
...
reason_for() {
    case "$1" in
        ...
        check-execution-mode.sh)       echo "asserts what a FINISHED suite run did - needs the mode and its surefire reports" ;;
        *)                             echo "excluded" ;;
    esac
}
