...
# manager at all, so there is no manifest, no lockfile and nothing for any updater to bump. It is
...
# even while that module has no manifest at all: dropping it would make the absence silent again,
...
UNCOVERABLE = [
    ("parallel-consumer-proxy-clients/parallel-consumer-proxy-client-cpp",
     "C++ - no package manager, and at present no manifest of any kind: the module's build is a "
     "single compiler invocation against the standard library, so there is no version for any "
     "updater to bump. When the real client arrives it gains a CMakeLists.txt, which resolves "
     "through find_package/PkgConfig against system packages and is no more coverable - its "
