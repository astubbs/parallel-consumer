...
export const BUILD_OVERHEAD_SECONDS = 160 // serial build + job setup, re-paid by every shard
export const FORKS = 4 // forkCount inside each shard; a measured ceiling, see the script's header
...
/**
 * `--fail-over <seconds>` from an argv, or an error string. Pure, so the self-test can pin every
 * malformed shape without spawning: a missing value, a non-number, a negative, a following flag, or
 * the flag given twice all return an error rather than degrading to advisory - `Number(undefined)`
 * is NaN and every `drift > NaN` is false, so a scheduled job that asked to BLOCK would have exited 0
 * having gated on nothing.
 */
export function parseFailOver(argv) {
    const i = argv.indexOf('--fail-over')
    if (i === -1) return { failOver: null }
    if (argv.lastIndexOf('--fail-over') !== i) return { error: '--fail-over given more than once' }
    const raw = argv[i + 1]
    const failOver = Number(raw)
    if (raw === undefined || raw.startsWith('--') || !Number.isFinite(failOver) || failOver < 0) {
        return { error: `--fail-over needs one finite, non-negative threshold in seconds (got ${raw ?? 'nothing'})` }
    }
    return { failOver }
}
...
export function lpt(items, bins) {
...
export const shardWall = (classes) => Math.max(...lpt(classes, FORKS).sums) + BUILD_OVERHEAD_SECONDS
export const criticalPath = (shards) => Math.max(...shards.map(shardWall))

// --- main ------------------------------------------------------------------------------------
function main() {
    const parsed = parseFailOver(process.argv)
    if (parsed.error) {
        console.error(`check-integration-shard-balance: ${parsed.error}`)
        process.exit(2)
    }
    const failOver = parsed.failOver
