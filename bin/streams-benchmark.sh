...
SCENARIOS  (--scenario, default: payments)
  payments   Card-payment authorisation screening. The demonstration - one plausible
             workload, readable output. Three arms: two stock (the noise
             floor) and one PC.                                    ~15 minutes
  ...
  all        Everything above.                                       ~35 minutes
...
EXAMPLES
  ...
  bin/streams-benchmark.sh --pool 1     # the negative control - it is SUPPOSED to fail
EOF
}
...
REPORTS="parallel-consumer-streams/target/surefire-reports"

# A SKIP IS NEVER A PASS, and this script exists in a module where that is easy to get wrong. The
# benchmarks live in src/test/java, so SUREFIRE runs them - this module has no src/test-integration.
# The version of this script on the branch this suite came from drove FAILSAFE with -Dit.test and
# -DskipUTs, which on this layout selects nothing, runs nothing, and exits 0.
#
# Two consequences, both deliberate:
#
#  - -Dtest is used to pick the scenario, and it silently overrides the <includes> of the Kafka
#    upstream executions too, so those run nothing during a benchmark. That is fine here and fatal
#    for the seam-on evidence lane, which is why that lane has its own script and narrows the
#    module's own tests with -Dincluded.groups instead.
#  - failIfNoSpecifiedTests has to be off for the same reason (those executions would fail on an
#    empty selection), so maven can no longer tell us the scenario ran. The report check below is
#    what replaces it: every selected class must have left a report, or this exits 2.
for ((run = 1; run <= REPEAT; run++)); do
  ...
  for class in "${selected[@]}"; do
    if ! ls "$REPORTS"/TEST-*."$class".xml > /dev/null 2>&1; then
      missing="$missing $class"
    fi
  done
  if [[ -n "$missing" ]]; then
    echo "streams-benchmark: NOTHING RAN for:$missing" >&2
    echo "streams-benchmark: no surefire report under $REPORTS, so this run measured nothing - which" >&2
    echo "streams-benchmark: is an error and not an empty result. Check the class name and that the" >&2
    echo "streams-benchmark: performance tag is still on it." >&2
    exit 2
  fi
done
