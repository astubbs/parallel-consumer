...
bash-4-builtin	(^|[^[:alnum:]_-])([[:alnum:]_]*[_-])?(mapfile|readarray)([[:space:]]|$)	`mapfile` and `readarray` are bash 4 BUILTINS; macOS ships bash 3.2, where the line dies with "mapfile: command not found" and the script exits 127 - a gate that never ran, reported as neither verdict. ShellCheck models the language, not which builtins the running bash has, and passes it. Use `arr=(); while IFS= read -r x; do arr+=("$x"); done < <(...)`.
