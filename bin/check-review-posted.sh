...
#
# The token is this run's id, supplied as an argument. It is not a secret and is not trying to
# be: it exists so that a comment QUOTING this marker - this file's own documentation, a doc
# describing the protocol, a review of the PR that introduced it - cannot open a fake comment
# in the stream, because it will not carry the current run's token. Keep the marker's shape in
# step in three places: here, the `--jq` in .github/workflows/claude-code-review.yml, and
# bin/test-check-review-posted.sh.
#
