...
print(json.dumps({
    "hookSpecificOutput": {
        ...
        "additionalContext": (
            ...
            "\n"
            "3. THE CHECK IS NOT THE ONLY EVIDENCE. Codecov holds the OUTCOME AND WALL-CLOCK of "
            "every test, per commit, for longer than a CI log is kept - so when a check is red you "
            "can ask whether that test has ever failed before, and at which commit it changed, "
            "instead of inferring it from one run. It needs no token (this repo is public):\n"
            "     bin/inflight.mjs codecov test <name>   one test's outcome per commit - the bisect\n"
            "     bin/inflight.mjs codecov flaky         tests recorded with more than one outcome\n"
            "     bin/inflight.mjs codecov slow          what owns the wall-clock\n"
            "   Reach for it when deciding flake-vs-regression, when a quarantine entry needs "
            "sighting evidence, and in any bisect. It answers from recorded history rather than by "
            "re-running builds. Its duration figures are runner wall-clock and are NOT a throughput "
            "benchmark - never feed them to a performance comparison.\n"
            "\n"
