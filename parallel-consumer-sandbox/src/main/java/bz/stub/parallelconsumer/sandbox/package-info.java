 ...
 * </ol>
 *
 * <h2>The hydration: what a record contains</h2>
 * The driver is fed by a function from a record's index to its value, one per topic - and
 * {@link bz.stub.parallelconsumer.sandbox.RandomObjects} is the one it gets when the caller declares none. It
 * fills the route's declared type with realistic random data: Instancio for the object graph, Datafaker for leaf
 * values chosen by field <em>name</em>, so an {@code email} field holds an email address and a
 * {@code totalAmount} holds money, and Avro's own {@code RandomData} for a specific record, whose schema knows
 * more about its fields than its Java types do. A Protobuf message type is refused naming the type: this version
 * has no filler for one.
 *
 * <p>It is a <b>default, not a requirement</b>, which is the seam that matters: an application publishing its own
 * domain objects declares {@code Sandbox.builder().feeding(topic, index -> ...)} and the hydration never runs. A
 * route whose type nothing can name is refused at start, naming the topic, unless its caller said one of those
 * two things.
 *
 * <p>Addressed by <em>index</em> rather than called in sequence, so the record at a given index is the same
 * whatever order the topics were served in and whatever the pacing did - which, with a seed, is what makes a
 * sandbox failure reproducible without replaying the run up to it.
 *
