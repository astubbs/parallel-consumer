...
import bz.stub.parallelconsumer.streams.benchmark.BenchmarkReport;
import bz.stub.parallelconsumer.streams.benchmark.BenchmarkWorkload;
import bz.stub.parallelconsumer.streams.benchmark.GeneratedRecord;
import bz.stub.parallelconsumer.streams.benchmark.KeyDistribution;
import bz.stub.parallelconsumer.streams.benchmark.NoiseFloor;
 ...
 * <p>
 * This is the second benchmark in this module, and its job is not to beat the first one's bounded multiplier.
 * Its job is to measure the same property on a workload nobody chose to flatter it, so that "synthetic,
 * unfair, false advertising" has nowhere to land.
 * <p>
 * <b>No figure from an earlier run appears in this file.</b> The numbers are whatever the run prints, beside
 * the {@link bz.stub.parallelconsumer.streams.benchmark.NoiseFloor} the same run measured - a figure written
 * into prose describes the machine it was taken on and drifts silently from the one that quotes it.
 *
 ...
 *       independent of a fix landed days earlier. Measured, the split-wait branch fires on the large majority
 *       of records, and the control arm in {@link #howMuchOfTheAdvantageSurvivesWithoutWakeOnWork()} shows
 *       most of the advantage collapsing without it - both arms print their own numbers, so read them there
 *       rather than from here. A backlog keeps the <em>broker</em> supplied, but
 ...
 *
 * <p>
 * The first benchmark, {@code HeadOfLineBlockingBenchmarkTest}, is deliberately not carried onto this
 * stack: it isolates head-of-line blocking on a fixture whose own construction bounds its multiplier, and
 * the settled decision is to lead with backlog catch-up instead. It stays on the branch forest.
 *
...
class BacklogCatchUpBenchmarkTest extends StreamsBenchmarkHarness {
    ...
    void aBacklogDrainsFasterWithTheSeamOn() {
        ...
        // The noise floor: identical to the arm above in every term, including the seam. Whatever these two
        // differ by is this machine on this run, and it is the only honest scale for the ratio below - a run
        // whose two stock arms differ by more than its headline has measured nothing.
        ArmResult stockAgain = runArm("catchup-stock-again", workload, records, false);
        ...
        NoiseFloor floor = NoiseFloor.between(stock.getSustainedRatePerSecond(),
                stockAgain.getSustainedRatePerSecond());
        ...
        log.info(new BenchmarkReport("Experiment C1 - cold-start backlog catch-up")
                ...
                .measurement("NOISE FLOOR (two stock arms)", rate(stock), rate(stockAgain),
                        String.format("%.2fx", floor.getRatio()))
        ...
        assertThat(floor.beatenBy(rateRatio))
                .as("C1: and it must be outside what this machine does to a ratio on its own. Measured %.2fx "
                        + "against a floor of %.2fx, so this run had to beat %.2fx. Stock %.1f/s, stock again "
                        + "%.1f/s. The fixed threshold above is a claim about the mechanism; this one is a "
                        + "claim about the run, and only the second can tell a busy machine from a result.",
                        rateRatio, floor.getRatio(), floor.getThreshold(), stock.getSustainedRatePerSecond(),
                        stockAgain.getSustainedRatePerSecond())
                .isTrue();
