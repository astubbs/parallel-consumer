...
class PaymentAuthorisationBenchmarkTest extends StreamsBenchmarkHarness {
     ...
     * <p>
     * <b>Three arms, not two.</b> The extra one is a second STOCK arm, run against the same records, and it
     * exists to measure what this machine does to a ratio when nothing has changed at all - see
     * {@link NoiseFloor}. Without it on screen the headline is a number with no scale, and a run in which the
     * machine was busy looks exactly like a finding.
     */
    @Test
    void catchingUpOnAQueueOfAuthorisations() {
        ...
        // Identical to the arm above in every term, including the seam. Anything it differs by is this
        // machine, in this state, on this run - which is the only honest scale for the ratio below.
        ArmResult stockAgain = runArm("payments-backlog-stock-again", workload, records, false);
        ...
        log.info(new BenchmarkReport("Card-payment authorisation screening - catching up after an outage")
                ...
                .measurement("NOISE FLOOR (two stock arms)",
                        String.format("%.1f/s", stock.getSustainedRatePerSecond()),
                        String.format("%.1f/s", stockAgain.getSustainedRatePerSecond()),
                ...
                .finding("NOISE FLOOR: " + floor.describe())
                .render());
        ...
        assertThat(floor.beatenBy(rateRatio))
                .as("a queue of independent authorisations should clear faster when more than one card can be "
                        + "screened at a time - and by more than this machine moves a ratio on its own. "
                        + "Measured %.2fx against a floor of %.2fx, so the threshold this run had to beat was "
                        + "%.2fx. Stock %.1f/s, stock again %.1f/s, PC %.1f/s. A bare 'greater than one' here "
                        + "is satisfied by a run in which the seam did nothing at all.",
                        rateRatio, floor.getRatio(), floor.getThreshold(), stock.getSustainedRatePerSecond(),
                        stockAgain.getSustainedRatePerSecond(), pc.getSustainedRatePerSecond())
                .isTrue();
    }
