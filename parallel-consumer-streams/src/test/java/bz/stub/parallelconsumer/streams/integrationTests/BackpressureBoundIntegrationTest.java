...
/**
 * The memory bound, proven rather than asserted (astubbs#255).
 * <p>
 ...
 * the reason that occupancy is derived from collections safe to read from a thread that does not own them.
 *
...
class BackpressureBoundIntegrationTest extends BrokerStreamsIntegrationTest {
     ...
     * At {@link #POOL_SIZE} threads this drains roughly 400 records per second against a broker that
     * delivers {@link #TOTAL} in well under a second.
     */
     ...
     * <p>
     * <b>No pool-size term.</b> It is tempting to add one, reasoning that a record handed to a worker frees
     * room for a replacement - but the occupancy counted here <em>excludes</em> records that have been
     * handed out, so a worker starting can only lower this number, never raise it, and the replacement
     * arrives inside the next poll batch that is already counted. Those extra records would be pure silent
     * regression budget in the one assertion this whole unit rests on.
     */
    ...
    void withBackpressureOnRecordsStopAccumulating() {
        ...
        assertThat(PcDispatchSwitch.isBackpressureEnabled())
                .as("the varied term, read back rather than assumed - an arm that silently ran the other "
                        + "configuration would be comparing a run against itself")
                .isTrue();
        ...
        assertThat(peak)
                .as("THE assertion of this unit: with a processor slower than the feed, records stop "
                        ...
                        + "batch %s", peak, BOUND, BUFFERED_RECORDS_PER_PARTITION, MAX_POLL_RECORDS)
