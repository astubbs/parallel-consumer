...
- **Records that never succeed pin the committed offset, and what sits above it has to fit in the commit metadata.**
  - The committed offset is the lowest incomplete one. Everything completed above it is encoded into Kafka's 4 KB commit metadata, as a run-length or a bitset, whichever is smaller.
  - Regular patterns compress to almost nothing: a contiguous run of failures, or every second offset failing, costs next to no space however long it runs.
  - It takes an irregular scatter of permanent failures among successes, over a wide range, to outgrow the limit. When it does, back-pressure stops admitting that partition's records, and under retry-forever nothing clears it: the base can never move past a record that never completes.
  - Separately, records held for retry count toward the in-flight target (`maxConcurrency` x `batchSize`, times the load factor). A small target with a steady supply of failing records pauses the broker poller the way any full buffer does; a larger `messageBufferSize` raises that ceiling.
  - This release makes the gate's latched state visible: a WARN when it has stayed loaded with nothing retiring ([#497](https://github.com/astubbs/parallel-consumer/pull/497)). Before it, the only signal was the paused-partition gauge.
  - Workarounds today: handle terminal failures inside your function so the record retires, as the README's Skipping Records section describes, or a `retryDelayProvider` with a growing back-off.
  - The fix for records that cannot succeed is the dead-letter queue, under What comes next ([#487](https://github.com/astubbs/parallel-consumer/pull/487); [#149](https://github.com/astubbs/parallel-consumer/issues/149), upstream [#310](https://github.com/confluentinc/parallel-consumer/issues/310)).
