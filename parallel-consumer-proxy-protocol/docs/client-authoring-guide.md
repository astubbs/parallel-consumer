...
beside it. A worked example of that fallback arrives with the Go client; until then, read it on the branch
that has it - `git show origin/feats/proxy-requirements:parallel-consumer-proxy-clients/parallel-consumer-proxy-client-go/scripts/generate-proto.sh`.
<!-- file-refs: N/A - the Go client's generator script is named as work that has not landed on master; the line above gives the command that reads it where it exists -->

**No client generates from this schema on `master` yet.** The language modules under
`parallel-consumer-proxy-clients` are build scaffolding (astubbs/parallel-consumer#380) - toolchain,
compile, run, one fixture line - so this section describes what a client author does, not what the
...
**2026-08-15, the in-flight ceiling (§3.2)**, from the cross-client divergence review (finding 2), which
read all five non-JVM clients side by side. The review itself travels with the clients it read, on
astubbs/parallel-consumer#293 - `git show origin/feats/proxy-requirements:docs/inflight/branch-client-divergence-review.md`.
**Three of five authors implemented this rule wrongly from the same text, so the text was the defect** -
the rule said "queue", the worked example said "unresolved", and the rule is what gets built:
<!-- file-refs: N/A - the divergence review is a branch-resident note on astubbs#293, reached with the git command above rather than by a path on master -->
