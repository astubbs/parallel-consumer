...
* Mutation testing (PIT) runs once per PR instead of three times (`maven.yml`), mutates only changed classes in packages where a surviving mutant is decidable (`offsets.*` to start, naming those it declined), and lists each survivor as `file:line` with what was changed - so the output is a work item rather than a percentage. In the concurrency core, mutants hang instead of dying and a survivor cannot be told from an unobserved race. The `internal.*` sweep, which had never completed, is now manual-only (`mutation-full-sweep.yml`) and likewise retargeted; a flaky test no longer aborts the whole run (`skipFailingTests`). Reasoning, measurements and open questions: link:docs/plans/2026-08-03-002-mutation-testing-plan.md[the mutation testing plan] (https://github.com/astubbs/parallel-consumer/pull/111[#111]).
