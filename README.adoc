...
* Mutation testing (PIT) now also runs advisory on the GitHub-hosted PR build (`bin/ci-mutation-test.sh`), scoped to the core classes changed vs the PR base so it fits the 2-core runner - the full sweep stays a self-hosted/manual job.
