...
Run a main method::
`mvn exec:java -Dexec.mainClass="io.MainClass`

==== Running Tests

Running tests in submodules may required module deps to be installed locally.

Run a single unit test::
`mvn -Dtest=TestCircle test`

Run a specific integration test method the `core` submodule project, skipping unit tests::
`mvn -Dit.test=TransactionAndCommitModeTest#testLowMaxPoll -DskipUTs=true integration-test  -DfailIfNoTests=true --projects parallel-consumer-core`

Run a integration test in the vert.x project::
`mvn -DskipUTs=true -DfailIfNoTests=false failsafe:integration-test --projects parallel-consumer-vertx -Dit.test=VertxConcurrencyIT`
