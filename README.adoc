...
NOTE:: *Entries are terse.* An entry highlights a change and points at the detail - it is not the
story of the change. Reasoning, evidence and corrections belong in the linked plan doc,
`docs/solutions/` write-up or PR. Entries here run ~90 words; several times that means you are
writing the story. When correcting an entry, replace text rather than adding to it.
...
* Optional, non-gating self-hosted fast-feedback lane on a high-CPU runner (`pr-highcpu-fast-feedback.yml`, `runs-on: [self-hosted, highcpu]`): performance and the Chaos Pain Suite run as a concurrent matrix on real cores. It carries only work the 2-core GitHub-hosted gate cannot do well - unit and integration were removed once measured as no faster there than the hosted gate that already runs them. Same-repo-guarded (fork PRs never run on the self-hosted box) and `continue-on-error` + not required, so an offline runner never blocks a merge (the required gate stays GitHub-hosted; exception: the `Chaos Pain Suite` entry is deliberately NOT `continue-on-error` - a chaos RED is a real finding and shows red - though still not a required gate). Triggers on `pull_request` + `workflow_dispatch`. See link:docs/SELF_HOSTED_RUNNER.md[docs/SELF_HOSTED_RUNNER.md].
* Mutation testing (PIT) runs once per PR instead of three times (`maven.yml`), and mutates only changed classes in packages where a surviving mutant is decidable - `offsets.*` to start - naming the classes it declined. In the concurrency core mutants hang rather than die and a survivor cannot be told from an unobserved race, so those runs spend a runner slot to produce nothing actionable. The full `internal.*` sweep, which had never completed, is manual-only (`mutation-full-sweep.yml`) and likewise retargeted. A flaky test no longer aborts the whole run (`skipFailingTests`), and every run reports whether it scored, skipped, or died. Reasoning, measurements and open questions: link:docs/plans/2026-08-03-002-mutation-testing-plan.md[the mutation testing plan] (https://github.com/astubbs/parallel-consumer/pull/111[#111]).
