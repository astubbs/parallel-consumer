...
* Mutation testing (PIT) runs once per PR instead of three times, scoped to the changed classes (`maven.yml`). The full `internal.*` sweep - which had never completed - is manual-only (`mutation-full-sweep.yml`) and retargeted to `offsets.*`, where mutants are decidable. A flaky test no longer aborts the whole run (`skipFailingTests`), and every run writes a job summary saying whether it scored, skipped, or died before scoring. Not yet measured under the new target: no score should be quoted until a sweep completes. Reasoning and open questions: link:docs/plans/2026-08-03-002-mutation-testing-plan.md[the mutation testing plan] (https://github.com/astubbs/parallel-consumer/pull/111[#111]).
