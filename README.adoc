...
See {base_confluent_url}/issues/12[issue #12], and the `ParallelConsumer` JavaDoc:
...
* Mutation testing works again. `ProducerManagerTest.producedRecordsCantBeInTransactionWithoutItsOffsetDirect` failed roughly 1 run in 6, and PIT refuses to score any mutants while a test is unstable *without* mutation - so that one race turned the whole `Mutation (PIT, scoped)` lane red and nothing was measured. The lane's greenness had become a proxy for suite stability rather than for mutation coverage. The test was releasing its produce lock inside its own user function, before the work reached the controller's inbound queue, which let the controller take the commit lock and collect offsets one behind - the test's own TODO had flagged this. It now acquires against the real context and lets the production path release it post-commit-payload, as `WorkContainer#onPostAddToMailBox` requires, and asserts that ownership so a future edit fails loudly instead of flakily. Diagnosed by controlled experiment: an identical delay injected after the unlock failed 8/8, before it passed 8/8. No main-code change - the exactly-once invariant was never actually violated in production (https://github.com/astubbs/parallel-consumer/pull/110[#110]).
...
* Optional, non-gating self-hosted fast-feedback lane on a high-CPU runner (`pr-highcpu-fast-feedback.yml`, `runs-on: [self-hosted, highcpu]`): performance and the Chaos Pain Suite run as a concurrent matrix on real cores. It carries only work the 2-core GitHub-hosted gate cannot do well - unit and integration ran here too until they were measured as *not* faster than the hosted gate that already runs them, making them a second copy of an existing verdict and one more tick to triage per PR (the measured fork tuning is kept in the workflow comments for whoever re-adds them). Same-repo-guarded (fork PRs never run on the self-hosted box) and `continue-on-error` + not required, so an offline runner never blocks a merge (the required gate stays GitHub-hosted; exception: the `Chaos Pain Suite` entry is deliberately NOT `continue-on-error` - a chaos RED is a real finding and shows red - though still not a required gate). Triggers on `pull_request` + `workflow_dispatch`. See link:docs/SELF_HOSTED_RUNNER.md[docs/SELF_HOSTED_RUNNER.md].
* Mutation testing (PIT) runs advisory on the GitHub-hosted PR build (`bin/ci-mutation-test.sh`), scoped to the core classes changed vs the PR base so it fits the 2-core runner - and that is now the *only* per-PR mutation lane. It had been running three times per PR (twice on highcpu, plus the hosted lane), two of them the same scoped computation - with a fourth copy configured in the `local` lane, dormant only because that workflow's trigger is disabled while its runner is offline. That copy also checked out shallow, which silently promotes a scoped run into the full sweep, so re-enabling it would have shipped the hazard too. **The full sweep is now manual-only and points somewhere decidable** (`mutation-full-sweep.yml`, with `target-classes`/`target-tests` inputs). The old `internal.*` sweep had never once completed - 42+ min on CI, 83+ min locally with minions dying on `MEMORY_ERROR` - so it scored *zero* mutants while holding a 16-core runner for its full timeout on every push. `internal.*` is the concurrency core, which makes it close to the worst possible target: mutants to locks, loop conditions and timeouts hang by construction rather than dying fast, and its timing-based tests make a survivor unfalsifiable, since "nothing asserts this" is indistinguishable from "the race didn't happen this run". The sweep now targets `offsets.*` instead, where the tests are deterministic - so a survivor is a real gap - and the stakes are highest, since a silent bug in offset encoding means lost or duplicated records. It stays manual and un-scheduled until a run actually completes: nothing has been measured yet, and scheduling a job on a hoped-for runtime is how the previous one went unnoticed for so long. **A single flaky test no longer switches mutation testing off repo-wide:** PIT refuses to score anything while any test is unstable *without* mutation, so one unrelated flake meant zero mutants everywhere (twice: #101, #110). `skipFailingTests` now makes it drop that test's coverage instead of aborting - it must be set in the pom, since the mojo parameter declares no `property` and `-DskipFailingTests` is silently ignored. Finally, every run writes a GitHub job summary saying which outcome it was - scored (with the totals), *skipped* because nothing changed, or died before scoring - because a green mutation tick means "nothing to mutate" at least as often as it means "all mutants killed". Analysis, and the remaining work (running the retargeted sweep to find out whether it completes), in link:docs/plans/2026-08-03-002-mutation-testing-plan.md[the mutation testing plan] (https://github.com/astubbs/parallel-consumer/pull/111[#111]).
...
* The automated PR reviewer can now run the test suites itself (`claude-code-review.yml`): JDK 17 + Maven cache are set up, and `bin/ci-unit-test.sh` / `bin/ci-integration-test.sh` are granted via `--allowedTools`. The review fires on `pull_request` *before* the CI test jobs finish, so it could never read their results - it could only restate the author's claims. PR https://github.com/astubbs/parallel-consumer/pull/100[#100] showed why that matters: a regression test was named so surefire never collected it, and the suite was green precisely because the new test never ran. The reviewer is now also told to check which tests actually ran, not just the totals. Chaos stays out of reach (too slow for a review pass) and the job is capped at 30 minutes. **Blocking findings are now raised as inline review comments** rather than buried in the summary: the `master` ruleset requires review threads to be resolved before merge, so an inline thread is the only thing that actually gates a merge - a blocking finding written only in a PR-level comment can be merged straight past. Non-blocking notes stay in the summary, so routine observations don't become merge gates. The reviewer is also granted the repo's fast read-only verification scripts (`bin/check-*.sh`) so it can independently re-run what a PR claims - e.g. confirm a copyright-header or quarantine-registry claim - instead of inferring it from the diff (https://github.com/astubbs/parallel-consumer/pull/102[#102]). Those grants were inert until now: `--allowedTools` entries are *prefix* matches, so `Bash(bin/ci-unit-test.sh:*)` never matched `./bin/ci-unit-test.sh` and the reviewer was denied every natural invocation - one review logged 10 permission denials, gave up before running anything, and still reported success. Both spellings are now granted for every script, and `./mvnw` is granted too so the reviewer can run a single test class: that makes the highest-value check affordable - revert a PR's fix, re-run only its new test, and confirm it actually goes RED, rather than taking a claimed RED/GREEN on trust (https://github.com/astubbs/parallel-consumer/pull/108[#108]).
* Test classes that CI silently never ran are now impossible to add: a new ArchUnit convention rule (`TestConventionRules`, applied per-module) fails the build when a class holding `@Test`/`@ParameterizedTest` methods is named something surefire's default includes (`Test*`, `*Test`, `*Tests`, `*TestCase`) do not match. Three such tests had been dormant on master - `MockConsumerTestWith{CommitTimeout,SaslAuthentication}Exception` and `MockConsumerTestWithEarlyClose`, two of which cover the commit-exception ladder - and have been renamed and re-enabled (all pass). Also fixes the `largeOffsetMap` flake at its source: `OffsetMapCodecManager`'s mutable static codec state was guarded by a one-sided lock, so the reading test could observe another test's forced codec; the reader now declares the same `@ResourceLock`. The automated PR reviewer now also reviews the PR **title and description** as part of the diff - checking they cover every substantive change, that specifics (counts, names, claimed verification) match reality, and that terminology is not misleading - since on a squash merge the title becomes the permanent commit title. Also fixes a long-standing shutdown-commit flake in `ParallelEoSStreamProcessorTest` that waited on control-loop *cycles* for a commit driven by wall-clock - it now waits for the commit itself. That one test aborted the whole PIT mutation job whenever it flaked, since PIT fails when a test is unstable without mutation (https://github.com/astubbs/parallel-consumer/pull/101[#101]).
* New `docs/TODO_INDEX.md`: a generated index of every `TODO`/`FIXME`/`XXX` marker in the tracked tree (91 at time of writing), grouped by module with file and line. Regenerate with `bin/todo-index.sh`; `bin/todo-index.sh --check` fails when the committed index is stale, so it can be wired into CI. The markers are *triaged* in the existing `docs/refactoring.md` backlog rather than a new list - two findings from this pass landed there (the timing-based waits behind a known flaky test, and one Apache Kafka `MockConsumer` workaround duplicated across four example tests). `AGENTS.md` gains a *Where things live* table - one row per document, what it owns and what it is explicitly not for - because the docs split is enforced by convention rather than tooling, so the usual failure is never opening the right file rather than misreading it (`docs/SELF_HOSTED_RUNNER.md` was linked from nowhere at all) (https://github.com/astubbs/parallel-consumer/pull/103[#103]).
* The `PR Checklist` gate now also enforces AGENTS.md's changelog reference convention: when a PR adds a new `CHANGELOG.adoc` entry, that entry must cite the PR itself. This was being missed structurally rather than carelessly - the entry is written before the PR exists, so there is no number to cite at authoring time and nothing prompts you to return; four PRs in a row shipped without one. Only *added* entry lines are checked, so reflows and edits to older entries are unaffected, and a PR that deliberately adds no entry of its own can opt out with `changelog-ref: N/A - <reason>` in its body (https://github.com/astubbs/parallel-consumer/pull/104[#104]).
* A PR's still-running CI is now cancelled when the PR closes (`cancel-closed-pr-runs.yml`). Merging or closing a PR does not stop its in-flight runs, which is mostly harmless on hosted runners but wasteful on the small self-hosted pool: observed with five branches' runs queued against five online runners, three of those branches already merged. The existing per-suite `concurrency` only supersedes the same suite on the same branch, so nothing about a merge looked like a new push. Applies to closed-unmerged PRs too, refuses to touch the default branch, and skips runs from a different head repository so an identically-named fork branch is unaffected (https://github.com/astubbs/parallel-consumer/pull/107[#107]).
* The automated PR reviewer no longer posts duplicate reviews. `claude-code-review.yml` had no `concurrency` group, so a push shortly after opening a PR left both runs alive and each posted its own review (seen on #107, 23 seconds apart). That sequence is what this repo's own rules produce: the changelog gate requires the entry to cite the PR number, which only exists once the PR is open - so opening is routinely followed within seconds by a push. Now one review per PR at a time, newest wins. The rule that caused that push is also gone: instead of citing its own PR, a changelog entry now cites **the issue it addresses** - which is known before the work starts, so no second push is needed. Required only for user-visible sections (`Breaking`/`Improvements`/`Fixes`/`Examples`); `Build & CI` is exempt, since tooling work here is self-directed and we don't create issues just to satisfy a changelog rule (https://github.com/astubbs/parallel-consumer/pull/109[#109]).
...
* Fixed a latent flaky integration test inherited from upstream: `BrokerPollerBackpressureTest.brokerPollPausedWithEmptyShardsButHighInFlight` awaited a condition that was unsatisfiable once records arrived (static `messageBufferSize` load-factor cap leaves 50 of 200 records in the shards forever) and only ever passed by evaluating vacuously before partition assignment - so it failed deterministically on loaded CI boxes. Rewritten as `brokerPollPausedWhenBlockedInFlightFillsBuffer` awaiting the real steady state, which also pins the take-cap and verifies upstream #836's in-flight-counts-toward-backpressure behaviour non-vacuously for the first time. Root-cause writeup: link:docs/solutions/test-flakiness/vacuous-await-condition-brokerpoller-backpressure-2026-07-31.md[vacuous-await solutions doc].
...
* fix: draining close no longer busy-spins nor zombie-holds partitions. A duplicated shutdown flag stopped `consumer.poll()` from being invoked during the `DRAIN` phase of `close()`, so the closing consumer busy-spun its poll loop (~10k iterations/s, a full core) and - because rebalance participation happens inside `poll()` - remained a live group member holding its whole partition assignment without being able to respond to rebalances, starving same-group consumers for up to `max.poll.interval.ms`. The flag is removed; the poll system's lifecycle is now the single source of truth, and a draining consumer keeps polling (paused) at long-poll cadence (https://github.com/astubbs/parallel-consumer/pull/80[#80]; relates to upstream https://github.com/confluentinc/parallel-consumer/issues/857[#857]).
...
* fix: a commit landing during a group rebalance no longer kills the consumer. Kafka reports this as `RebalanceInProgressException` and resolves it on the next `poll()`, but PC left it unhandled: it escaped the broker-poll thread's control loop and killed that thread permanently - and since it is the only producer of commit responses, every subsequent commit blocked until `offsetCommitTimeout` and then brought the whole PC instance down with a misleading "Timeout waiting for commit response". The commit is now deferred (offsets stay dirty and are re-committed on the next cycle, rather than being falsely recorded as committed) and waiting committers are released immediately. Most likely to bite under the cooperative-sticky assignor, where members keep committing *during* rebalances by design (https://github.com/astubbs/parallel-consumer/pull/100[#100]; relates to upstream https://github.com/confluentinc/parallel-consumer/issues/857[#857]).
* fix: a commit rejected because this consumer left the group is no longer recorded as successful. Kafka reports this as `CommitFailedException`; PC caught it, logged that the poller would retry later, and then returned normally - which marked the offsets as committed, so nothing ever did retry. Those offsets are now left dirty and re-committed on the next cycle, so a partition's new owner does not resume behind records PC had already recorded as done (relates to upstream https://github.com/confluentinc/parallel-consumer/issues/857[#857]).
