...
* fix: a closing consumer no longer wedges its consumer group - it stopped polling while draining, so it burned a core and blocked rebalances for the other members (https://github.com/astubbs/parallel-consumer/pull/80[#80]; relates to upstream https://github.com/confluentinc/parallel-consumer/issues/857[#857]).
...
* fix: a commit landing during a group rebalance no longer kills the consumer - it died later with a misleading commit timeout. Most likely to bite under the cooperative-sticky assignor (https://github.com/astubbs/parallel-consumer/pull/100[#100]; relates to upstream https://github.com/confluentinc/parallel-consumer/issues/857[#857]).
* fix: a commit rejected after this consumer left the group is no longer recorded as done - the partition's new owner could resume behind records PC had already marked complete (relates to upstream https://github.com/confluentinc/parallel-consumer/issues/857[#857]).
