...
* Offset encoding no longer walks the entire offset range when it doesn't have to. `OffsetSimultaneousEncoder.invoke()` previously visited *every* offset between the committed base and the highest succeeded offset; for a partition whose range spans a very large gap (up to ~2.1 billion) that is a multi-second scan on the commit path. It now visits only the offsets where the completion state actually changes, whenever every active encoder is distance-based - which is exactly the large-range case, since `BitSet` encoders cannot be constructed at that size. The full scan is retained whenever a `BitSet`/`ByteBuffer` encoder is active, as those genuinely need every offset. Encoded output is unchanged: `RunLengthEncoder` accumulates run length by *delta* rather than by counting calls, so visiting only each run's first and last offset is byte-identical to visiting all of them - enforced by a differential test that compares both strategies across 130 generated scenarios. Resolves a long-standing `TODO` in the encoder ("VERY large offset ranges is slow").
