...
* PR checklist is now enforced: a single `.github/PULL_REQUEST_TEMPLATE.md` (deduplicated) with a `PR Checklist` CI gate (`pr-checklist.yml`) that fails when a checklist box is left unchecked without an `N/A` reason - so changelog/docs/tests/title-body-sync don't get skipped (bots without the template are exempt).
