...
NOTE:: *What earns an entry.* This file answers one question for its reader: should I upgrade, and will anything change for me? An entry must therefore be observable by a user or an operator - API, behaviour, performance, logs and metrics, or the published artifact. Write one sentence and link the PR: the diagnosis, the experiment and the rejected alternatives belong in the commit message, the PR body and `docs/solutions/`, one click away for anyone who wants them. Internal tooling and CI work does not earn an entry each - it is summarised in a single `Build & CI` entry per release, covering only what tells a reader something about how carefully the project is tested.
...
* fix: a closing consumer no longer wedges its consumer group. During the drain phase of `close()` it stopped polling, so it busy-spun a core and - because rebalance participation happens inside `poll()` - held its partitions without responding to rebalances, starving the other members for up to `max.poll.interval.ms` (https://github.com/astubbs/parallel-consumer/pull/80[#80]; relates to upstream https://github.com/confluentinc/parallel-consumer/issues/857[#857]).
...
* fix: a commit landing during a group rebalance no longer kills the consumer. Kafka reports this as `RebalanceInProgressException` and resolves it on the next `poll()`, but PC left it unhandled: it killed the broker-poll thread, and since that thread is the only source of commit responses, every later commit blocked until `offsetCommitTimeout` and took the instance down with a misleading timeout message. The commit is now deferred and retried. Most likely to bite under the cooperative-sticky assignor, whose members commit *during* rebalances by design (https://github.com/astubbs/parallel-consumer/pull/100[#100]; relates to upstream https://github.com/confluentinc/parallel-consumer/issues/857[#857]).
* fix: a commit rejected because this consumer left the group is no longer recorded as successful. PC caught the `CommitFailedException`, logged that the poller would retry, then returned normally - which marked the offsets committed, so nothing retried. They now stay dirty and are re-committed, so a partition's new owner cannot resume behind records PC had already recorded as done (relates to upstream https://github.com/confluentinc/parallel-consumer/issues/857[#857]).
...
* Test and CI infrastructure was substantially strengthened while preparing this release. The parts that say something about how carefully the library is tested:
** *Quarantine lane* - a known-failing test is annotated, registered, and tracked to an owning fix PR. CI enforces the registry in both directions, and a release is blocked while any test is quarantined.
** *Chaos Pain Suite* - seeded, replayable scenarios that drive consumer fleets through rebalance storms under no-loss, bounded-duplicate and progress SLOs, built to catch the upstream https://github.com/confluentinc/parallel-consumer/issues/857[#857]-family silent stalls. Two of the fixes above were found or confirmed this way.
** *No silently-uncollected tests* - an ArchUnit rule fails the build when a class holding tests is named something surefire never runs; three dormant test classes were found this way and re-enabled. A second rule keeps Docker-dependent tests out of the fast unit suite.
** *Mutation testing* per PR on the classes a PR changes, plus forked-per-broker integration tests that removed a long-standing flake class.
** *Self-hosted high-CPU lane* - the full suites, including chaos and performance, run per PR on real hardware. Advisory only; the required merge gate stays on GitHub-hosted runners. See link:docs/SELF_HOSTED_RUNNER.md[docs/SELF_HOSTED_RUNNER.md].
+
