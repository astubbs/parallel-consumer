...
* The optional macOS self-hosted PR checks (`pr-mac-fast-feedback.yml`) are disabled while that runner is offline — they showed as eternally-pending on every PR; manual dispatch still works, and re-enabling is a one-line trigger restore documented in the workflow.
