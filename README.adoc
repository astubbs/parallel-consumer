...
* The automated PR reviewer no longer posts duplicate reviews. `claude-code-review.yml` had no `concurrency` group, so a push shortly after opening a PR left both runs alive and each posted its own review (seen on #107, 23 seconds apart). That sequence is what this repo's own rules produce: the changelog gate requires the entry to cite the PR number, which only exists once the PR is open - so opening is routinely followed within seconds by a push. Now one review per PR at a time, newest wins (https://github.com/astubbs/parallel-consumer/pull/109[#109]).
