...
* Unit-test gate packs its fork tail: the `ci` profile now runs test classes slowest-first (surefire `runOrder=balanced`, with the statistics files pinned via `runOrderStatisticsFileChecksum` to `<module>/.surefire-pc-unit-times` and checked in, so the ordering works on a cold CI checkout). Forks pull from one shared queue, so putting the long classes first stops a fork idling at the end of the run. Measured on a 2-CPU replica of the GitHub-hosted gate. Write-up, including why Maven `-T` module parallelism *loses* on a 2-core runner and why this suite's wall time is hard to benchmark: link:docs/solutions/test-flakiness/unit-gate-fork-tail-packing-and-measurement-variance-2026-08-03.md[fork-tail packing].
