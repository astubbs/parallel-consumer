...
* New `docs/TODO_INDEX.md`: a generated index of every `TODO`/`FIXME`/`XXX` marker in the tracked tree (101 at time of writing), grouped by module with file and line. Regenerate with `bin/todo-index.sh`; `bin/todo-index.sh --check` fails when the committed index is stale, so it can be wired into CI. A companion `docs/TODO_TRIAGE.md` groups them into priority order (P1-P4) by theme rather than line number, so it survives index rebuilds - the machine owns what/where, a human owns what-matters. `docs/inflight.md` links out to both rather than restating marker text, keeping the ledger for work someone should schedule and leaving per-line context in the code where the index surfaces it (https://github.com/astubbs/parallel-consumer/pull/103[#103]).
