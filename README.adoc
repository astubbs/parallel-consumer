...
* New `docs/TODO_INDEX.md`: a generated index of every `TODO`/`FIXME`/`XXX` marker in the tracked tree (91 at time of writing), grouped by module with file and line. Regenerate with `bin/todo-index.sh`; `bin/todo-index.sh --check` fails when the committed index is stale, so it can be wired into CI. The markers are *triaged* in the existing `docs/refactoring.md` backlog rather than a new list - two findings from this pass landed there (the timing-based waits behind a known flaky test, and one Apache Kafka `MockConsumer` workaround duplicated across four example tests). `AGENTS.md` now points at `refactoring.md` as the owner of marker triage, since it is easy to miss by grepping only `docs/inflight.md` (https://github.com/astubbs/parallel-consumer/pull/103[#103]).
