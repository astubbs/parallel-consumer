 ...
 * <p>
 * <b>The unbounded growth was not the only defect in that field, and the second one was
 * reproduced rather than reasoned about.</b> A Lincheck harness aimed at the commit path
 * produced this, from two concurrent {@code createOffsetAndMetadata} calls with no injected seam:
 * <pre>
 ...
 * </pre>
 * That is the classic torn-grow signature - both threads read the same {@code size}, one grows the
 * backing array, the other writes past the array it had already loaded - and the throw is the
 * <i>lucky</i> outcome. The ordinary one is a silently dropped or duplicated {@code Meter.Id},
 * which then makes deregistration miss a meter on partition revocation. So the collection change
 * and the {@code metersLock} that guards every mutation of it are one fix, not two: the leak and
 * the race share a field.
 * <p>
 * These tests are single-threaded and do not attempt to reproduce that interleaving - the
 * evidence is recorded here because it is why the lock exists, and a later reader deciding the
 * lock is redundant should meet it first.
 *
