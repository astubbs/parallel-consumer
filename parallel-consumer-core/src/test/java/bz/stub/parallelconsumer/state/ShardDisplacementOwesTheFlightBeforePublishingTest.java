 ...
 * must owe the displaced container's flight BEFORE it publishes the replacement, because a scan can land between
 * the two.
 * <p>
 * <b>The interleaving.</b> {@code workMap.put(offset, fresh)} makes the fresh container resident, and resident is
 * selectable. The first version of the fix recorded the displaced container in {@code flightsOwed} inside
 * {@code retire(displaced)}, several statements after the put - so a scan arriving in that gap saw an empty debt
 ...
 * displacement branch reaches exactly once, after the put and before the (old) owe - the seam is the shard's own
 ...
 */
class ShardDisplacementOwesTheFlightBeforePublishingTest extends ShardSeamTestBase {

    private static final long CONTESTED_OFFSET = 7L;

    @Test
    void aScanLandingBetweenThePublicationAndTheOweTakesNothing() {
        ...
        assertWithMessage("KEY ORDER VIOLATED in the gap between publishing the replacement and owing the "
                + "displaced flight: a scan there was handed the fresh container while the displaced one was "
                + "still running (astubbs#178, second door, astubbs/parallel-consumer#517 review)")
                .that(takenInsideTheGap).isEmpty();

        // and the debt is settled the ordinary way
