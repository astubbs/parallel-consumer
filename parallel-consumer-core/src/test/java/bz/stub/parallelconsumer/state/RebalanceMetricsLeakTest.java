 ...
 *
 * <p><strong>An identity assertion was tried and rejected, so nobody spends the attempt again.</strong> Adding a
 * test accessor for the cached field and asserting the same instance across two assignments passes - but it
 * passes for the wrong reason. Mutating the CALL SITE to {@code new OffsetMapCodecManager<>(module)} while
 * leaving the field in place keeps that test green, because a {@code final} field is trivially the same
 * instance. It fails only when the field is deleted outright, and then it fails to compile rather than to
 * assert. A real seam would have to observe how many instances the assignment path constructs, which nothing
 * currently exposes.
 *
