...
class ProcessingShardStaleReplacement909Test {
    ...
    void staleResidentPlantedDirectlyIsReplacedByFreshWork() {
        ...
        // plantResident, not a raw map put: the shard's map is private precisely because every insertion has to
        // pair with a RecordPopulation admission, and a test that bypasses it drifts the population silently
        shard.get().plantResident(staleWc);
        assertWithMessage("PRECONDITION: the stale resident is in place")
                .that(shard.get().getWorkContainerAtOffset(700L).get().getEpoch()).isEqualTo(epoch0);

        addWork(epoch2, 700, "K_D");

        assertWithMessage("The fresh container must have REPLACED the stale resident, not been dropped. "
                + "See https://github.com/confluentinc/parallel-consumer/pull/909")
                .that(shard.get().getWorkContainerAtOffset(700L).get().getEpoch()).isEqualTo(epoch2);
    }
