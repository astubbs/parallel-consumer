...
public class OffsetMapCodecManager<K, V> {
    ...
    String makeOffsetMetadataPayload(long nextExpected, long highestSucceeded, TopicPartition tp, Set<Long> incompleteOffsets) throws EncodingNotSupportedException {
        String offsetMap = serialiseIncompleteOffsetMapToBase64(nextExpected, highestSucceeded, tp, incompleteOffsets);
        return offsetMap;
    }

    String serialiseIncompleteOffsetMapToBase64(long nextExpected, long highestSucceeded, TopicPartition tp, Set<Long> incompleteOffsets) throws EncodingNotSupportedException {
        byte[] compressedEncoding = encodeOffsetsCompressed(nextExpected, highestSucceeded, tp, incompleteOffsets);
     ...
     */
    byte[] encodeOffsetsCompressed(long nextExpected, long highestSucceeded, TopicPartition tp, Set<Long> incompleteOffsets) throws EncodingNotSupportedException {
        OffsetSimultaneousEncoder simultaneousEncoder = new OffsetSimultaneousEncoder(nextExpected, highestSucceeded, incompleteOffsets).invoke();
