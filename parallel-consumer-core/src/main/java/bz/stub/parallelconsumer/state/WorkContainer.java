...
public class WorkContainer<K, V> implements Comparable<WorkContainer<K, V>> {
    ...
    private boolean isClaimableFrom(ExecutionState observed) {
        return isNonResourceClaimable(observed) && hasResourceCreditIfTagged();
    }

    /** The claim's NON-resource terms over one observed state - the single source both claim and attribution read. */
    private boolean isNonResourceClaimable(ExecutionState observed) {
     ...
     * its retry delay passed. Delegates to the same {@link #isNonResourceClaimable(ExecutionState)} term the
     * claim itself evaluates, so U4's attribution gate cannot drift into an approximation of the claim it
     * explains. Pure, and package-private - only {@code ProcessingShard}'s attribution site reads it.
     */
    boolean isEligibleButForResourceCredit() {
        return isNonResourceClaimable(state.get());
    }
