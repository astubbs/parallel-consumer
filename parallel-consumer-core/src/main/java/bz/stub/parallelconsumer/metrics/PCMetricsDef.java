...
public enum PCMetricsDef {
    ...
    NAVIGATOR_SHARE_FRACTION("navigator.share.fraction", "Navigator (partition-share R9, KTD6) - this instance's current share of the tagged resource's declared rate, as a fraction in [0, 1]: local rate over global rate, so under partition-share it is the fraction of the subscription's partitions this instance holds. 0 while the instance holds no partition or its partition total is unresolved (R5), and under any allocator whose local rate is zero", PCMetricsSubsystem.NAVIGATOR, GAUGE, tag("resource", "resourceName")),
    NAVIGATOR_SHARE_CREDITS_PER_QUANTUM("navigator.share.credits.per.quantum", "Navigator (partition-share R9, KTD6) - the credits this instance's current share of the tagged resource is worth per quantum: local rate times the contract's quantum length in seconds. Fractional values are real - a share below one credit per quantum accrues through the allocator's remainder rotation rather than rounding to zero", PCMetricsSubsystem.NAVIGATOR, GAUGE, tag("resource", "resourceName")),
