...
public class ProducerWrapper<K, V> implements Producer<K, V> {
     ...
     *
     * @param expectedTransactionalId the {@code transactional.id} the configuration the factory received carried, or
     *                                null where it carried none (every commit mode but the transactional one)
     * @throws ProducerFactoryContractException when a {@link KafkaProducer} was not built from that configuration:
     *                                          transactional when the map said not, or the reverse, or transactional
     *                                          under a different id than the map carried
     */
    public static <K, V> ProducerWrapper<K, V> forPcBuilt(ParallelConsumerOptions<K, V> options,
                                                          Producer<K, V> producer,
                                                          String expectedTransactionalId) {
        return new ProducerWrapper<>(options, producer, Optional.of(Optional.ofNullable(expectedTransactionalId)));
    }

    /**
     * @param expectedTransactionalId empty on the producer-instance path, where nothing is expected; on the PC-built
     *                                path, the id the factory's map carried, or an empty inner value where the map
     *                                carried none
     */
    private ProducerWrapper(ParallelConsumerOptions<K, V> options, Producer<K, V> producer, Optional<Optional<String>> expectedTransactionalId) {
        this.options = options;
        this.producer = producer;
        boolean discovered = discoverIfProducerIsConfiguredForTransactions();
        Optional<Boolean> expectedTransactional = expectedTransactionalId.map(Optional::isPresent);
        ...
        if (discovered && producer instanceof KafkaProducer && expectedTransactionalId.isPresent()) {
            // the presence check above passed, so the map carried an id: a KafkaProducer can say which one it was
            // actually built under, and a substituted id breaks the same fencing a dropped one does - the replacement
            // would initialise under an id that fences nothing. Found by the review of astubbs#420.
            String expected = expectedTransactionalId.get().get();
            String actual = getTransactionManager().transactionalId();
            if (!expected.equals(actual)) {
                throw new ProducerFactoryContractException(msg("The ProducerFactory returned a KafkaProducer built " +
                                "under {} '{}', but the configuration PC handed it carried '{}'. A factory must build " +
                                "the producer from the map it is given, with {} unaltered: a replacement initialised " +
                                "under a different id fences nothing, and the id falls outside the TransactionalId " +
                                "ACL prefix the derived one was designed to fit.",
                        ProducerConfig.TRANSACTIONAL_ID_CONFIG, actual, expected, ProducerConfig.TRANSACTIONAL_ID_CONFIG));
            }
        }
