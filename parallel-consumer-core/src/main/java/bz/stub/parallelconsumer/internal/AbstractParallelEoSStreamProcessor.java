...
public abstract class AbstractParallelEoSStreamProcessor<K, V> implements ParallelConsumer<K, V>, ConsumerRebalanceListener, Closeable {
    ...
    /**
     * The confinement the ledger replay declares with {@code @ThreadConfined}, asserted rather than trusted: once a
     * control thread exists, only it may run it. Before one exists - an instance never started - nothing runs
     * concurrently, so there is nothing to confine, and a test may drive it directly.
     */
