...
public abstract class AbstractParallelEoSStreamProcessor<K, V> implements ParallelConsumer<K, V>, ConsumerRebalanceListener, Closeable {
    ...
    private <R> void controlLoopPass(Function<PollContextInternal<K, V>, List<R>> userFunction,
        ...
        if (revokeCommit != null) {
            ...
            if (isProducerBeingReplaced()) {
                // see maybeAcquireCommitLock: the request is answered, the commit itself is refused downstream
                log.warn("A revocation commit was requested while the producer is being replaced: no producer " +
                        "exists to commit with, so nothing is committed for the revoked partitions and their work " +
                        "since the last commit is reprocessed by the next owner. Their aborted transaction " +
                        "published nothing, so no duplicate results.");
            }
        }
    ...
    private boolean maybeAcquireCommitLock(boolean revokeCommitRequested) throws TimeoutException, InterruptedException {
        // a periodic commit waits out a producer replacement; a revoke commit does not, because its request was
        // taken for this pass and must be answered. While no producer exists, commitOffsets refuses with
        // ProducerInvalidatedException, commitOffsetsReportingPollerDeath absorbs it, and the request completes
        // with nothing committed - which is consistent: the aborted transaction published nothing and no offset
        // was committed, so the next owner reprocesses. controlLoopPass says so at WARN when it happens.
        final boolean periodicCommitDue = isTimeToCommitNow() && wm.isDirty() && !isRebalanceInProgress.get() && !isProducerBeingReplaced();
