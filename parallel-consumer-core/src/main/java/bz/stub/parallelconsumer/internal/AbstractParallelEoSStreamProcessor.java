...
public abstract class AbstractParallelEoSStreamProcessor<K, V> implements ParallelConsumer<K, V>, ConsumerRebalanceListener, Closeable {
    ...
    private void reportLoadFactorAtCeiling() {
        if (isVirtualThreadPool()) {
            // Silent under a virtual-thread pool: there is no dispatch queue to run dry, so "the queue is low" is
            // that pool's permanent resting state rather than a symptom, and both branches below would mislead -
            // raising either load-factor setting buys a deeper record buffer, not more concurrency, because
            // getQueueTargetLoaded() deliberately stops multiplying in this mode. Raise maxConcurrency instead.
            return;
        }
