...
public abstract class AbstractParallelEoSStreamProcessor<K, V> implements ParallelConsumer<K, V>, ConsumerRebalanceListener, Closeable {
    ...
    private boolean maybeAcquireCommitLock(boolean revokeCommitRequested) throws TimeoutException, InterruptedException {
        // isProducerBeingReplaced() gates the PERIODIC commit only. A revocation request still overrides it, as the
        // javadoc above says: the request IS the revoke commit, and a pass that cannot commit fails the request
        // rather than leaving the callback to sit out its deadline.
     ...
     */
    static final String CONTROL_THREAD = PartitionState.CONTROL_THREAD;
     ...
     * thread by construction - which is also why the producer recovery pass and the ledger replay
     * ({@link #maybeRecoverProducer}, {@link #replayWorkDiscardedByAbortedTransaction}) may be driven directly by a
     * test while still asserting their own {@code @ThreadConfined} declaration at runtime.
     */
