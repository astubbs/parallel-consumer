...
final class JacksonFormats {
    ...
    /**
     * No instances: this class is a holder for the shared mapper and the two factories {@link Formats} calls once
     * it has confirmed Jackson is there.
     */
    private JacksonFormats() {
    }

    /**
     * A route's JSON format for one class, labelled with the class's simple name so that a definition printed back
     * to its author reads as what they wrote rather than as a deserialiser class name (R4).
     */
    ...
    /**
     * One reader, resolved when the format is built, shared by every worker that decodes this route - see the
     * comment inside for why the per-record form was not good enough.
     *
     * @return a deserialiser that answers null for a null payload, so a tombstone reaches the route's function as
     * a null value rather than as a decode failure
     */
    private static <T> Deserializer<T> deserializer(Class<T> type) {
        ...
        return new Deserializer<T>() {
            /**
             * Satisfies {@link Deserializer#deserialize(String, byte[])}, and adds two things the contract does
             * not: a null payload is a null value rather than a failure, and everything Jackson throws is turned
             * into the exception type the engine's failure path expects, naming the topic and the target type.
             */
            @Override
    ...
    /**
     * The write half, for a route that produces JSON. It needs no declared type - Jackson writes whatever it is
     * handed - which is why this takes no argument where {@link #deserializer} takes a class.
     */
    private static <T> Serializer<T> serializer() {
        return new Serializer<T>() {
            /**
             * Satisfies {@link Serializer#serialize(String, Object)}, with the two additions its read-side twin
             * makes: a null value serialises to null, so producing a tombstone is possible, and Jackson's failures
             * arrive as the exception type the produce path expects.
             */
            @Override
