...
resolution_type: test_fix_precondition_accepts_null_or_empty
status: "SOLVED - test-side fix; the control experiment below is the evidence. Nothing enforces the rule beyond this write-up."
applies_when:
...
tags: [mdc, logback, surefire, fork-reuse, order-dependent, precondition, flake]
---
...
## The fix, and why it is in the test

PC treats `null` and `{}` identically - both mean "no diagnostic context", and `MdcPropagation` is written
that way. The precondition was asserting an implementation detail of the logging binding and calling it
PC's behaviour. It now reads the map and, if non-null, asserts it is empty; the comment above it names the
arming classes so the next reader does not rediscover them.

The product-side leftover is deliberately not touched: `{}` on PC's own controller thread is harmless,
and changing control-loop code to satisfy a test precondition would be the wrong direction. Adding an
`MDC.clear()` to the two arming tests was also considered and rejected as the primary fix - it would hold
only until the next test drives the control loop on the test thread, whereas accepting `{}` holds
regardless of who ran first.

## What to take from it

- **A precondition on shared thread state is a claim about every test that ran before yours on that
  thread.** With reused forks, that set is decided per run, so the assertion has a per-run outcome even
  when nothing about the test changes.
