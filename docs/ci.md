...
- **`repo-hygiene.yml`** - cheap repo-wide static checks needing no JDK, no broker and no build.
  **One job per concern**, named `<area>: <check>` to match the master ruleset's context
  ...
  shellcheck does not detect it. `rename` runs `bin/test-rename-packages.sh`, the self-test for the
  ...
  depends on it. `actions` runs `bin/check-action-versions.sh`, keeping every GitHub Action pinned
  to one version across all workflows. Self-tests run first. **`shell: sigpipe` and
  `workflows: action versions` are required status checks** - which is exactly why the job names
