...
| None of this was findable without knowing to grep | `inflight-type`/`impact`/`state` on every open note, grouped by consequence at session start, with a gate that rejects a tag the index could not place (astubbs#324) |
