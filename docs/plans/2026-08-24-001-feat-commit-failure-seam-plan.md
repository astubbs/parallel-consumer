...
- R19. Configuring a non-default commit-failure handler or continue mode together with the async consumer commit mode fails options validation, with a message naming the modes that support the seam and citing the async follow-up issue - the async exclusion is enforced at build time, not only documented.
...
- KTD2. **Exhaustion re-routes on the existing commit-response channel as a typed outcome.** The poll-thread commit layer catches `OffsetCommitBudgetExceededException` before `notifyPollerDied` fires, keeps the poll loop alive, and answers the waiting committer with a new commit-failure response variant - reusing the "answer the waiter even when the commit was deferred" pattern in `ConsumerOffsetCommitter#maybeDoCommit`. No new interrupt or bare flag crosses threads (the interrupt bit is already overloaded - see `docs/solutions/workflow-issues/waking-a-thread-by-interrupting-it-2026-08-17.md`). The waiter side changes with it: `commitAndWait` stops using `offsetCommitTimeout` as its wait deadline - with defaults that deadline expires before the poll-side budget can even exhaust (the budget needs more than the same duration, on a clock that starts later), so the old wait would kill the instance through the generic timeout before the failure response exists. Instead the waiter waits for an affirmative signal - its typed response (success or failure) or the poller-death notification - keeping the existing retry-limit drain backstop. This partially settles the waiter half of `docs/inflight/bug-offset-commit-timeout-does-two-jobs.md`; U2 updates that note in the same change. Covers R4.
- KTD3. **The handler runs from the control thread through a time-bounded invocation.** The control thread submits the handler to a dedicated single-thread executor and waits at most the handler time bound (default 30s); on timeout or a thrown exception it proceeds as `SHUT_DOWN`, so a hung handler ends in a real shutdown, never a wedged instance, and the broker-poll thread never executes user code. The invocation happens after the control thread has exited the synchronized `commitCommand` block - the failure propagates out as a typed outcome and the decision runs monitor-free - so a slow handler cannot stall a revocation callback contending for that monitor. Covers R14, R15.
...
- KTD7. **One exhaustion event; four handler-free exits.** Both budget lanes that already exhaust into `OffsetCommitBudgetExceededException` (`offsetCommitTimeout` and `saslAuthenticationRetryTimeout`) reach the handler as the same event. Handler-free and fatal, as today: non-retriable classifications (R6), poller death (R7), and the close-path abandonment exit (`commitSync`'s close-time single-attempt short-circuit throws `InternalRuntimeException`, not the exhaustion event) - which is also the resolved answer to "CONTINUE versus a close already in progress": once close begins, the handler is never consulted. The fourth exit is handler-free but NOT fatal: budget exhaustion inside a revocation-time (owner-thread) commit has no waiter to answer, so it follows the deferral disposition - caught at the deferral layer, offsets stay dirty for the new assignee, the poller stays alive (KTD9). Classification is per commit RPC, not per partition (Scope Boundaries).
- KTD8. **The transactional path gains the budget in place of its fixed attempt count.** `ProducerManager#commitOffsets` replaces the 200-attempt retry count with the `offsetCommitTimeout` budget and surfaces exhaustion as the same event type feeding the same handler, on the control thread it already runs on. Its existing fatal-vs-retriable block comment is the classifier spec; `sendOffsetsToTransaction` failures and the commit-lock acquisition timeout stay fatal. On a `CONTINUE` decision, the next cycle first attempts to complete the still-committing transaction and otherwise aborts it before beginning a fresh one - produced-but-uncommitted records replay because their offsets stayed dirty, and a transaction PC cannot abort classifies fatal per R6; without this recovery the stale transaction makes cycle two terminally fatal and the EOS pause window is one cycle long. Cites R1, R11.
- KTD9. **Rebalance-lane accounting lives beside the poller-death record.** A deferral counter and first-deferral timestamp sit next to `pollerDeath` in `ConsumerOffsetCommitter`; deferral time feeds the R3 history, and deferrals persisting past the bound clock synthesize an escalation to the handler on the control thread. Revocation-time commits are recognised via the existing `isRebalanceInProgress` flag and never consult the handler; budget exhaustion on that owner-thread path is caught and deferred (KTD7's fourth exit) rather than escaping to kill the poller; history is scoped to the assignment. Anything reachable from a rebalance callback takes locks only via `tryLock()` - the AB-BA deadlock in this exact area (astubbs#29) has an *unproven* fix, so the seam must not add blocking acquisitions there or depend on that fix landing. Covers R8, R13.
...
- **Requirements:** R1, R2, R3, R10, R11, R12, R19 (KTD1, KTD4, KTD6).
- **Dependencies:** none.
...
- **Approach:**
  ...
  3. Options fields with `@Builder.Default`; `validate()` gains both the EOS coercion and the R19 async-mode rejection, beside `transactionsValidation()`.
...
- **Test scenarios:**
  ...
  - Covers R19: async commit mode plus a non-default handler or continue mode fails validation, naming the supported modes and the follow-up issue; the all-defaults async configuration still builds.
...
- **Approach:**
  ...
  3. Rework `commitAndWait` per KTD2: wait on affirmative signals (typed response or poller death) rather than the `offsetCommitTimeout` deadline, and update the waiter row of `docs/inflight/bug-offset-commit-timeout-does-two-jobs.md` in the same commit.
  4. Control thread assembles the context (enrich `OffsetCommitBudgetExceededException` with the attempts/elapsed the budget loop already tracks; new consecutive/time-since accounting beside `lastCommitTime`).
  5. Bounded handler invocation (KTD3); `SHUT_DOWN` routes through the existing `closeOnException` path with the commit failure as cause; `CONTINUE` records, logs, leaves offsets dirty.
  6. Rewrite the exception's message and javadoc - the shipped text says "there is no way yet to hand this decision to your application", which becomes false (grep anchor: `no way yet to hand`).
  7. Re-verify `commitOffsetsReportingPollerDeath`'s backstop javadoc claims once exhaustion no longer kills the poller.
...
- **Test scenarios:**
  ...
  - A rebalance callback is not blocked while a slow handler is deciding (KTD3's monitor-free invocation).
  ...
  - The waiter outlives a slow budget: with a commit attempt held open past the old 10s deadline, the handler still fires and no `Timeout waiting for commit response` escapes (KTD2's affirmative-wait).
...
- **Approach:**
  ...
  4. On `CONTINUE`, resolve the pending transaction per KTD8 - complete it if the producer reports it finishing, otherwise abort - before the next commit cycle begins.
- **Execution note:** characterization coverage before behaviour change; the multi-cycle test verifies KTD8's complete-else-abort recovery rather than discovering whether any recovery exists.
- **Test scenarios:**
  ...
  - Repeated exhaust-continue cycles complete without tripping the terminal previous-fatal-or-abortable classification (KTD8's complete-else-abort recovery), and aborted-transaction records replay.
...
- **Test scenarios:**
  ...
  - Budget exhaustion during a revocation-time commit (consumer and transactional lanes) defers: poller alive, instance alive, handler not consulted, offsets dirty for the new assignee.
...
- **Approach:** new `COMMITTER` subsystem enum value; the four KTD10 meters; state gauge follows the `PC_POLLER_STATUS` model (`initMetrics` pattern in `BrokerPollSystem`); one ERROR log line per exhaustion regardless of decision. The R17 register entry in `docs/inflight/web-gui-surfaces.md` already exists (created with this plan); U6's obligation is keeping it accurate as the final metric names land.
- **Test scenarios:**
