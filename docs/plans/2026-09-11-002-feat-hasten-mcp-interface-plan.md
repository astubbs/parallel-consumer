...
- KD14. **The MCP server and the web GUI share one module.** Both are HTTP services over the same snapshot, so they ship together in the dashboard module, as one deployable. (session-settled: user-directed - chosen over a separate module with its own Java 17 floor: two HTTP services over one snapshot are one thing to add to a classpath.) The consequence planning must settle: the dashboard module inherits the Java 8 bytecode target and current JVM MCP SDKs are built for Java 17, so either that module's floor rises to 17, as the Mutiny module's did, or the MCP transport is implemented without the SDK. Governs R1, R12, R14.
...
- **Assumes** a maintained MCP server implementation for the JVM exists that can be hosted beside the dashboard's server; planning verifies this and chooses. Its class-file version is a selection constraint, not an afterthought: the dashboard module inherits the project-wide Java 8 bytecode target, and current JVM MCP SDKs are built for Java 17, so with KD14's shared module the choice is between raising the dashboard module's floor, as the Mutiny module did, and implementing the transport without the SDK. The Mutiny module's pom records why the build cannot detect a mismatch on its own.
...
- Whether the MCP route mounts on the dashboard's existing server and port or on a second port in the same module (R1, R14, KD14).
- Whether the dashboard module raises its bytecode floor to Java 17 to host an MCP SDK, or the MCP transport is implemented without the SDK to keep the Java 8 floor (KD14).
