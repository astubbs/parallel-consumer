...
- KD9. **The controller node is the navigator's controller rung, not a second coordinator.** The node that will coordinate named shared resources across applications (the rung astubbs#228 keeps open) is the node that holds the fleet view and, later, receives writes. (session-settled: user-directed - chosen over a separate fleet aggregator: the MCP surface meshes with the named-resource controller system rather than duplicating its election and its view.) Direction for the fleet milestone.
