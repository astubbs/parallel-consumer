...
- **Open blockers:** None blocking design. The module shape is chosen (KD14) and the substrate it sits on is astubbs#514, which is listed for confirmation rather than decision; every other open item is deferred to planning, named in Outstanding Questions.
...
- Which HTTP server this module runs, and its port-selection behaviour (R1, R14). It is its own listener now rather than a route on the dashboard's, so it registers the host allowlist and the cross-origin rejection itself - inheriting those by name from the dashboard is not the same as having them, and an MCP server that skipped them would have none.
