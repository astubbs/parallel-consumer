...
- **Which module shape carries the two surfaces** (KD14): one shared module, an MCP module depending on the dashboard module, or a substrate module both depend on. It decides whether a web-GUI-only application must resolve the MCP SDK, which is the Open Question below, and whether KD13's substrate API is a published artifact or a description of the snapshot. Planning cannot lay out the work without it.
