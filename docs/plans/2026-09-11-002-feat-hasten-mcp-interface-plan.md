...
- **Open blockers:** None blocking design. The module shape is chosen (KD14) but its artifact is not in this PR, so it is listed for confirmation rather than decision; every other open item is deferred to planning, named in Outstanding Questions.
...
- KD13. **Four surfaces over one API, and the MCP server first.** The target shape is a web GUI, a CLI tool, an MCP server and the API they all consume; the API is the substrate, and each of the others is a projection of it, the CLI included. The MCP server is the first of the projections built, because the audience with an agent attached is the one it unlocks, and because Parallel Consumer holds internal state - the sparse completion frontier, the offset map's fill, the ordering domains, the admission reasons - that the project believes no comparable library exposes. That belief is a **claimed** differentiator in the adjacent-systems register's evidence vocabulary: nobody has checked it, and the register's question set does not yet ask what a neighbour exposes rather than how it schedules. It must not be stated as fact in announcement material until it is. Pointing an agent at the dashboard's state document was the rejected alternative for the first projection: the discriminator is that MCP tools describe themselves, so an agent with no Hasten knowledge attaches, selects and interprets without a human wiring a URL or explaining the fields. The cost is a JVM MCP dependency inside the user's application classpath, weighed under Dependencies. In milestone one the API is the dashboard's published snapshot and its state-document route, which R6, R7, R8 and R18 extend; that is the contract the CLI's "once the API is stable" precondition refers to, so no second contract is created. The work is a candidate for the 6.1 release. (session-settled: user-directed - chosen over a CLI first and over the state document alone: all four surfaces are wanted, the MCP server is the initial aim, and the CLI follows on the same API.) Governs R1, R5, R17.
- KD14. **The MCP server and the web GUI ship together, and Java 17 is their floor. Which module shape delivers that is reopened.** Both are HTTP services over the same snapshot and neither is wanted without the other's substrate, so they are one unit of work and one runtime floor; the core library's floor is untouched and follows Kafka's, per the standing rule in `pr-53-java-baseline-kafka4`. An application on Java 8 or 11 keeps the core and loses both surfaces, which the owner judged a reasonable compromise: a Java 17 floor for that code massively simplifies its development. (session-settled: user-directed - chosen over keeping the surfaces at the core's floor, and over implementing the MCP transport without an SDK to stay there.) **Chosen 2026-09-12, and built on a branch this PR does not carry: the third shape.** A substrate module, `parallel-consumer-observability`, holds the state reading and declares only core, so the web GUI and the MCP server each depend on it and neither inherits the other's dependencies. That makes KD13's "the API is the substrate" a real artifact rather than a description, and it is what lets an application take the web GUI without resolving the MCP SDK - the Open Question this plan carried. The two surfaces still share one module above it at the Java 17 floor; the substrate stays at core's floor, which follows Kafka's, because nothing in it needs more. **The module is not in this PR's tree** - it lives on an unpushed branch - so a reviewer here cannot check the dependency or Java-floor claims against an artifact. Treat the shape as chosen and evidenced elsewhere rather than proven here, and re-confirm it when that branch lands. Governs R1, R12, R14.
...
- R2. Every tool is a read, and that property is enforced at the protocol layer rather than by HTTP method: the server advertises a fixed, enumerated read-only tool set, registers no write-capable tool, resource or prompt, declares no capability that lets it act back on the host, and answers any request outside that set as a protocol error. The dashboard's rejection of every non-read HTTP method cannot carry this property, because every MCP call is a POST, including a pure read.
...
- R18. Per-partition evidence carries whether the partition is blocked from taking new work, and how much of its offset-map capacity is used against the threshold that blocks it, so the cause can be attributed to a partition rather than counted instance-wide. **This is new engine telemetry, not a projection.** The engine retains neither figure today: the encoded payload length is a local inside the encoding step, and what persists is a boolean plus an aggregating distribution summary. So the engine must retain and publish them, with its own sampling semantics and owner - it cannot be derived on a request thread without violating R4, and calling it a projection would violate KD3.
...
- R15. A staged scenario runs one instance into the situation "a partition has stopped taking new work because its offset map reached capacity behind one incomplete record". It is **not** a Kafka partition pause, and the scenario must not assert one: crossing the metadata threshold clears that partition's `allowedMoreRecords`, which stops non-frontier work being taken for it, while the engine's only consumer-level pause covers the whole assignment and fires on global buffer load. Asserting a pause would either fail or observe a coincidental global pause and attribute it to the wrong mechanism. It is a new phase built on the dashboard's showcase scenario, not a reuse of it: that scenario's head-of-line phase strands a deliberately small band of offsets which its next phase clears, and no phase pauses a partition. The new phase holds one partition blocked until its offset map crosses the back-pressure threshold, asserts that the partition is paused, and keeps it paused for the length of the agent's session.
...
- AE1. The paused partition is explained
  - **Covers R15, R16, R18.**
  - **Given** the staged scenario has stopped partition 3 taking new work because its offset map reached capacity behind one incomplete record. The partition is blocked, not Kafka-paused.
  - **When** an agent connected only to the MCP server is asked why partition 3 is paused and what to do.
  - **Then** the agent's answer names the blocked partition, the incomplete record holding the frontier, the offset-map capacity as the cause, and an action that addresses the incomplete record rather than the partition count.
...
- **A web-GUI user cannot decline the MCP dependency** — Dependencies / Assumptions (P2, security-lens, confidence 75)

  An application that wants only the web GUI resolves the MCP SDK and its transitive tree anyway, because both surfaces ship in one module by KD14, and it cannot opt out. The alternatives each cost something real: an optional dependency the application declares itself, a second artifact, or accepting the footprint. Left open because it is a packaging decision with no obviously right answer, and KD14 chose the shared module before this consequence was named.
