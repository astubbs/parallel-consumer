...
- KD14. **The MCP server and the web GUI share one module, and that module's floor is Java 17.** Both are HTTP services over the same snapshot, so they ship together in the dashboard module as one deployable, and the module declares a Java 17 runtime floor the way the Mutiny module does, so a JVM MCP SDK built for 17 can be hosted in it. The core library's Java 8 bytecode target is untouched, but an application on a Java 8 or 11 runtime cannot load the shared module at all, so it loses the web GUI as well as the MCP server. Milestone one accepts that rather than mitigating it. (session-settled: user-directed - chosen over a separate MCP module with its own floor, and over implementing the MCP transport without an SDK to keep the module at 8: two HTTP services over one snapshot are one thing to add to a classpath, and a Java 17 floor for that module massively simplifies its development; the compromise is that an application still on Java 8 gets the core library and not the GUI or MCP module, which the owner judges reasonable.) Governs R1, R12, R14.
