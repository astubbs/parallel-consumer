...
- R2. A route binds one topic, or under R5 a set of topics, to one processing function; the API spells the binding `topic` and `topics`. Registering a second route for a topic already routed is refused at definition time, and the refusal says that a second function over the same topic is a second definition with its own consumer group (owner, 2026-09-10; fan-out inside one instance is a later engine unit, listed under considerations).
...
- Fan-out, two functions over one topic in one instance (owner-directed deferral, 2026-09-10): not a lookup but an engine unit. One work container per record per function, with the offset completing when all of them do, so a failed function retries alone and its side effects are not repeated by a succeeded one; retry, park and stop per function follow from that; the shard key includes the function under key ordering; the route meters gain a function tag. Until then a second function over a topic is a second definition with its own consumer group, which is Kafka's own model and gives each function its own progress, retries and parked set, and the definition-time refusal says so (R2). Builds on the per-topic function registry, so it lands after it.
