...
- R2. A route binds one topic, or under R5 a set of topics, to one processing function; the API spells the binding `topic` and `topics`. A topic may carry any number of routes, their functions running concurrently over every record; in this release the record succeeds when all of them do and a throw from any is a retry of the record (KTD15; the refusal of a second route that the first cut shipped is removed by the unit KTD15 governs, and per-function retries and parked sets are its later step).
...
- KTD4. **Park rides the retry-delay hook; the facade keeps the user-function attempt count, and the engine's count is a cross-check.** Every throw the facade makes to hand a record back to the scheduler, a breaker withhold, a route at its limit, an export re-dispatch, the stop fence, advances the engine's failure counter as if the function had failed, because the engine has no other hand-back path today. So the facade keeps a per-record count keyed by topic, partition and offset that advances only when the user function ran (a transient decode failure counts, R12), cleared for revoked partitions by its rebalance listener, which is R10's per-assignment rule; that count is what the retry limit, the parked view and the export headers use. The engine's own count, rebuilt on reassignment, is read only as a control-arm cross-check, and in Milestone A the two agree because every throw there is a real failure. The engine takes the count and every hand-back over when it owns the outcomes natively (U17), with the return-with-delay of U11 as the stepping stone. The wrapper records its intent for a record before it throws, in a thread-local keyed by topic-partition and offset (a failed batch calls the provider for every container on one thread, and two partitions carry the same offset number), because the engine calls the provider synchronously inside the failure path; the provider returns a far-future but representable delay for park, the remaining open duration for a breaker withhold, or the age bound for a timed export. The engine replaces a provider that throws, returns null or returns a negative delay with the default delay and a rate-limited warning, so a facade fault would turn parks into one-second retries silently; the control arms pin that. The parked view reconciles the facade's map against the engine's incomplete offsets for assigned partitions in the control-thread snapshot, since a worker finishing after a revocation can leave a phantom entry. Every facade-originated throw extends the engine's retriable exception so it logs quietly. Governs U3, U21, U22, U4, U7.
...
- KTD6. **Stop is mark, pause, throw, then a close from the handle's own thread.** A worker cannot close the engine it runs in, because close awaits the worker pool, and the control loop cannot close itself from its loop-end hook. The wrapper marks the record stopping with a far-future delay so a drain does not re-invoke it, calls the engine's non-blocking pause so no further record is dispatched in the window before the close, throws, and signals the handle, which closes on the declared path and records the reason so `awaitShutdown` can tell stop from close from failure. Pause stops the control thread handing out work but not the tasks already queued in the worker pool, so the wrapper also sets a stopping flag it checks before every function call: any record dispatched after the mark is thrown back with the far-future marker instead of run, stays incomplete, and is delivered again after the restart. The drain-first path dispatches the records already buffered before it closes; only the dont-drain path gives "in-flight completes, nothing new starts", which is what AE14 now says. Governs U4, U21.
...
- KTD15. **The engine takes any number of functions per topic, runs them concurrently, and judges them collectively first.** A record on a topic with N functions is one work container whose N functions run concurrently on the worker pool; the record succeeds when all of them have returned and a throw from any one is a retry of the record, which re-runs them all. Those are the semantics of one function that calls the others by hand, so nothing is hidden, and neither the offset map nor the shard key changes. Isolated success, one work container per record per function so a failed function retries alone and each has its own parked set, is the later step with the same public shape. The classic API gets the same registration in its own verbs, and the facade registers each route's function against its topics at start instead of looking the route up by topic inside one engine function, which deletes the route table and the one-route-per-topic refusal. (session-settled: user-directed, 2026-09-10 - first as a per-topic registry to delete the facade's lookup, widened when the owner asked why a second function on a topic would be a problem at all and why it would ever run in sequence, and staged collective-then-isolated as the owner's stepping stone.) Amends KTD2's one-function-with-a-route-table and R2's refusal; governs the unit that follows the simplify pass.
