...
- KD15. **A replaced member keeps its old name as a deprecated delegate once it has shipped - and not before.** When a member that has been released is superseded, the old name stays, marked `@Deprecated` and delegating to the new one, with a javadoc line saying what to call instead and why it changed; the new name is what the README, the examples and the Illustrative surface use from that moment, so a reader meets one name and a compiler meets both. Removing such a member is then a separate decision, release-gated, belonging to the breaking-change queue in `docs/refactoring.md` rather than to this plan. **The distinguishing question is "could someone already be calling this from a release?", not "is the type annotated unstable".** For the fluent package the answer today is no: it has never been released, nothing outside this repository can be calling it, so a deprecated alias there protects nobody while its javadoc claims a compatibility that was never at stake - which teaches a reader the wrong thing about what this package guarantees. The fluent package therefore renames outright, and earns the protection above the day it ships; that is also the day its first removal becomes release-gated. The classic API has shipped and keeps the full protection now, which is also all KD1 ever said about it as a surface: nothing here deprecates the classic API in favour of this one. **This decision was first recorded the other way on 2026-09-12 and corrected the same day, and both halves are kept here deliberately, because a reader who found only the corrected version would re-derive the wrong one.** As first written it applied the deprecation rule to the whole fluent surface, reasoning that `@InterfaceStability.Unstable` on every type in the package, and the compatibility gate's exclusion for it (R20), describe what the project reserves the right to do rather than what it should do to someone who has already built against a branch or a snapshot - and that on this surface someone had, because Milestone A is in flight and its examples are written against the names it carries today. The owner corrected that premise the same day (owner-directed, 2026-09-12: "Remember there are no users of the fluent api yet. It doesn't exist yet."), so the protection had nobody to protect: the delegates the KD16 rename pass had added under the earlier wording were removed rather than left lying, and `git log --follow` on `ParallelConsumerDefinition.java` carries both passes. The instruction that produced the rule in the first place (owner-directed, 2026-09-12: "Deprecate the old metrics method don't just rename it. Just because it has the annotation doesn't mean we need to pull the rug out from under people. Remember that.") stands unchanged for every shipped member, which is what it was about; what was wrong was extending it to an API that has never shipped, where there is no rug and nobody standing on it. Governs R19, R37, KD16.
- KD16. **Every setting on the definition carries a `with` prefix, and the whole family changes in one pass.** Not the two new ones alone: a surface where two settings are prefixed and the rest are bare is inconsistent with itself, and a reader then has to know which family a setting belongs to before they can guess its name. One rule removes that. (owner-directed, 2026-09-12: "Make the with consistent. Now is the cheapest it'll be and 'with' is expected.") The mapping, so that a later implementer follows it rather than re-derives it: `commitMode` becomes `withCommitMode`; `consumer` becomes `withConsumer`; `producer` becomes `withProducer`; `rebalanceListener` becomes `withRebalanceListener`; `meterRegistry` becomes `withMetrics`, which is also where the name stops naming the type it takes (R19); `whenClosing` becomes `withClosePath`, after the `ClosePath` it takes; `whenTopicMissing` becomes `withMissingTopicPolicy`; and the per-route defaults take the prefix in front of the one they already carry, so `defaultOrdering`, `defaultConcurrency`, `defaultRetryLimit`, `defaultRetryForever`, `defaultRetryDelay`, `defaultAfterRetries` and `defaultOnParked` become `withDefaultOrdering` and the rest in that shape. `withTracer` (R37) joins as a new member rather than a renamed one. Route declaration and lifecycle are not settings and keep the names they have: `route` with its `topic` and `topics` spellings, `start`, `validate` and `close`. Two sub-families needed a judgement rather than a mechanical prefix, and this is it. The `when...` pair reads as English today and the prefix makes both names slightly worse rather than better, which is the honest cost of the rule; they take it anyway, because an exception here is exactly the thing a reader would have to memorise, and each keeps its meaning by naming the type it takes rather than the sentence it completed. The `defaultX` family becomes double-barrelled, and that is the right trade: `default` is not decoration, it distinguishes an instance fallback from the per-route declaration of the same value, which is KD11's spelled-apart rule, so it stays inside the name and the prefix goes in front of it. **The one I am least sure of is `withMissingTopicPolicy`**, because `whenTopicMissing(FAIL)` reads as a sentence and its replacement names a policy object the user is not holding; `withMissingTopic` was rejected as reading like a topic being supplied. Being slightly wrong is cheap here and that is deliberate - though not for the reason first recorded, which was that under KD15 every old name survives as a deprecated delegate, so a name judged wrong later costs a second deprecation and never a break. KD15 was corrected the same day to cover shipped members only, so on a package that has never been released a name judged wrong costs a rename instead: cheaper than a deprecation rather than dearer, but cheap only until the package ships, after which it does cost the deprecation. That is an argument for settling the whole family now, which is what this decision does, and not a reason to reopen the mapping above. Governs R19 and R37; the mapping above is the authority, and the Illustrative surface below still shows the pre-KD16 spellings, which it says at its own head rather than being rewritten here, because this is a dated record.
- KD17. **A setting taking a percentage takes a `Percent`, and there is one such type for the whole library.** A bare number was not enough because a percentage has two plausible spellings, out of a hundred and as a fraction of one, and a caller who picks the wrong one is out by two orders of magnitude with nothing to tell them; the type states its unit at the factory, where the call site is written, rather than only on the class a reader never opens. (owner-directed, 2026-09-12: "Those dlq methods should take doubles not int unless we add a Percent.percentOf or something which would be nice when you add the static import. Actually yes do that and an overload that takes a double. Drop the int version.") Each such setting keeps a plain-number overload for a caller who does not want the ceremony, and that overload builds the type, so the refusals are the same through either door; the integer form is gone rather than deprecated, KD15 covering shipped members only. **Where it lives was settled the same day** (owner-directed, 2026-09-12: "Of course DRY if the engine already has a percentage class then use it of course") - the engine had none, so it is created in core's own package rather than beside the first settings to take it, and no second percentage type may be added anywhere in this library. That makes it part of the shipped surface rather than the incubating one, so it carries `InterfaceStability.Evolving` as `ParallelConsumerOptions` does rather than the fluent package's `Unstable`, and it is what the pause threshold takes if U10 makes that an instance option. **What the type refuses, and what it does not**: it refuses what is not a percentage at all - not a finite number, zero or negative, above a hundred - and refuses it at construction, so a bad value never reaches a setting whose own refusal would explain it as something else. It does not carry this engine's ceiling: that is measured down from the pause threshold, so it stays on the setting, because a type that refused values above that threshold could not express the threshold itself. Governs R27, and R28's payload fraction if that ever becomes a setting rather than a reading.
...
- R4. A route declares its deserialisers through the general form, consumed with a key and a value deserialiser, or through a format helper that resolves to the deserialiser already on the classpath for JSON, Avro or Protobuf, with the key defaulting to string; a format-named route, json, avro, protobuf or bytes with the topic, is sugar that desugars to the route form. The JSON helper without a class yields the payload as a map of field names to values, so a topic nobody has a class for can be consumed and inspected by field name with nothing declared; it uses the same optional JSON dependency as the class-typed form. A route's deserialisers are applied per record inside the facade and, when the route declares produced types, its serialisers are applied to produced records before they reach the produce path; the consumer and the producer are both configured for raw bytes by the facade, never by the user. The facade creates a producer only when something needs one: a route that declares produced types (R3), a dead-letter destination (R13), or the transactional commit mode (R6); a definition with none of these opens no producer, and the type-level rules of R3 and R8 are what make producing without one a compile error rather than a runtime failure. Deserialiser settings supplied in the connection properties are refused at definition time with a message naming the setting and the route that supersedes it; the remaining properties are passed to each route deserialiser's configuration.
  - **A route's deserialisers are declared per worker, not per route (owner-directed, 2026-09-12: "then the hand in must be a supplier").** Kafka contracts a `Deserializer` to be safe on the single thread a consumer polls on and promises nothing more, and this facade decodes on whichever worker took the record - so one instance per route silently required thread safety of every deserialiser a user handed in, which is the defect the cross-model review raised. A route may therefore be declared with a SUPPLIER of each half, `Consumed.perWorker` and `Produced.perWorker`, and each worker thread is given its own instance, made once for that thread and reused for every record it decodes. The instance-taking declarations are kept, because Kafka's own stock serdes are stateless and `Consumed.with(Serdes.String(), ...)` is the shape this plan borrowed deliberately; what changed is that they now SAY the instance is shared and must be thread-safe, the sharing having been the unstated part rather than the wrong part. This refines the last clause above: the remaining properties are passed to the configuration of every instance a route makes, not to one per route, and every instance made is closed when the instance shuts down.
...
- R6. Only the commit mode and, once the commit-failure seam lands (astubbs#352), the commit-failure policy are instance-wide, because the engine has one consumer, one commit and one transaction; declaring either on a route is refused at definition time. Everything else is per route with an instance default: dead-letter destination, ordering mode, concurrency limit, which is the route's admission target (CONCEPTS.md), retry limit, retry delay, park policy and breaker are per route: each route takes a copy of the instance default unless it declares its own, and the surface names the two kinds apart, instance-wide settings plain and per-route defaults with a default prefix. Per-route ordering is an engine change on one seam, the shard key and the shard's head check, so it ships in the medium tier and the tiny tier accepts ordering only as the instance default. Cross-topic key identity, whether one key on two topics is one shard, stays with astubbs#150. The declared admission target is a starting point, not a constant: the self-scaling work (astubbs#333 and the navigator rungs astubbs#392, astubbs#456) makes admission adaptive per route at runtime, and merges after this. The export percentage of R27 is an instance default, seventy unless declared, which a park policy may override; it is a whole percentage of the cap, and a value above the engine's pause threshold minus five points, seventy today, is refused at definition time.
  - **The commit-failure policy is removed from Milestone A on 2026-09-11, owner-directed - the requirement stands, the setting does not ship until the commit-failure seam lands (astubbs#317, astubbs#352).** Milestone A shipped `CommitFailurePolicy`, `ParallelConsumerDefinition#commitFailure` and `Route#commitFailure` as a type that existed only to be named in two refusals: the instance-wide setter refused because the seam had not landed, and the per-route setter refused the placement. That is the third state `062bdea6d` removed for the circuit breaker - a refusal to document now and delete again later - and it is worse here, because the refusal is keyed to a pull-request number rather than to a durable design line, and a user who tries the route form is refused twice to learn the feature does not exist. A method that is simply absent says the same thing in one hop, at compile time. The seam's own vocabulary is `CommitFailureHandler`, `CommitFailurePolicies` and `CommitFailureContinueMode`, so the pull request that brings the setting back decides what the fluent word maps onto rather than inheriting `KEEP_GOING` from a placeholder, and must settle the seam's refusal of a non-default handler under the asynchronous commit mode, which the sample below uses. Nothing else in the plan changes; the sample below and KD11 both describe the API R6 still specifies.
...
- R27. A parked record stays incomplete in the offset map, holds no worker, and is not re-attempted until its park delay elapses, which is how scheduled retry (astubbs#234) is delivered: a park policy may declare a delay and a number of cycles together, after each delay the record is attempted once more and after the declared cycles it parks without delay, or no delay, meaning parked until resumed or exported; a permanent decode failure (R12) never takes the delay path, and the parked view reports a record's cycles beside its attempts (R28). Otherwise it waits until an operator resumes it through the handle (R28), or a restart re-delivers it (R10's per-assignment rule). Parked records count against the partition's offset-map payload, never against the intake load gate. When a partition's payload reaches the declared percentage of Kafka's commit-metadata cap, seventy by default, its parked records are exported oldest-first to the declared dead-letter destination until the payload is below that percentage; with no destination declared, the partition stops at the cap's pressure threshold as it does today: it fetches nothing more and takes no buffered record above its highest succeeded offset, and since a parked record never completes by itself, nothing shrinks the payload until an operator resumes or exports through the handle (R28). A declared age bound exports a parked record before the topic's retention could delete it; when no bound is declared and retention removes a parked record on the broker, the running instance is unaffected: the engine holds the parked record in memory, so it can still be resumed or exported, a small advantage of park in place over a broker-side queue; only a restart loses it, since the record cannot be re-polled, and the map entry is dropped at partition bootstrap. The documentation says so, and says a route whose parked records must survive a restart parks with a destination or an age bound (user-directed, 2026-09-10: document it, no client-side state changes on a broker-side retention). An instance may declare export-immediately, which is the classic dead-letter queue. A route may instead declare stop on exhaustion: when one of its records exhausts its retries the instance stops through the declared close path as if the function had reported the stop outcome (R24), with the exhausted record left incomplete for re-delivery after a restart; it is the third reaction beside park and export, so the retry epic's skip, dead-letter or die (astubbs#239) is answered as data, and the loop an automatic restart creates is the definition author's to break, as R24 says (user-directed, 2026-09-10). The setting is a whole percentage of the cap, spelled `dlqWhenOffsetPayloadReaches` on the API, and its ceiling is the engine's pressure threshold minus five points: the threshold is seventy-five percent today, at which the partition stops taking work, so a percentage at or above it is never reached, and five points below it is the margin the owner chose. The default is that ceiling, seventy, and a declared value above it is refused at definition time. Making the threshold a per-instance setting is a small-tier engine change planning may take instead of holding the ceiling down. Both numbers are provisional on today's encoding: exact continuous offset encoding (astubbs#237, confluentinc#53) makes the payload size precise, so the default and the ceiling are revisited when it lands, and `docs/inflight/core-237-continuous-offset-encoding.md` carries that reminder. The parked set is queryable and metered (R28), and the documentation states that consumer-group lag reads as stuck at the oldest parked record.
  - **The dead-letter surface moved out of Milestone A on 2026-09-11, owner-directed - the requirement stands, and it moved rather than being dropped: it is carried whole on a pull request stacked above astubbs/parallel-consumer#502, which stays a draft until the engine can honour it.** Milestone A accepted a dead-letter policy at definition time and then refused at start, so a user could write a definition that looked complete and learn at start that its declared reaction does not run. What it waits on is not a detail: export is a re-dispatch, and the commands it needs - producing from the parked set within the transaction's budget, oldest-first once a partition's offset payload reaches a fraction of the commit-metadata cap - are Milestone C. Two settings have already left this milestone on the same reasoning, the circuit breaker (`062bdea6d`) for being accepted and inert and the commit-failure policy (`b8371c5d1`) for existing only to be named in a refusal; this one differs in size and in being worth keeping written down, which is why it is relocated rather than deleted. Gone from Milestone A: `AfterRetries`' `dlqTo`, `dlqImmediately` in both forms, `dlqOlderThan`, `dlqWhenOffsetPayloadReaches`, `MAX_PAYLOAD_PERCENTAGE` and the accessors over the destination and the three triggers; `ParallelConsumerDefinition#dlqWhenOffsetPayloadReaches` and the start-time refusal; `DefinitionRules`' four dead-letter refusals; `ParkedView#dlq` in both forms and `estimatedTimeToExport`, which measures a distance to an event this release has no way to cause. The stacked work also adds the reaction the enum never had - a dead-letter member beside park and stop, with the factory a user reaches for - since the surface could qualify a park with an export trigger but could not say "dead-letter it". Nothing else in this plan changes: R13, R14, R15, R27 and R28 all still specify the behaviour, and the requirement returns with the engine work rather than being re-derived.
  - **The percentage is a `Percent` value type rather than a whole number, owner-directed on 2026-09-12.** The sentence above still reads "a whole percentage", and it is not rewritten: what changed is the type the setting takes and not the quantity it means, and a percentage that happens to be whole still is one. KD17 owns the decision, including why the ceiling stayed on the setting while the unit went onto the type, and why the type lives in core rather than in the fluent package. The Illustrative surface below shows the pre-KD17 spelling for the same reason it shows the pre-KD16 one, which its own head states.
  - **The setting is spelled `withDlqAtOffsetPayload` on the definition and `dlqAtOffsetPayload` on a route's park policy** (owner-directed, 2026-09-12: "withDlqAtOffsetPayload is great"). R27's sentence above still gives the earlier spelling and is left as written, as are the Illustrative examples below. The two read alike deliberately: it is one trigger declared on either side, so only the `with` prefix separates them - the definition-versus-route distinction KD16 draws, whose mapping covers settings on the definition while a route's own qualifiers (`dlqTo`, `dlqOlderThan`) stay bare. The name came from using the API rather than reading it: once the value carries its own unit (KD17), `...Reaches` was the only word still doing work and it wants a bare number after it, where a preposition fits the phrase the value now reads as. The refusal quotes whichever of the two spellings the user wrote, which is why both live in one place in `DefinitionRules`.
...
- R29. A route may declare a circuit breaker: a failure-rate threshold over a window of attempts, and an open duration. A failed attempt is a throw (R9), a transient decode failure (R12), or a park or export on exhaustion, so a dependency that is down opens the route on its first records rather than after each has exhausted its retries. When the rate crosses the threshold the route opens: its records are withheld without counting an attempt for the open duration, then a declared number are let through half-open, and the route closes when they all succeed or re-opens for another open duration when any of them fails. Other routes are unaffected: the breaker declared on the definition is the per-route default of R6, copied into each route, and no breaker state is shared between routes. Open, half-open and closed transitions are counted (R19) and the state is on the route's handle (R28). An open route's records are still fetched and wait in memory until the medium tier pauses its partitions on the poll thread (R31). The policy is data. Retries and the breaker answer different failures: a retry is one record's transient failure, the breaker is a dependency that is down.
  - **Removed from Milestone A on 2026-09-11, owner-directed - the requirement stands, the setting does not ship until U7 builds it.** Milestone A shipped `CircuitBreakerPolicy`, `Route#circuitBreaker`, `ParallelConsumerDefinition#defaultCircuitBreaker` and `RouteView#circuitBreaker()` as an accepted setting that nothing read: the policy was validated, copied into every route and then never consulted, because the behaviour above is U7's and U7 is Milestone B. That contradicts the package's own rule that a setting it cannot honour yet refuses at definition time rather than accepting silently - R6's per-route ordering is the model, and it throws. Deleting beats refusing here: a refusal is a third state to document and to remove later, while the requirement is already specified in this line and scheduled in U7, so the accessor returns in the pull request that builds the breaker. Nothing else in the plan changes; the sample below and the scope table's `In scope (R29), per route` row both describe the API R29 still specifies.
...
- R17. The handle exposes a bounded graceful shutdown that drains in-flight work, is usable with try-with-resources, and a blocking wait for shutdown. The handle's close drains, bounded by the drain timeout, with the shutdown timeout bounding the close that follows; this differs from the classic API, whose plain close does not drain and is bounded by the shutdown timeout alone.
  - **What this document calls the handle is spelled `ParallelConsumerInstance` in the Java API as of 2026-09-11, owner-directed** - it was `ConsumerHandle`, which the owner rejected on review of astubbs/parallel-consumer#502 ("for now", so the name may move again before the surface leaves incubation). Nothing about the requirement changes: the word *handle* stands in this document as the thing a started definition hands back, and the repository's own vocabulary already said *instance* - `CONCEPTS.md` carries an entry for it, and `InstanceFailedException` and `InstanceControl` were named for it before this. `InstanceControl` keeps its name. The per-route accessor it hands out is still `RouteHandle`.
...
- R19. Outcome counts (succeeded, filtered, parked, exported) and the parked-set gauges of R28 are published through the existing metrics integration, tagged by topic and, for the gauges, by partition. The declaration that supplies the registry is spelled `withMetrics` on the definition, not `meterRegistry`, so that it names what it turns on rather than the type it happens to take and reads as a pair with the tracer declaration of R37 (owner-directed, 2026-09-12: "meterRegistry is overloaded, registry is too vague; we should deprecate it and replace with withMetrics and future: withTracer etc"). On the fluent API the old name is simply gone: `meterRegistry` was renamed rather than kept beside the new name as a deprecated delegate. As first recorded this requirement said the opposite, that it stays as a delegate under KD15 so that the package being incubating and excluded from the compatibility gate (R20) is not read as permission to break whoever has already built against the branch; KD15 was corrected on 2026-09-12 to cover shipped members only, and that reasoning fails on its premise, because nobody outside this repository has built against this package at all. It arrives with `withTracer` and with the rest of the setting family under KD16, in one pass rather than one setting at a time. On the classic API this plan takes nothing: the options builder's meter-registry setting is shipped and stable, and replacing it there is its own decision in two halves, an addition, which would be a new entry on R34's closed list and must be recorded there rather than folded in, and a removal, which is release-gated and belongs to the breaking-change queue in `docs/refactoring.md`, the section that owns changes of that kind. Raised beside the fluent change on 2026-09-12; the two are the same instinct and not the same cost.
- R37. A tracer is declared on the definition, beside the metrics declaration of R19 and with the same lifecycle: supplied once before start, in force for the instance, and released when the instance closes, the way its meters are registered at start and removed at close. It is spelled `withTracer`, the name `withMetrics` was chosen to pair with. With none declared the library traces nothing and costs nothing, which is the shape the absent meter registry already has, where the engine registers into a no-op registry and nothing is published. The declared type is this library's own, not a telemetry vendor's, and that is where this surface diverges from the stream-processing libraries users compare us with, whose equivalent takes the vendor's type directly; KTD16 takes their shape where one is open but requires the constraint be named at the point of divergence, and the constraint is core's dependency footprint, a published, user-visible promise with a feature record of its own (`docs/features/core-dependency-footprint.yaml`) that core stays focused on the Kafka client and its direct support dependencies. No requirement here may put a telemetry SDK on core's classpath as a side effect, so core declares a small interface and a telemetry-backed implementation satisfies it from outside core. Where that implementation lives is open: a module of its own beside the integration modules, the user's own code, or the proxy-client side that "Parked, post-v6: OpenTelemetry across the client libraries" argues from. The metrics half is not the precedent for taking a vendor type, because it rides a vendor-neutral facade that is already a compile dependency of core, and this would be adding one. What the seam must carry is stated as behaviour, not signature: a record arriving carries whatever trace context its producer put on it, the processing of that record happens inside a scope derived from that context, and a record this instance produces carries context derived from the processing that caused it. The mechanism is the engine-side inject and extract of the parked note, whose argument is that the engine is the side that touches Kafka so doing it there gives every language binding propagation without each writing it; that reasoning is the note's and is cited rather than restated, as is the boundary it fixes, that context travels in record headers and never in a payload envelope. The declaration is how a user supplies the thing that does the propagating, and nothing more. It does not displace the record-tracing note, "Record-level semantic tracing: why did this record take 1.8 seconds?", which owns the per-record timeline of why a record waited and is a different instrument on the same tracer. What it must not do: no telemetry-vendor dependency in core; no trace context propagated onto the poll thread's own work, which belongs to no record and would attach every fetch to whichever trace happened to be last; and nothing that makes an absent tracer measurable on the record path. Milestone and timing: not Milestone A. The declaration alone is facade-only under R30's tiny tier, a value held and handed on, but the behaviour behind it is not this plan's to schedule; it is the OpenTelemetry work targeted at 0.6.1.0, which has no plan of its own and is blocked on a gap the parked note records, so it sits outside this plan's milestones. This plan is where the surface is specified and the implementation is owned elsewhere, under the same discipline R22 applies to the outcomes: the behaviour is fixed here and whoever implements it honours it unchanged. A declaration accepted and then ignored is worse than no declaration, because it reads as working, so the surface does not land ahead of the implementation merely to show the shape; if it is wanted visible earlier it lands refusing at definition time with a message naming what has not shipped, which is what R14 already does for an export destination under the transactional commit mode. That last is an argument offered, not a decision taken: the owner asked on 2026-09-12 for the declaration to be planned as the interface to the OpenTelemetry work, not for it to ship before that work. Whether this is one declaration or two was open when this requirement was drafted, and the naming settles it: `withMetrics` beside `withTracer` is two, which is how the libraries users compare us with keep metrics and tracing apart, and our metrics half was already an abstraction the user supplies rather than one half of a single observability setting (KD16). What stays open is named here rather than answered. Whether a route may declare its own, or whether a tracer stays instance-wide as the meter registry is, since KD11's per-route-value-with-an-instance-default shape does not reach settings that are properties of the clients rather than of the work. And what becomes of a parked record's trace context, since a park can outlive any span it would belong to and a record resumed a day later either rejoins a span nobody is holding or starts a trace linked to the old one; R27 makes this concrete, because a parked record may also be exported or resumed by an operator through R28 rather than by its own processing.
...
- R23. Routes do not compete for one shared limit: each route is bounded by its own admission target (R6), a route declared over a set of topics (R5) shares that one limit across them, and the engine's total admission is the sum of the route limits. The in-flight buffer and back-pressure remain the shared bounds. In the tiny tier the facade enforces each route's limit at the function boundary by handing a record past the limit back to the scheduler with a short delay, holding no thread and no lock; isolation is best-effort, since the scheduler is route-blind and a busy route's backlog still occupies the shared pool between hand-backs, and the documentation says so. The static guarantee lands in the small tier as deferral: a record for a route at its limit returns to the retry queue with a short delay and no attempt counted, and the delay is what stops a saturated route re-selecting the same records in a spin. Adaptive admission stays large with the self-scaling work.
  - **Withdrawn from Milestone A, and the surface with it (owner-directed, 2026-09-12: "Withdraw the per route concurrency from this milestone. Only do global for now.").** The route's own `concurrency(...)` declaration, `RouteView.concurrency()` and the summing of route limits into the engine's `maxConcurrency` are removed rather than deprecated, the fluent package never having shipped (KD15). `withDefaultConcurrency` remains and is the only way to bound concurrency in this milestone; it bounds the instance, and it keeps `default` in its name against this requirement returning, exactly as `withDefaultOrdering` does. **Why the tiny tier's hand-back arm does not work**: bounding a route in the facade means refusing a record the scheduler has already chosen, and the scheduler is route-blind - so a busy route keeps being handed records and handed them back, occupying the pool while the routes that are under their limits wait for it. That starves the other routes instead of bounding the busy one, which is the opposite of what this requirement asks for. A limit has to be applied where work is *selected*, which is inside the engine, and the note "Per-function capacity arbitration: the scaling unit is the function, not the application" already owns that shape - including the argument that the budget is measured rather than configured. R23 and AE17 therefore move out of Milestone A entirely; the tiering below and the U7 and U10 units that cover them are left as they were written, because this is a dated record, and this annotation is what supersedes them.

### Illustrative surface

Illustrative, not binding: the names are placeholders and the compiled README example decides the syntax (Outstanding Questions). What the examples fix is the shape the requirements imply: a definition from properties, one statement per route ending in `process` or a sink, policy as data, a handle out. Type declarations borrow Kafka Streams' `Consumed.with` and `Produced.with` shape and its `Serdes` names, in this library's own package so no Streams dependency arrives; the chain grammar of the Streams DSL is deliberately not borrowed across routes (KD3). The verb is not `stream`, because a stream in Streams is the start of a topology and this is a topic bound to one function. Format helpers such as `json(Order.class)` resolve to the deserialiser already on the classpath, and the format-named routes `json`, `avro`, `protobuf` and `bytes` are sugar for `topic(...).consumed(...)`. The document calls the binding a route, a topic bound to one function with its own policy; the API spells it `topic`, because that is the word a Kafka developer reads, with `topics(...)` for a set. In the same way the document calls copying a record to a dead-letter destination export, and the API spells it `dlq`, as a verb: `dlqTo`, `dlqImmediately`, `dlqOlderThan`, `dlqWhenOffsetPayloadReaches`, `Outcome.dlq`, and the parked set's `dlq` command (user-directed, 2026-09-10). The same verb defines one before start, adds one after start, and on the handle retrieves one. The one instance-wide setting, commit mode, is plain; per-route defaults carry the prefix `default`, and a route's own setting overrides its copy. **That last sentence, and every setting name in the examples below, predates KD16**, which was taken on 2026-09-12 and gives every setting on the definition a `with` prefix with the `default` family keeping its own inside the name; the mapping there is the one an implementer follows, and each spelling shown here was replaced outright rather than surviving as a deprecated delegate, KD15 having been corrected the same day to cover shipped members only.
...
- AE17. **Covers R6, R23.** Given two routes with concurrency limits of ten and one hundred, both with backlogs larger than their limits, when the instance runs, then at most ten records of the first and one hundred of the second are in flight at once, and draining the first route's backlog does not change the second's throughput; the throughput clause holds from the small tier (R23, R30).
  - **Not in Milestone A (owner-directed, 2026-09-12).** This example describes per-route behaviour the milestone no longer promises - see the annotation on R23 for why the facade cannot deliver it and where it belongs. Nothing in Milestone A asserts either clause, and the starvation test U7 was to have pinned is not written: what the milestone pins instead is that the engine's limit is the instance-wide setting whatever the routes are (`ClientConstructionTest.theEnginesAdmissionTargetIsTheInstanceWideSettingWhateverTheRoutes`, retargeted from the sum it used to assert).
...
| Trace-context propagation | In scope (R37), the declaration only | Asked for on 2026-09-12: 0.6.1.0 targets OpenTelemetry, so the tracer declaration is specified here beside the metrics one and named with it. The propagation behind the declaration is that work's, blocked on a gap its parked note records, and the per-record timeline stays with the record-tracing note |
...
- KTD7. **Format helpers resolve reflectively; only Jackson is declared, as optional.** Registry deserialisers are not on Maven Central, so `avro(...)` and `protobuf(...)` look up the deserialiser class by name on the user's classpath and refuse at definition time naming the missing library. The JSON helpers use Jackson through an optional dependency in core. Connection properties the facade owns (bootstrap, group, client serialisers) are consumed; every other key is passed to each route deserialiser's configure call unchanged. Governs U2.
  - **Each serialisation format gets a module of its own, and that is what replaces the optional dependency this decision describes (owner-directed, 2026-09-12: "We need to do module per format anyway. Can't do optional dependencies - they suck.").** An optional dependency compiles core against a library it does not ship, so the burden of working out what to add lands on every user who reaches for a format helper - and it lands as a missing class at the moment they run, not at the moment they chose the feature. The reflective lookup and the definition-time refusal above are the mitigation for that, and a mitigation is what they remain; they make the failure legible, they do not stop the user having to discover the coordinate. A module per format moves the fact into the coordinate instead: what you depend on is what says what you get, resolution brings the serialiser with it, and there is nothing left for the user to find out. **It does not land in this milestone.** Milestone A ships the format seam - `Format`, `Formats` and the helpers over them - and the split is later work. What that split has to preserve is also the reason it can be later: the surface a user writes against stays exactly where it is, so moving an implementation out of core is a dependency change for a caller and never a source change, and no definition written against Milestone A is rewritten by it. So the optional Jackson dependency U2 adds is correct as written and is the first candidate to move out, rather than a mistake to undo. **Two things the owner raised the same day are recorded here as open, not decided.** Schema-registry-backed formats as a target once the modules exist: a format whose serialiser talks to a registry is precisely the dependency core must not carry, so it is the case the split is for rather than an addition to it, and it is the same set as the registry deserialisers above that are not on Maven Central - which is why they are reached reflectively today. And a module for the API surface on its own, with JPMS module descriptors for the set; whether the API surface wants its own module is open independently of the formats. **JPMS is blocked today for a concrete reason, stated here so nobody re-derives it: Jabel compiles Java 17 source to Java 8 bytecode, and a `module-info.java` cannot ship into a Java 8 target.** Descriptors therefore wait on the Java baseline change (astubbs#53), the same thing KTD1 has the sandbox module waiting on. None of this changes the decision above or U2; core's dependency footprint is a published promise with a feature record of its own (`docs/features/core-dependency-footprint.yaml`), and a module per format is how the format work keeps it rather than a new constraint on it.
...
- KTD16. **Where a shape is genuinely open, take the one users already know.** The tie-breaker for any design question this document leaves open is what the stream-processing libraries users compare us with already do, because familiarity is the point of this work: a user arriving from one of them should find the shape they expect and have nothing to learn. It applies to naming, to the shape of the builder, to what is refused versus accepted, and to the settle-then-assert pattern of a broker-free test driver. It is a **tie-breaker, not a requirement** - it decides nothing a requirement above has already decided, and it does not license copying a shape that is wrong here. **Diverge where a real constraint forces it, and say what the constraint is at the point of divergence.** The worked example is the test driver: theirs processes a piped record synchronously and lets the caller assert on the next line, which it can do because that engine is single-threaded and the pipe call itself runs the topology. This one cannot - the whole point of this library is that records are processed concurrently on a worker pool - so a piped record is not finished when the pipe call returns, and the sandbox needs a settle between publishing and asserting. The divergence is forced by the concurrency, the settle is the smallest thing that answers it, and the rest of the pattern - publish, then assert on what came out, with no broker anywhere - is taken unchanged. **The cost of not doing this is not hypothetical**: on 2026-09-11 one word in the sandbox meant two different things, the thread that publishes records to a stopping rule and the machinery that fills the fake values, and an analysis asked to assess splitting "the generator" out assessed the wrong one and concluded it was impossible; the repair (`1ef1b86b5`) took the vocabulary the comparable libraries and this module already had - driver, hydration, feed - rather than inventing a fourth. Prefer their noun to a better-reading coinage of ours. (session-settled: owner-directed, 2026-09-11 - the owner's standing tie-breaker, recorded here without naming the library, which is this document's standing phrasing for it.) KTD15 is the worked precedent, having reached for exactly this argument before it was written down.
...
- **Approach:**
  ...
  5. Meters: outcome counters tagged topic and outcome; parked count and oldest age gauges tagged topic and partition; registered through the module and deregistered on close. The declaration that supplies the registry is spelled `withMetrics` (R19), with `meterRegistry` renamed rather than kept beside it as a deprecated delegate, since KD15's deprecation rule covers shipped members and this package has not shipped, and the setting family takes its `with` prefix in the same pass (KD16); the classic API's own setting is untouched.
