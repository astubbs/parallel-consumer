...
- R34. Changes to the classic API are limited to what is engine-level and non-breaking, and are these: the deserialisation-failure policy on the poll path (R25); the batch defects fixed (R32); and, once the engine owns park and export natively, a thrown terminal signal in the 2022 draft's shape (astubbs#8), so a classic user can park or export a record from the function they already have. Separate consume and produce types (astubbs#243) stay fluent-only, because on the classic interface they are a breaking change to its generics; the fluent API is the non-breaking route to that issue. Nothing else on the classic API changes.
...
|---|---|
| astubbs#243 | Separate consume and produce types: each route declares its own, produced types separately (R3); fluent-only, since on the classic interface it is breaking (R34) |
