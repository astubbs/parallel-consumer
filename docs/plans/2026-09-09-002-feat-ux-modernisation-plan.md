...
Retry and park policy, every call optional, as a per-route default on the definition or on one route (R10, R13, R27, R29). The export fraction has an instance-level default, seventy percent, itself configurable and refused at or above the engine's pause threshold:

```java
pc.defaultAfterRetries(park()
        .exportTo("orders.dlq")                      // optional; without it park is bounded by the map alone
        .exportAtOffsetMapFraction(0.5)              // optional; overrides the instance default of 0.7
        .exportOlderThan(Duration.ofDays(2)))        // optional; before retention wins
  .exportAtOffsetMapFraction(0.7)                    // the instance default, if you want it explicit; 0.75 and above is refused, the engine pauses there
