...
**And the number the operator would need does not hold still while they are not looking.** The capacity of whatever the user function reaches moves on its own schedule: hour to hour, degraded through an outage of a dependency, halved when somebody scales a downstream service down, or shifted by an interaction between several services that nobody has modelled. A value that was correct at deploy time is not correct at 3am, and nothing about the deployment changed.

**The leverage is that none of that has to be modelled.** The user function's measured execution time is already an encapsulation of the entire chain behind it, however complex - one service or six, a database and two APIs and a cache, healthy or degraded, in whatever combination. The controller does not need to know what the function does, what it depends on, or why it slowed down. It observes the single number all of that complexity already sums into, and adapts to it. Arbitrary downstream complexity is handled by declining to look at it - which is why the black box is a feature of this design rather than a limitation of it.
...
- KD1. **Control admission, not the pool.** The controller adjusts the in-flight admission target; the worker pool sits fixed at the effective maximum. One control variable that already feeds both dispatch quantity and poller backpressure. (session-settled: user-approved - chosen over runtime pool resizing: admission is the universal knob. Virtual threads are opt-in and resolved at runtime, so whether there is a resizable pool at all is not known when the controller is designed and can differ run to run on the same build; admission is the one control that behaves identically either way, without the controller having to ask which engine it woke up on. The tree already documents `maxConcurrency` as a target the control loop aims at rather than a cap the pool enforces.) Governs R2, R7.
