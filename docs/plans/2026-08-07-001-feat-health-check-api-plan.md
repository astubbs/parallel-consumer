...
- **Approach:**
  ...
  3. Override `getHealth()` in `AbstractParallelEoSStreamProcessor`. Read each field **exactly once into a local**, in the order `state`, then `failureReason`, then the poller state, and build `PCHealth` from those locals - so the snapshot cannot contradict itself by re-reading a field that changed mid-construction. The order is the reverse of the write order and is load-bearing: both failure paths write `failureReason` before transitioning the state, so reading the state first means the volatile happens-before edge guarantees the cause is visible. Reading the cause first admits a null cause paired with a `CLOSED` state - the shape of a clean shutdown, reported for a crash.
  3b. Reconcile the controller state against the control thread's own liveness (`areMyThreadsDone()`). The control task catches `Exception`, so an `Error` escaping it kills the thread without writing either field - leaving `RUNNING` with no cause. Without this the verdict is strictly weaker than `isClosedOrFailed()`, which already consults the thread.
