...
- **Narrate the current activity on the page itself.** Watching the demo is currently guesswork - the numbers move but nothing on screen says what is being done to the instance, so a viewer stares at pretty numbers inferring the plot. The text already exists and has nowhere to go: every `ScenarioPhase` carries a human description, the demo banner prints them as a running order, and `ScenarioRunner` logs each one as it begins.
  ...
  **Build the seam generic, not demo-specific.** The dashboard must not learn what a scenario is, or demo concepts leak into a library module. Instead give it an optional *current activity* annotation any embedder may set - a short label plus an optional longer line - which the state document carries and the page renders prominently. The demo sets it from the running phase's description; a real application can set "nightly reindex running" and give the same answer to whoever opens the dashboard mid-incident wondering why the shape changed. Same effort, and useful beyond the demo.
