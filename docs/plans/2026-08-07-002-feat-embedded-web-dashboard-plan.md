...
- KTD3. **Prefer the `MeterRegistry` where it suffices; add core accessors where it does not.** Micrometer is already `compile` scope in `parallel-consumer-core`, and `PCMetricsDef` already publishes every per-partition offset quantity R5 needs, tagged by topic and partition. Reading through the registry is the *default* because it is genuinely less coupling - the dashboard gains new meters for free as they land, without tracking field names - not because core is off limits.

  Where the registry cannot answer, add the accessor. Exact incomplete-offset *positions* (the scatter R48 renders), per-shard breakdown, `lastCommittedOffset`, the rebalance-in-progress flag and mailbox depth are all in-process state that is either private or aggregate-only in the meter set, and the honest answer is a small read-only accessor rather than a worse graphic. These are additive getters on existing classes; they change no behaviour.
...
- KTD19. **New numbers become meters in `PCMetricsDef`, not dashboard-private state - but not in this PR.** (session-settled: user-directed - chosen over letting the dashboard accumulate its own private measurements: an operator with Prometheus should see what the person with the page open sees, and a number that exists only inside a GUI is a number nobody can alert on.) The dashboard is a *renderer* of the metric surface, not a parallel one, and building this work is expected to reveal several quantities worth metering - encoding-budget statistics (R46) among the first.

  Two constraints shape how that lands:
  ...
  - **Cardinality.** Not everything the page shows should become an exported meter. Per-partition and per-shard labels are a well-known cardinality hazard in Prometheus, and the dashboard's real advantage is that being in-process it can render detail that would be irresponsible to export. The rule: aggregates and anything an operator would alert on become meters; genuinely high-cardinality per-partition or per-shard detail is read in-process and stays on the page, exportable only behind an explicit opt-in. Note PC already exports per-partition offset gauges, so this rule governs what is *added*, not what exists.
...
- `snapshot/MeterRegistrySampler.java`
