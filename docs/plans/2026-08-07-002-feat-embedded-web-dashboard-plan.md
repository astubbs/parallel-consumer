...
R50 (TLS) was in this list and has been deferred - see Scope Boundaries. The server speaks plain `http` and has no option for anything else.
...
- **Narrate the current activity on the page itself.** Watching the demo is currently guesswork - the numbers move but nothing on screen says what is being done to the instance, so a viewer stares at pretty numbers inferring the plot. The text already exists and has nowhere to go: every `ScenarioPhase` carries a human description, the demo banner prints them as a running order, and `ScenarioRunner` logs each one as it begins.

  **The dashboard may know it is running a demo, and should say so.** (session-settled: user-directed - chosen over an abstract "current activity" annotation that kept demo concepts out of the library module: the leak is not a cost here, it is the feature.) A visible demo banner is worth having on its own merits, separately from narration: anyone seeing a screenshot, the recording on the landing page, or a shared screen should know at a glance that the numbers are staged rather than production. That protects the honesty of every claim made with the artefact, which is worth more than module purity - and a generic annotation would have hidden exactly the fact worth stating.

  So: a demo-mode flag the page renders prominently, plus the running phase's description and its position in the running order ("phase 3 of 7: stranding work behind a failing key"). If a general-purpose annotation for real applications turns out to be wanted later, it can grow from this; do not build the abstraction first.

  Pairs with the pacing modes below: pacing controls how fast the story runs, narration says what the story is. Narration is the more valuable half - slow and unexplained is still unreadable.

- **Demo pacing modes.** Feedback from actually running Phase 1: the default pace is too fast to follow the first time - the interesting transitions happen quicker than a person can locate them on the page. Two levers already exist (workload publish rate, and the baseline user-function dwell U12b added so shard occupancy is observable at all), so this is tuning rather than new machinery. Add **modes to the one script, not a second script**: a second script drifts, stops receiving new phases, and ends up demonstrating something different six months later. Sketch: default keeps today's realistic pace; `--teach` slows the workload, pauses between phases and narrates what to watch for using the phase description the runner already logs; `--perf` runs flat out for the throughput story. Slower alone is not enough - an unexplained slow wall of moving numbers is still unreadable, and the narration is what makes a first viewing comprehensible.
...
- **R50. TLS, optional and off by default.** When enabled without a supplied certificate the server generates a self-signed one so it works with no setup; a user-supplied certificate and key are accepted. It is off by default, because a self-signed certificate makes a browser interrupt with a warning and that would spoil the zero-setup path the module exists for. Enabling it is a single option. The requirement keeps its number - it is deferred, not withdrawn, and the reasoning above is why it is still worth having.

  **Why it moved out of Phase 1** (session-settled: user-directed). It was built, tested and off by default, and then removed before release. Nobody can use it in the skateboard: the demo is loopback on a certificate the server signed for itself, which no browser accepts without a click-through, so the feature's entire effect on the demo path is an interstitial. Against that it cost an optional dependency, five tests, a branch in `DashboardServer.start()`, three fields on `DashboardOptions`, and a second possible value for the scheme that three separate places had to agree on. Removing it collapsed the scheme to the constant `DashboardOptions.SCHEME`.

  **The finding that must survive, because reviving TLS will otherwise rediscover it the hard way.** Vert.x's `io.vertx.core.net.SelfSignedCertificate` **does not work on Java 16 or later**. It delegates to Netty, whose fallback generator reflects into `sun.security.x509` - a package `java.base` has never exported, and which stopped being reachable by illegal access when Java 16 flipped the default to deny. The failure surfaces as a `CertificateException` reading "no provider succeeded", which says nothing about the cause. There is **no pure-JDK alternative**: the JDK has no public API for building an X.509 certificate at all. So generating a certificate at startup requires BouncyCastle (`org.bouncycastle:bcpkix-jdk18on`, which Netty picks up off the classpath), and the only other route is making the user supply a certificate and key - which is no longer a zero-setup path, and therefore no longer the thing R50 was asking for. Reviving R50 means choosing one of those two costs deliberately; it is not the "couple of Vert.x options" KTD9 originally assumed.
...
- KTD9. **Ship safe defaults, not security features; loopback by default and validate the `Host` header unconditionally.** The feature side is settled and out (see Scope Boundaries): no auth, no TLS, no roles. This decision covers only what is left, which is cheap and which the module would be irresponsible without. Binding loopback does not stop DNS rebinding - the attacker's page is served from their domain, DNS is rebound to `127.0.0.1`, and the browser issues requests to the loopback server from the attacker's origin. This is current, not theoretical: CVE-2024-28224 against Ollama, CVE-2025-66414 against the MCP TypeScript SDK. The cited mitigation is a `Host` allowlist, so that is what ships, extensible for port-forwarding. Bind via `InetAddress.getLoopbackAddress()`, which is correct for IPv4 and IPv6 and performs no DNS lookup - not `getLocalHost()`, which is Pekko's mistake of a default that is sometimes loopback and sometimes routable. Rejected: Kafka Connect's posture of all-interfaces and unauthenticated by default.
  ...
  **TLS was assumed cheap here and is not** (R50, now deferred - see Scope Boundaries for the full finding). The original reading was that Vert.x makes it a couple of options and can generate a self-signed certificate, so enabling it needs no preparation. That is false on Java 16 and later: `io.vertx.core.net.SelfSignedCertificate` reflects into `sun.security.x509` through Netty, `java.base` does not export it, and the JDK offers no public API for building an X.509 certificate at all - so generation needs BouncyCastle on the classpath or it needs the user to supply a certificate. Either way it is not free and it is not zero-setup. The `Host` allowlist below therefore carries the rebinding defence on its own, which is what KTD9 asks of it anyway; TLS was only ever a stronger second answer to it, never the shipped one.
...
**Requirements:** R3, R4, R13, R20, R21, R22, R23, R24, R25, R26, R27, R41, R42
...
8b. **Plain `http`, and only `http`** (R50 deferred). The scheme is a constant, `DashboardOptions.SCHEME`, not a derived value - the startup URL, the `/status` page and `HostAllowlist`'s same-origin comparison all read it, and a same-origin check that disagreed with the scheme actually served would reject the page's own requests. Do not reintroduce a second possible value without reading the R50 entry in Scope Boundaries first.
...
- The server answers plain HTTP, the startup URL and the `/status` page both say `http`, and an `https` `Origin` against the same host and port is not same-origin.
