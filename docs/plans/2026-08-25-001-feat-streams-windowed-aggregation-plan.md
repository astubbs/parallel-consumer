...
- **Stop conditions.** Four outcomes are pre-registered, and only the fourth - the bet is off - is direction-closing:
...
- **Exit criteria - the verdict is absolute, the ratios are the attribution:**
  ...
  - **Marginal:** the best arm sits between F2 and F1. Nothing stops - windowed aggregation is offerable only with the placement and the rate stated up front. Hopping-at-the-aggregator conclusions stop only when the B-against-D attribution below confirms the multiplier reaches the crossing, which is an **independent** test: where the rate lands and whether the multiplier reaches the crossing are separate questions, and fusing them was an error.
