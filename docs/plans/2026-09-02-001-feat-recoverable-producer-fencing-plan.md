...
- **"PC has no recovery path from a poisoned transaction short of `close()`"** - settled from the code in the note `bug-wedged-after-poisoned-transaction` (retired by the 2026-09-08 grooming sweep, astubbs#476, which folded it into its sibling; the version this plan read is `git show 6aab3ff5a^:docs/inflight/bug-wedged-after-poisoned-transaction.md`), with the design decision left open. This plan is that decision for every condition in R8: abort, replace, replay. A transaction poisoned by a cause outside R8 (a `RecordTooLargeException`, say) stays where astubbs#262 left it; the recovery machinery built here is the obvious home for it, and `docs/inflight/bug-poisoned-transaction-not-aborted-while-running.md` now points back here for that follow-up.
