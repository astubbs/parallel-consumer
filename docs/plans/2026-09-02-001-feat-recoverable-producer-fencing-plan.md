...
- **"PC has no recovery path from a poisoned transaction short of `close()`"** - settled from the code in the note this plan read as `docs/inflight/bug-wedged-after-poisoned-transaction.md`, with the design decision left open. That note was folded into `docs/inflight/bug-poisoned-transaction-not-aborted-while-running.md` and removed on 2026-09-08; read it as the plan did with `git show 6aab3ff5a^:docs/inflight/bug-wedged-after-poisoned-transaction.md`, grepping for "is wedged for the life of the instance". This plan is that decision for every condition in R8: abort, replace, replay. A transaction poisoned by a cause outside R8 (a `RecordTooLargeException`, say) stays where astubbs#262 left it; the recovery machinery built here is the obvious home for it, and `docs/inflight/bug-poisoned-transaction-not-aborted-while-running.md` now points back here for that follow-up.
