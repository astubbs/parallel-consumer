...
> **Done (landed in PR astubbs#80).** The drain-path zombie/busy-spin fix shipped, guarded by
...
The PR astubbs#80 investigation root-caused a drain-path defect: `BrokerPollSystem.drain()` calls
...
confluentinc#857's "paused consumption after rebalance". Verified: neither PR astubbs#29 (confluentinc#857) nor PR astubbs#31 (confluentinc#909)
fixes this — they fix sibling, non-conflicting mechanisms.

User-directed sequencing: **(A)** land focused test coverage of the theory on the PR astubbs#80 branch → commit;
**(B)** land the minimal fix → commit; **(C)** build the uber branch(es) merging astubbs#29 + astubbs#31 and measure
...
(PR astubbs#80, stacked on astubbs#75), worktree `.claude/worktrees/fix-flaky-partitionstate`.
...
- R3. Uber experiment: A/B-measure the marginal effect of the drain fix with astubbs#29 + astubbs#31 merged in, using
  (a) astubbs#29's chaos harness `MultiInstanceRebalanceTest#largeNumberOfInstances` (their residual: 10-20%
...
- NOT touching `PartitionStateCommittedOffsetIT`'s await (no masking — unchanged from PR astubbs#80's stance).
...
- NOT attempting the deeper fail-safe drain options (hard deadline → explicit LeaveGroup; eager leave) —
  documented in the report's design review, deferred to the confluentinc#857 stream.
- NOT fixing astubbs#29's pom conflict "properly" — resolve minimally for the experiment only.
...
- Merge dry-runs (verified with `git merge-tree --write-tree`):
  - astubbs#29 (`bugs/857-paused-consumption-multi-consumers-bug`) into PR astubbs#80 branch → **1 conflict: `pom.xml`
    only** (confluentinc#857 adds ~7 lines of test deps).
  - astubbs#31 (`fix/909-stale-container-replacement`) → **clean**.
- Chaos harness (on astubbs#29's branch; `@Disabled` on master): `MultiInstanceRebalanceTest#largeNumberOfInstances`
  ...
  astubbs#29's own acceptance note: ~90% pass, residual ~10% attributed to "Kafka behaviour under churn" — our
...
- **Fix depth (user-selected): collapse the duplicated lifecycle state, not just re-time it.**
  ...
  *class* becomes structurally impossible, per the "collapse parallel state when bugs recur" rule. The
  ...
  the confluentinc#857 stream; the report's design review gains it explicitly as the end-state option.
...
- **A/B arms isolate the drain fix's marginal effect.** Both arms = current PR astubbs#80 base + astubbs#29 + astubbs#31
  merged; arms differ *only* by the drain-fix commit (branch off `HEAD` vs `HEAD~1`... concretely: off the
  fix commit vs its parent). astubbs#29's 10-20% residual claim becomes measurable against an identical base.
...
**Verification:** new test green on current code; full unit suite green; commit + push (updates PR astubbs#80).
...
**Verification:** RED→GREEN demonstrated; suites green; commit + push (updates PR astubbs#80).
...
- Compare arms. Prediction on record: if zombie-drain explains astubbs#29's residual, `uber-fix` chaos failures
...
**Verification:** results section complete with raw counts; committed + pushed to PR astubbs#80; uber worktrees
...
- **Unchanged invariants:** `DONT_DRAIN` close path (calls `transitionToClosing()` directly) unaffected;
  `PartitionStateCommittedOffsetIT` untouched; PR astubbs#75 workflow untouched; diagnostics from PR astubbs#80 remain.
- **Interaction risk:** rebalance-during-drain now *executes* revoke callbacks (previously the member was
  protocol-absent) — this is the desired behaviour but touches astubbs#29's CME/commit-lock territory; the uber
  arms (which include astubbs#29's fixes) are exactly where that composition gets exercised.
...
| Chaos test won't run locally (perf-tagged, resource-heavy) | Verified tag/exclusion mechanism; 12-core + Docker matches astubbs#29's own dev runs; fall back to fewer instances param variant if the box can't sustain it |
...
| Uber merges pull in astubbs#29's test changes that fail on new base | Arms only need compile + the two experiment workloads green-ish; unrelated astubbs#29 test failures recorded, not fixed |
...
3. Units 3-4: both arms built; 5+3 runs per arm executed; results table in the report answers the
   prediction; PR astubbs#80 updated with all commits.
