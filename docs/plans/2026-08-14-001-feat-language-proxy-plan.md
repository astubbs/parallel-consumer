...
- **A true in-process C-ABI binding - re-scoped 2026-08-17, gated on a qualification probe.** The shared-library dead end below is superseded on direction: the owner has now asked for the exploration, and the ideation continuation (`docs/ideation/2026-08-14-language-proxy-interaction-model-ideation.html`, the `native-bindings` section) worked it through. Its conclusion narrows rather than widens this plan: the FFI track's constituency is Rust, C++ and edge targets with no process to spare; Go, Python, Ruby, Node and .NET are better served by the sidecar for reasons a better FFI design does not remove (cgo forfeits Go's static builds, fork() kills isolate threads under preforking servers, Node addons rebuild per ABI version, and two garbage collectors share one address space). Nothing enters this plan's units: the track starts, if ever, with a throwaway `--shared` qualification probe in the U1/U26 tradition against the recorded unknowns, its kill criterion written first, and its Rust experiment runs against the invisible-sidecar control arm KTD41 ships.
...
**Superseded 2026-08-17, grounds preserved:** the direction half of this rejection dissolved when the owner proposed the direction; the untested-caveats half stands and became the qualification probe's checklist. The re-scoped track - Rust, C++ and edge targets only, behind that probe - is recorded under Deferred for later above, with the full analysis in `docs/ideation/2026-08-14-language-proxy-interaction-model-ideation.html`.
...
- KTD11. **Loopback bind, authority allowlist, and no listener authentication in v1.** The proxy binds `InetAddress.getLoopbackAddress()` explicitly rather than the wildcard address, and a `ServerInterceptor` reads `ServerCall.getAuthority()` and closes an unlisted authority with `PERMISSION_DENIED` before the service method runs. This is the cleared gate's mechanism, proven by unchanged service-invocation and application-message counters. Unix domain sockets would close the peer-identity gap and are not adopted in v1 - but not for the reason previously recorded here. `grpc-netty-shaded` 1.73.0 **does** bundle the Linux epoll transport - `EpollServerDomainSocketChannel` and the x86_64/aarch64 native libraries are in the shaded jar - so UDS is reachable on the cleared stack on Linux with no switch to unshaded `grpc-netty`. What the jar does not bundle is a kqueue transport, so macOS would lose parity; that and the v1 no-authentication scope boundary are the grounds the decision actually stands on, not transport unavailability.
  ...
  **Residual risk, recorded in full and deferred to post-v6.** Security work on this seam is deliberately not scoped into this plan. It is written out here rather than summarised, because R35's reversal (see the preamble) rests on an argument that does not hold, and a reader in six months must be able to see that rather than infer it.
  ...
  - **Resolved by decision (2026-08-17), implementation owed:** how a client locates the sidecar binary was unspecified here - an easier attack than any socket race, since a path or `PATH` lookup is attacker-influenceable, and under KTD4 ten authors would each have decided it independently. KTD41 now decides it once: the package vendors the binary and spawns exactly what it shipped, so no lookup exists to poison. The client-authoring guide still owes the rule's wording, and KTD41's attach mode deliberately reopens the question for the deployments that choose it - an attached address is trusted input, and saying so belongs in the same guide.
...
- KTD41. **The client package vendors the sidecar binary; spawning it is invisible, and attaching is the escape hatch.** (session-settled: user-directed, 2026-08-17 - the spawn-as-child model of KTD4/R52 was confirmed as the product's original intent, "the invisible sidecar", and the binary-location question left open under KTD11 is decided by vendoring.) The default in every language package is invisible: the package carries the platform-matched sidecar binary as a resource - the shape confluent-kafka-python and puppeteer normalised - and the client library spawns and supervises it on first use, so the user never installs, deploys or operates a process. Vendoring is also the answer to KTD11's binary-location gap: there is no path or `PATH` lookup for an attacker to influence, and the engine version is pinned to the package version so skew is impossible - the accepted trade is that an engine patch rides the release cycle, which KTD36's lockstep release already implies. The visible mode is a constructor-supplied address and nothing else: same binary, same protocol, the library skips the spawn (the Dapr platform-injected/self-spawned pair and librdkafka's bundled-vs-system choice are the precedent). It exists for process models the 1:1 spawn cannot serve - a preforking gunicorn or Puma wants one shared per-pod sidecar, not one child per worker each holding a group membership - and for platforms that forbid executing a package-vendored binary. Dialects follow the mode through capability negotiation, with no schema change: a spawned local child negotiates no reconnect or manifest capabilities, because over a local socket to a supervised child connection death is process death and the KTD19 pipe covers lifetime in both directions; an attached sidecar that restarts independently of the app keeps them. Full reasoning, the per-language analysis, and the visible-vs-invisible trade table: `docs/ideation/2026-08-14-language-proxy-interaction-model-ideation.html`, the `native-bindings` continuation, idea 11. Governs R52; shapes U11, U12, U22 and U32.
...
- **Approach:**
  ...
  6. **Vendor the sidecar binary in every package, per KTD41.** Each package carries the platform-matched binary (platform wheels, NuGet `runtimes/`, the Go embedding shape decided in U12), and step 5's clean-environment check must therefore also prove the vendored binary spawns on that platform - an install that succeeds while its binary is missing or non-executable is this unit's version of the silent-skip defect. The attach mode ships alongside as a documented constructor option, exercised by at least one smoke run.
