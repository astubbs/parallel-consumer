...
| Is an `INSTANCE_STALL` seed that replays clean on an idle box a WEDGE under CPU load? | `bin/exp-instance-stall-load-versus-idle.sh <seed>...` | **answered 2026-09-08 for `ChaosChurnStormIT` - no.** Six runs, three seeds, two load levels: zero `INSTANCE_STALL` dumps and zero violations, every accused member busy in user code. **Still open: the dose-response**, which needs a quiet machine this run did not have - so the runner stays. `docs/inflight/bug-857-family.md`, "2026-09-08: the load arm" | anywhere, but a dose-response arm needs an otherwise idle box |
