...
| Does an `INSTANCE_STALL` seed that replays clean on an idle box reproduce under CPU load? | `bin/exp-instance-stall-load-versus-idle.sh <seed>...` | **answered 2026-09-08 for `ChaosChurnStormIT`** - the loaded arm reproduces the firing as the non-gating `INSTANCE_BUSY_IN_USER_CODE`, never the violation, which is starvation and not a wedge; kept because it is the arm any *future* "clean on idle" claim on this family owes, and the runner is generic over seeds. `docs/inflight/bug-857-family.md`, "2026-09-08: the load arm" | anywhere |
