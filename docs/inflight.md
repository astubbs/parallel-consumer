...
>
> **In-code `TODO`/`FIXME`/`XXX` markers live in two companion files:**
> [`docs/TODO_INDEX.md`](TODO_INDEX.md) - generated by `bin/todo-index.sh` (`--check` fails when
> stale), the raw what/where - and [`docs/TODO_TRIAGE.md`](TODO_TRIAGE.md) - hand-curated groups in
> priority order, naming themes rather than line numbers so index rebuilds never invalidate it.
> Entries here should *link* to those rather than copy marker text, which goes stale. Rule of thumb:
> work someone should schedule belongs here; context for whoever next edits a line belongs in the
> code, where the index surfaces it. The triage's P1 groups are the current best answer to "what in
> here is actually worth doing".
