> **Salvaged 2026-08-08 - unmodified below this line.**
>
> This document existed **only** on the unmerged branch `origin/refactor/test-hardening`, recovered
> with `git show 16ce9727:docs/test-hardening/large-volume-in-memory-tests-oom-diagnostics-2026-04-22.md`.
> That branch is on the safe-to-delete list, and this was the **only copy** of the measurements
> showing that restoring `LargeVolumeInMemoryTests` to 1,000,000 messages produces
> `java.lang.OutOfMemoryError: Java heap space` in the close path. Deleting the branch would have
> deleted the evidence, and the next person to try the "one-line" restore would have rediscovered
> the OOM from scratch.
>
> It is reproduced **verbatim**, including its own dating, branch references and open questions, so
> it can be trusted as a primary record rather than a paraphrase. Read its `## Related` section with
> that in mind: it refers to branch state as of 2026-04-22, and the predecessor audit it cites
> (`disabled-and-weakened-tests-audit-2026-04-22.md`) is superseded by
