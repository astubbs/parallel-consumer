...
changes it forced were merged onto `research/market-analysis-recut` as a measurement, with no
...
1. **`if (isOrderRestricted()) break;` sits outside the take/skip branch.** A thread that finds the
   ...
   itself.
2. **The claim decides, not the availability check** - `isAvailableToTakeAsWork() && onQueueingForExecution()`.
