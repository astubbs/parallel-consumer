# A machine-readable index of the product's features

Renamed twice, because both earlier names led with the wrong noun and each time the ideation inherited
the error. It was `next-per-pr-docs-and-feature-index.md`, which put the PR first when a PR is merely
when a page gets written. Then `next-feature-docs-index.md`, which reads as an index *of feature docs*
when the thing is an index *of features* that happens to carry documentation.

**Starting now.**

## What it is, and who it is for

A permanent, machine-readable index of what the product does. One file per feature. It carries enough
...
**The index lives as long as the product does.**

## The three properties that make it worth building

**Backfilled.** The existing feature surface has no entries, and an index covering only what lands next
...
- **The front matter schema**, field by field, and how a feature's identity is expressed so an entry can
  be bound to the code it describes. Binding both directions is what catches an entry that rotted after
...
- **Whether a committed index file is safe.** `docs/inflight/AGENTS.md` forbids one because every PR
  ...
  drift; otherwise `ls` is the index.
...
This note goes when the index exists, the backfill baseline is empty, the schema is settled and
checked, and the README duplication has ended at cutover. The index itself is never deleted.
