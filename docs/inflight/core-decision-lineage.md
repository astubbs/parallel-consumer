...
decision explanation ships with the scheduler; durable audit lineage is opt-in and independently
budgeted, with bytes-per-mutation and query cost measured before it is called a projection. With
that split: the scheduler must know the answer to make the decision - the product is retaining
