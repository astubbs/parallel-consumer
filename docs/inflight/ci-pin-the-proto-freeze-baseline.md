...
- **Before the schema reaches master it is unarmed.** The gate passes with *"nothing frozen to
  compare against"* until the schema lands on `master`, which is correct behaviour and the reason the
  gate's own self-test has a case for it - but it means the freeze is not load-bearing on the PR that
  brings the schema, only on every PR after it.
...
- **Pin the baseline** to the freeze commit rather than `origin/master`. The freeze commit on
  astubbs/parallel-consumer#293 is `c237940081620ab000d5e06edafa94c769903007`, identified while
  confirming the schema was unchanged apart from the six per-language file options landed
  deliberately before the gate armed. **That commit is not on `master`** and will not be: the schema
  arrives on `master` through the extraction rung, so the pin has to name a commit reachable from
  `master`, and picking it is part of doing this work rather than a detail settled in advance.
...
- **This also arms the gate immediately** instead of waiting for a merge - which is the more valuable
  half, since the schema is being changed *now* and protected *later*.
