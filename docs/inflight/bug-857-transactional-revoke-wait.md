...
<!-- inflight-vetted: 2026-09-08 - PROPOSED re-premise: the sweep read this on 2026-09-07 against a tree where `onPartitionsRevoked` still spun on `isTransactionCommittingInProgress()` with no deadline; astubbs#466 merged the same day and removed that spin - in transactional mode the callback now posts to the control thread and waits bounded by `commitLockAcquisitionTimeout` (`commitOnRevokeViaTheControlThread`), so the unbounded wait this note is about no longer exists in the code as written. What remains is what the note's own last section already says: whether that bound (the five-minute default) is the right one against confluentinc#803's complaint about burning `max.poll.interval.ms`, and astubbs#408's amendment. Four stale citations still stand from the 2026-09-07 read: "no open PR addresses it" and the no-code branch claim are false (astubbs#408 on `fix/803-bound-transactional-revoke-wait`); `next-recoverable-producer-fencing.md` is now `core-recoverable-producer-fencing.md`; the two file:line citations are the form the root AGENTS.md forbids; "do not write code before settling it" contradicts the open draft -->
