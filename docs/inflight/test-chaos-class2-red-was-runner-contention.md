...
**One criterion is still open, and it is the only reason this file exists.** Confirm the
co-residency rate is zero after a full week of chaos runs on the serialised lane, then delete this
note - the diagnosis and the decision are recorded in
[`bug-857-family.md`](bug-857-family.md)'s 2026-08-25 entry and in
[`pr-highcpu-fast-feedback.yml`](../../.github/workflows/pr-highcpu-fast-feedback.yml)'s own
concurrency comment, so nothing is lost with it.

The original criterion - "the contention is fixed or the bound is widened, and the finding is folded
into the chaos ledger" - is met on both halves: the lane is serialised, and the finding is folded in.
What remains is the measurement that the fix worked.
