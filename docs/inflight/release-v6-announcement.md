# v6 release: the theme, and the announcement
...
and engine-performance stacks, none of which merges before v6 - so the plan for the release's own
announcement was invisible from master. The content is unchanged apart from punctuation and the tags
...
- **The project is back** - in a new form, better than before: a fork of a no-longer-maintained
  upstream, brought back with new features rather than merely kept alive. In the owner's framing it
  is personal as well as technical, about himself as much as the codebase.
- **Reaching other runtimes** - the multi-language client libraries. What was a JVM-only library
  reaching other runtimes.
...
| J5 | The release theme | no | yes | The blog's frame. Too much for a reply |
