...
- **The rest of astubbs#197's triage list.** Four non-blocking defects were found while checking the
  two blocking ones; **two are still open**: `PCModule` builds `DynamicLoadFactor(static, static)`
  when `messageBufferSize` is set, so "Max loading factor steps reached" WARNs on every control-loop
  pass for anyone following the README's own tuning advice (astubbs#155); and MDC context is not
  captured at submit time, so a caller's `trace_id` is lost into the worker pool and the vert.x event
  loop. The other two are fixed: an `OffsetEncoding` magic-byte hazard in astubbs#217, and
  `release.yml` publishing an empty GitHub Release body - so the curated changelog never reached the
  release page - in astubbs#199, which replaced the inline `awk`/`sed` converter with
