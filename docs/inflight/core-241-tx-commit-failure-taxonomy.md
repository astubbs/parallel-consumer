...
branch both reconciled onto it. astubbs#257 does not go near `commitOffsets`. None of them changes the
commit retry loop, so a taxonomy change conflicts textually at worst. What still binds is the ORDER:
