...
`debug/chaos-w4-red-commit-response-stall` (#80 and #100, both with write-ups in `docs/solutions/`).

**Superseded:** `cherry-pick/893-offset-reset`, `cherry-pick/905-max-shard-metric`, `upstream-pr-893`,
`upstream-pr-905`, `pr-909-temp`, `bugs/859-pcmetrics-leak-v2` - all folded into #57. Plus
...
is the base of #29 and #31 - retarget those first. `dev-cc` is pinned at the same commit.
