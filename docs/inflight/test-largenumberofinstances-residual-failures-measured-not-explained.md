...
<!-- post-merge: checked-begin - dated records of measurements taken against astubbs/parallel-consumer#431's branch
     while it was open; every mention names that PR as the tree an arm ran on, and reads as history once it lands -->
...
### The reproducer is proven red, not merely green

Sabotaged per `docs/testing-at-write-time.md`: a 20s sleep at the top of `BrokerPollSystem#doClose()`
holds the member not-polling and not-left, which is the defect shape itself. Every joiner then closed
in 20.0s against the 10s bound. That check matters more here than usual, because every property this
test asserts already holds on master - so a green run says nothing about whether the test can see the
failure, and without the sabotage it would be an assumed test wearing a measurement's clothes.
