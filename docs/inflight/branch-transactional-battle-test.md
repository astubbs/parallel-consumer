...
|---|---|---|
| astubbs#44 / confluentinc#803 | open | **Not tested here.** Carries upstream's *verified bug* label (one of a couple of dozen): a transactional instance times out getting the commit lock when a second instance starts. Its fix is astubbs#29, still unmerged - but retargeted since this map was written, so it now sits on `master` and is ready rather than draft. This is the one hang the battle test planned around and deliberately did not reproduce - the chaos scenario that would (Phase B) is deferred behind astubbs#29, because calibrating its SLOs against a master that still deadlocks would fold the defect into the baseline. |
...
| astubbs#177 / confluentinc#833 | **mirror closed, upstream still open** | Its own forensics - ~17 minutes with no commit response produced, processed-records flat - fit the broker-poll-thread death astubbs#100 fixed far better than the 30s timeout its message names. The mirror was closed by the fork owner on 2026-09-01 as completed, with no reason recorded on the issue and no linking commit; confluentinc#833 itself is still open and still unanswered. So the assessment stands and the *telling* did not happen - see `next-tell-809-833-they-are-likely-fixed.md`, which is now the only place that state is written down. |
