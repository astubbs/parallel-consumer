...
Say that outright. "Pre-1.0 refers to the API surface, not to reliability" is a defensible, checkable
sentence; a vague "production ready!" is not, and a bare `<1.0` badge understates the truth.
...
The table is published in the README template and the pre-1.0 wording is corrected.
