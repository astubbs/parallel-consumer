...
| `CE_CLASS_ENVY`, `SPP_FIELD_COULD_BE_STATIC`, `HES_EXECUTOR_OVERWRITTEN_WITHOUT_SHUTDOWN`, `OCP_OVERLY_CONCRETE_PARAMETER` in `bz.stub.parallelconsumer.client.*` | 4 | `new` | These tables were measured on a tree without the proxy client modules. All four fire on deliberate design there, and **two of the offered fixes would introduce bugs**: `dispatchQueueDepth` is negotiated per session, so making it static shares one client's concurrency with every other in the JVM; the overwritten executor is assigned exactly once, a second `poll()` having already thrown. **Scoped to the package, not switched off by rule.** Reasoning per pattern is in `config/spotbugs-exclude.xml`. |
