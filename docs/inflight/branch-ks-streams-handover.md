...
1. **`WALL_CLOCK_TIME` punctuators fire unwarned**, where `STREAM_TIME` logs. Pre-existing rather than introduced by U13, and now the whole of what was defect 1 - a missing warning, not data loss. U10's territory.

   **The data-loss reading is measured and does not hold.** *Do punctuator effects survive a crash?* **Yes.** `feats/ks-streams-punctuator-effect-survival` punctuates three times with the commit interval at Kafka's 30s default, aborts via `abortAllActive()` ~600ms in so no commit is possible in the window, and finds all three forwards on the output topic and all three store writes on the changelog. The producer carries them without any `flush()`. **Caveat:** non-EOS only - under exactly-once the forward would sit in an open transaction, which is the one configuration where the concern bites; EOS is refused (U11) and out of scope (KTD7), so a future EOS decision inherits the question rather than this answer.
