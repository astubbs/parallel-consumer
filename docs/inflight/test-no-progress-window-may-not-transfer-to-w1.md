...
## The evidence, which is not mine - and there are TWO occurrences, not one

**Both are `ChaosChurnStormIT.churnStormMeetsSlosAndBalancesLedger`, both at exactly the bound, both
in the tail.** They were recorded independently, by different pieces of work, neither looking for
this:
...
The second one makes this actionable: **the settling experiment below is runnable today**, because
that seed exists.

Neither source was examining the calibration. astubbs#348's note is about a **reporting** defect - the
ambient autopsy printed `violations (0)` for its run, because fleet-scoped detectors cannot be
re-derived by the ambient probe, which has no consumed-count supplier. The fourteenth sighting records
its occurrence as one arm among several on a branch it was clearing of suspicion. This note exists so
the calibration question is not lost between the two.
