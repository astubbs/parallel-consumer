...
| `AmbientProbeExtensionTest.headroomIsReportedOnAPassingTestToo` and `.headroomOutcomeComesFromTheWatcherPhaseNotTheEndOfTheTestMethod`; a third method, `.headroomIsSilentWithoutADeadlineAndWithoutAMeasurement`, by the same mechanism - it captured another method's `PC-DEADLINE-HEADROOM ... test=mockedTest()` line | 2/2 isolated runs, 1 seen in a full core run (2026-09-02, local, astubbs#116); independently 4 of 5 local runs the same day on astubbs#225, where the third method fired too; three further local full runs on 2026-09-03, two on the branch behind astubbs#434 and one on its unmodified base, each failed a different subset, which fits; the third method 1 seen in 2 full core runs, and the original two 1 more full-run sighting (2026-09-07, local, the revoke-drain fix branch, which does not touch the probe) | **DIAGNOSED, and not a product defect** - two methods of one class each capture the *same process-global* logger with `LogCapture.of(AmbientProbeExtension.class)` while the suite runs them concurrently, so each sees the other's headroom line and the `hasSize(1)` assertion gets 2. Reproduces on demand - see below | <!-- post-merge: checked -->
| `LoadFactorCeilingReportingTest.fixedMessageBufferSizeDoesNotWarnOnEveryPass` | 6 seen (2026-09-02, and five times 2026-09-03, all local full runs) | Its WARN capture on the processor's shared logger caught a user-function failure line from an instance an earlier class left running; passes alone and beside `UserFunctionFailureLoggingTest`. Same shape as the `AmbientProbeExtensionTest` row - see below |
...
| `Chaos Pain Suite 1/4` lane - `Could not transfer artifact org.apache.maven.surefire:surefire-junit-platform:pom:3.5.6 from/to central ... Connection reset` | 1 seen (2026-09-07, astubbs/parallel-consumer#434, [job 101615543093](https://github.com/astubbs/parallel-consumer/actions/runs/33988636371/job/101615543093)) | Not a test - failsafe could not resolve its own JUnit provider from Maven Central, so the LANE failed with `No tests to run` before any scenario started. Same class as the row above (a lane dying on a download, not on the tree), a different artifact and a different repository. The other three chaos shards on the same head resolved it fine, so it is the connection, not the coordinates. Evidence for the parked pre-warm-the-Maven-cache question rather than for anything this branch changes <!-- post-merge: checked - a dated sighting on a named job --> |
...
**A third sighting, 2026-09-03**, `inFlightMessagesCommittedIfProcessedDuringShutdown(CommitMode)[3]` alone, in a
local full core run on the branch behind astubbs/parallel-consumer#434 - committed `[]` where `[1]` was expected,
0 of 3 on isolated re-run. The `[3]` arm is the transactional commit mode, so it was checked against that branch's
change before being counted here: the test never produces, and its base supplies a `Producer` instance
(`AbstractParallelEoSStreamProcessorTestBase` calls `.producer(producerSpy)`), which makes `canRecover()` false, so
the new send-callback path cannot execute in it. Same shape as the 2026-08-13 sighting above: a shutdown commit
missing its window, under full-suite load, on a head whose engine change is provably not on the path.
...
**Independently sighted the same day on astubbs#225, before this diagnosis existed, and it corroborates it.**
Four of five local runs failed - the full suite, three classes together, the class alone, and a control arm on
a detached worktree at the branch's merge commit, which settled master-state without a rate. The count varied
between 2 and 3 with method order, and the third method, `headroomIsSilentWithoutADeadlineAndWithoutAMeasurement`,
fired as well as the two named above - so the shared-logger reading covers all three, and the order of the
methods decides which capture sees the stray line. That entry's own guess was "an appender that outlives its
method, or two methods sharing the logger without a `@ResourceLock`", which is the second of the two.
...
<!-- post-merge: checked-begin - a dated sighting against a PR number, past tense, stays true after the merge -->
**Sighting of 2026-09-05, on astubbs/parallel-consumer#452's Integration Tests lane**
([run 33938334747](https://github.com/astubbs/parallel-consumer/actions/runs/33938334747),
[job 101230691107](https://github.com/astubbs/parallel-consumer/actions/runs/33938334747/job/101230691107)),
at head `9071c9baa`. Same assertion - `control thread must reach the mid-loop pause point (offset 25)`,
the `pausableSm.get().awaitPausePoint(30, SECONDS)` guard in
`parallel-consumer-core/src/test-integration/java/bz/stub/parallelconsumer/integrationTests/RegistrationRaceStaleResidentIT.java` -
after 30.94s against the 30s guard, 1 failure in 201 integration tests, the ambient probe clean (no rebalance dwell, no lag
stagnation, no frozen partitions). The diff touches none of the test's own files: its one main-code
change is the map type of `OffsetMapCodecManager.encodingCounters` and the single `computeIfAbsent`
that populates it, which this IT reaches only transitively, through the commits any
`PERIODIC_CONSUMER_SYNC` instance makes; the rest is unit tests and docs. The next head on the same
branch, `133070bb4`, which differs only in a unit test, passed the lane at normal length. Recorded,
not diagnosed.
<!-- post-merge: checked-end -->
