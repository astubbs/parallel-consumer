...
| `ParallelEoSStreamProcessorTest.queuedMessagesNotProcessedOrCommittedIfSubmittedDuringShutdown` and `.closeAfterSingleMessageShouldBeEventBasedFast` | 1 local sighting each, 2026-09-09, during the matched-pair campaign that retired the four rows above - a deliberately oversubscribed 12-core box running two full core suites at once. Neither reproduced again in that campaign; **that is one sighting each, not a rate**. A second sighting of `queuedMessages...` (`[2]`, the same `assertCommits` message) on 2026-09-18, on the astubbs#178 branch, in 1 of 3 full core unit-suite runs with nothing else building; 3/3 pass in isolation afterwards, and that change adds a wait that only a revoke can arm, which this test never performs | Recorded rather than diagnosed, because a local run keeps nothing once its output is gone. Both are shutdown-path tests in the class this pass was already working in, and the class's own defect history is cycle-counts standing in for events - so read them against that first. `queuedMessages...` is the harder one to place, because astubbs#101 *already* replaced its cycle count with `awaitUntilTrue(gotK0::get)` plus `awaitForCommit(1)`; if its mechanism is the same class, it is a second instance in a test that was supposed to be immune, and that is worth knowing. Nothing here establishes it is master-state: both arms of the pair carry `master`'s code for these two tests, so a sighting on either arm says nothing about a branch, but one sighting says nothing about a rate either |
| `PartitionOrderProcessingTest.allPartitionsAreNotProcessedInParallel` | Sighted (`node bin/inflight.mjs codecov test allPartitionsAreNotProcessedInParallel` for the recorded outcome per commit, which is the rate rather than any total written here) on 2026-09-18, astubbs#517, [job 105466597785](https://github.com/astubbs/parallel-consumer/actions/runs/35302041274/job/105466597785), at 19339c373 - a head that differs from the passing 67a6084dc by three javadoc lines in `ProcessingShard`; ambient probe clean, `forkCount=4`. The re-run of that job on the same head, [job 105470799479](https://github.com/astubbs/parallel-consumer/actions/runs/35302041274/job/105470799479), passed; a local matched pair on an 8-core box (six runs on the branch, five on a detached `origin/master` checkout, both arms against their own broker) went green on every run, so the failing shape was not reached off the hosted runner | A NEGATIVE test: it asserts that under PARTITION ordering with the default buffer some partition is STARVED by the time 500 records have been processed (`partitionCounts` not all above zero). The failing run saw `{0=109, 1=109, 2=109, 3=108, 4=109}` - every partition served evenly - because `pc.subscribe` precedes the 10000-record produce and the consumer's first fetch landed while each partition held only ~109 records, so the first polls carried all five partitions; the passing shape is a fetch after each partition holds 500 or more, where one partition's batch fills the 10-500 buffer and the intake gate pauses the poller with only that partition resident. Which shape a run gets is producer-versus-fetch timing on the runner, not the scan: the departure gate astubbs#178 adds can only WITHHOLD a shard's work, never widen the served set, and its set is empty without a revoke (`WorkManagerTest.resumesFromNextShard` under PARTITION is the deterministic proof that every populated shard is drawn from). Cleared on mechanism and on code identity with a passing head. Not quarantined, and one sighting is not a rate. The test's design - it asserts a raced outcome the README states only as a possibility - and what would make it deterministic is `test-partition-order-starvation-test-asserts-a-raced-outcome.md` <!-- post-merge: checked - names astubbs#517 in the past tense as the PR the sighting was on --> |
...
<!-- post-merge: checked-begin - names astubbs#493 in the past tense as the branch a sighting came
     from, which stays true once that work has landed -->
**Fourth sighting, 2026-09-09 on astubbs#493, and it is the cleanest mechanism-clear yet.** Same
repetition 1 of 5, same `consumeCount` 0 and the same `[Should have consumed messages - if 0, the PC
died from CME during rapid toggles]` message;
[job 102294781198](https://github.com/astubbs/parallel-consumer/actions/runs/34296644467/job/102294781198)
at `ff71b7dc3`. Repetitions 2 through 5 passed on the same commit, which is the recorded signature.

**The branch cannot reach it, and here that is provable rather than argued.** astubbs#493's entire
diff is two markdown files under `docs/inflight/` and one `<excludeVulnerabilityId>` element inside
`ossindex-maven-plugin`'s configuration in the root pom - every other pom line it touches is an XML
comment. That plugin is bound to `validate` and skipped unless `ossindex.skip=false`; the element is
an advisory-id filter. No dependency, version, classpath, surefire/failsafe setting or JVM argument
changes, so there is no path from the diff to a Kafka integration test at all. Previous sightings were
cleared by reading what the branch's Java touched; this one needs no such reading.

**What it adds: the "first completed run of a branch" condition survives, the lane-change one does
not.** The branch's two earlier pushes were cancelled mid-run by the next push, so this was its first
integration run to *complete* - and note that the cancelled runs are absent from every failure list,
exactly the reading trap the first sighting warned about. But nothing in `.github/` differs from
`origin/master` here, and the branch is cut straight from it, so this is not the "branch just took a
lane change" condition either. Two of four sightings now lack it, which leaves the fixed-sleep-under-
load defect above as the explanation carrying all four.
