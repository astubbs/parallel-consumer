...
| `MdcContextPropagationTest.anEmptyCallerContextIsHandledAndNothingLeaks` | 5 seen (2026-09-01 locally on astubbs#262; 2026-09-02 on CI twice, consecutive heads of astubbs#416; 2026-09-02 locally on astubbs#207; 2026-09-02 on **master's own `full build`** at `ebd8bcb19`) | Not from the original scan. Its own PRECONDITION fails - `expected: null but was: {}` on the JUnit thread before PC is involved - so it says nothing yet about MDC propagation. **The master sighting is the one that matters**: the `build` job is master's only coverage uploader, so its failure is why astubbs#414's merged glob fix has not taken effect. Diagnosis, hypothesis and every sighting live in [`test-mdc-empty-context-precondition-is-order-dependent.md`](test-mdc-empty-context-precondition-is-order-dependent.md); not quarantined - still no mechanism <!-- post-merge: checked - names the branches dated sightings came from, in the past tense --> |
...
#### The `[3]` sanity-check failure has now been seen four more times, and only ever under load

<!-- post-merge: checked-begin -->
Four further sightings, 2026-09-01, running the full core suite on astubbs#207 while the box was also
running CI and a second build - two failures, twice running, on differing parameters (`[2]`, then `[1]`
and `[3]`). Differing parameters are what rule out a deterministic break from the branch that saw them;
astubbs#207 changes offset decoding and nothing this test exercises.
<!-- post-merge: checked-end -->

**The mechanism is legible from the assertion, which is more than the ledger had before.**
...
**Control-armed, which is why this is recorded as contention rather than suspected:**
...
The last arm agrees with the "not reproduced in isolation" result above rather than contradicting it:
isolation removes the contention, so a clean isolated run is what the contention reading predicts.
