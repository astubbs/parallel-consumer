...
| `dups: clones` lane | 3 seen (2026-09-04, astubbs#433, latest [run 33840744923](https://github.com/astubbs/parallel-consumer/actions/runs/33840744923)) | Not a test - the LANE hit its `timeout-minutes: 5` cap with `npm exec jscpd` still running, and the runner killed it as an orphan. The action does PMD CPD **and** jscpd over the whole tree, twice (PR and base), inside those five minutes; the cancelled attempt used 5m07s, so the cap has **no meaningful headroom** and the lane tips over on a slow runner. Same shape as the PIT row above, with one difference that matters: this one is a REQUIRED check, so a cancelled row blocks the merge rather than just reading like a failure. **It is not PR-specific and not a finding about the code**: the same lane completed on an earlier head of the same PR and posted its full report, and the job is `if: pull_request`, so it never runs on master and master history cannot be used to attribute it. Durable fix, for a CI PR on master: raise the cap, or split the two engines into their own jobs so neither pays the other's time <!-- post-merge: checked - dated sighting on a named run --> |
...
| `RegistrationRaceStaleResidentIT.freshArrivalCollidingWithStaleShardResidentMustStillGetProcessed` | 7 seen (2026-09-01; 2026-09-03 six times - once on a producer branch, twice on astubbs#419, twice in a row on astubbs#429 whose same head then passed on a deliberate re-run, and once on astubbs#433) | Not from the original scan - found while babysitting astubbs#257. Failed its **saturation/pause-point setup guard**, not the confluentinc#909 signature assertion, so it proves nothing about the defect it reproduces - see below **It also has a same-day cross-branch control**: `bin/inflight.mjs codecov test freshArrivalCollidingWithStaleShardResidentMustStillGetProcessed` shows it failing on two unrelated branches within an hour with the identical `mid-loop pause point (offset 25)` signature, while passing on both of those same branches at neighbouring commits - so it is master-state and test-side, not any one branch's regression, answered from recorded history rather than by re-running builds. **And a WITHIN-branch control, which is stronger still**: astubbs#433 ran the same content twice, passing at `01fb0c4` and failing at `53ef4d6` after a re-cut that changed only the commit split and merged one unrelated reactor-test commit - so the same tree produced both outcomes, which no cross-branch comparison can establish. Elapsed 30.8s against a 300s deadline, so the probe reports 10% headroom used: what expires is the guard's own wait, not the test's budget. <!-- post-merge: checked --> |
