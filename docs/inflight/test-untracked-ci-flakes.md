...
| `RegistrationRaceStaleResidentIT.freshArrivalCollidingWithStaleShardResidentMustStillGetProcessed` | 3 seen (2026-09-01; 2026-09-03 on astubbs#420, [run 33706716163](https://github.com/astubbs/parallel-consumer/actions/runs/33706716163), and on astubbs#410, [run 33710182789](https://github.com/astubbs/parallel-consumer/actions/runs/33710182789) - same setup-guard failure each time, probe clean, passing on the heads either side; the recorded history also shows it red on an unrelated branch the same hour) | Not from the original scan - found while babysitting astubbs#257. Failed its **saturation/pause-point setup guard**, not the confluentinc#909 signature assertion, so it proves nothing about the defect it reproduces - see below <!-- post-merge: checked --> |
