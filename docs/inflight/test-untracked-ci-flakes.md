...
**Fifth sighting, 2026-09-09, on the ledger rung of astubbs#225 at `779b280c1`**
([job 102336356692](https://github.com/astubbs/parallel-consumer/actions/runs/34310554919/job/102336356692)),
the `[1]` arm, the same assertion - "Should have consumed messages - if 0, the PC died from CME during
rapid toggles" - at 20.0s against 20-22s on every neighbouring pass. `bin/inflight.mjs codecov test
rapidToggleShouldNotCreateDuplicateInstances` shows it passing at that branch's previous head and at its
base branch's head in the same half hour, on the same master, so the ledger code that branch adds is not
the variable: a one-in-twelve failure inside the recorded window, on a test whose fixed sleep is the
explanation above. Recorded, not diagnosed.
<!-- post-merge: checked-end -->
<!-- post-merge: checked-begin - a dated CI sighting against a branch head, past tense -->
**Sixth sighting, 2026-09-09, on the recovery rung of astubbs#225 at `6ff294ef5`**
([job 102339485076](https://github.com/astubbs/parallel-consumer/actions/runs/34311330542/job/102339485076)),
repetition `[4]` this time, the same assertion. The head differs from the one that passed a quarter of an
hour earlier by one docs commit, and the rung above it passed at a head carrying the same code plus its own;
the arm is the shipped default commit mode, so none of this stack's transactional-mode code is on the path.
The fifth and sixth sightings landed within the hour that every lane of a five-rung stack was pushed at
once to the same runner, which is the load the fixed sleep above cannot tolerate. Recorded, not diagnosed.
<!-- post-merge: checked-end -->
...
<!-- post-merge: checked-begin - a dated local sighting on a named branch, past tense -->
Seen again 2026-09-09 in a local full run of the core suite on the recovery rung after it merged the ledger rung and, through it, master as of astubbs#485: one failure, the same captured-line shape, passing alone. Master never carried this row - every sighting is local to this stack - so it is restored here after a merge that took the ledger from master.
<!-- post-merge: checked-end -->
