...
| `MdcContextPropagationTest.anEmptyCallerContextIsHandledAndNothingLeaks` | 3 seen (2026-09-01 locally; 2026-09-02 on CI twice, consecutive heads of astubbs#416) | Not from the original scan. Its own PRECONDITION fails - `expected: null but was: {}` on the JUnit thread before PC is involved - so it says nothing yet about MDC propagation. Diagnosis, the logback-clear hypothesis, its falsification path and both sightings live in [`test-mdc-empty-context-precondition-is-order-dependent.md`](test-mdc-empty-context-precondition-is-order-dependent.md); not quarantined - the second sighting brought a rate, not a mechanism <!-- post-merge: checked - names astubbs#416 as the branch a dated sighting came from, which stays true after it lands --> |
