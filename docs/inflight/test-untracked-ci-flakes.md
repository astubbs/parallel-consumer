...
| `Mutation Tests (PIT, PR-scoped)` lane | 1 seen (2026-09-02, astubbs#207, [run 33610711974](https://github.com/astubbs/parallel-consumer/actions/runs/33610711974)) | Not a test - the LANE hit its `timeout-minutes: 30` cap and was cancelled, on a **markdown-only** delta from a head where it had scored in 19m18s with the same class set. The cap had about a third headroom over a normal run, so it flapped on a slow runner. Addressed 2026-09-07 by the fold into `scan: repo` ([`ci-fewer-jobs-ruleset-edits.md`](ci-fewer-jobs-ruleset-edits.md)): the bound is now `timeout-minutes: 20` on the PIT **step**, so a hit ends that step and the job still reports, where a hit on the old job cap cancelled the whole row. Still `continue-on-error: true`, so it never gates a merge <!-- post-merge: checked --> |
...
**Sighting, 2026-09-05**, on astubbs#446
([job 101225574926](https://github.com/astubbs/parallel-consumer/actions/runs/33936545376/job/101225574926)),
one failure in 201 integration tests, same assertion - `control thread must reach the mid-loop pause
point (offset 25)` at line 221, probe clean, `forkCount=4`. **The cleanest control arm this test has
had.** That branch changes one markdown file under `docs/inflight/` and nothing else, and the same
lane had passed on the branch's previous push an hour earlier, when the only difference was four
more lines of that markdown. So the branch is ruled out by construction rather than by argument, and
the sighting belongs to the standing rate above. The same run also went red on
`ChaosChurnStormIT`'s `NO_PROGRESS` line, recorded in
[`test-no-progress-window-may-not-transfer-to-w1.md`](test-no-progress-window-may-not-transfer-to-w1.md);
two known flakes in one run of a docs-only PR is what the standing rate looks like from the outside,
and it is the cost the quarantine decision above is weighing.
