...
Also fixed and out, 2026-09-18: `PartitionOrderProcessingTest.allPartitionsAreNotProcessedInParallel`,
one sighting on astubbs#517 at a head three javadoc lines from a passing one. A NEGATIVE test that
asserted the untuned PARTITION-order starvation as an OUTCOME of the broker's fetch composition, which
producer-versus-fetch timing decided; it is deleted, and the mechanism it wanted - the intake gate
pausing on one partition's poll and staying paused until that partition drains to the threshold - is
two unit tests in `WorkManagerTest` with fixed inputs. The positive twin stays as the liveness claim.
[`a-negative-test-asserted-a-fetch-composition-the-broker-never-promised-2026-09-18.md`](../solutions/test-flakiness/a-negative-test-asserted-a-fetch-composition-the-broker-never-promised-2026-09-18.md)
carries the diagnosis, the options and the sabotage table; the note that held the decision open,
`test-partition-order-starvation-test-asserts-a-raced-outcome.md`, went with it.
