...
| `RegistrationRaceStaleResidentIT.freshArrivalCollidingWithStaleShardResidentMustStillGetProcessed` | 5 seen (2026-09-01; 2026-09-03 four times - once on a producer branch, twice in a row on astubbs#429 whose same head then passed on a deliberate re-run, and once on astubbs#433). **The astubbs#433 sighting is the one that isolates it from branch content**: that branch ran the SAME code twice, passing at `01fb0c4` and failing at `53ef4d6` after a re-cut that changed the commit split and merged one unrelated reactor-test commit, touching nothing this test drives. `bin/inflight.mjs codecov test freshArrivalCollidingWithStaleShardResidentMustStillGetProcessed` shows the pair, and shows the same guard red on an unrelated branch within the same hour. Elapsed 30.8s against a 300s deadline, so the probe reports 10% headroom used - it is the guard's own wait that expires, not the test running out of time | Not from the original scan - found while babysitting astubbs#257. Failed its **saturation/pause-point setup guard**, not the confluentinc#909 signature assertion, so it proves nothing about the defect it reproduces - see below <!-- post-merge: checked --> |
