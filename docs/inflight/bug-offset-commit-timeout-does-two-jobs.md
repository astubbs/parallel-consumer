...
|---|---|---|---|
| `ConsumerOffsetCommitter.commitAndWait` waiting for a commit response | **control** | no deadline of its own - released by an affirmative event: the typed commit response (committed / deferred / terminally failed) or the poller-death notification. One last-resort backstop remains, for the case neither event can ever arrive because the poll thread is wedged *outside* the commit path (`pollerWedgedBackstopFloor`) | 4 x `offsetCommitTimeout` + 60s |
...
The wedged-poller backstop above does not reopen it: it is derived from the option rather than equal to
it, minutes rather than seconds, and asserts only that nothing is being serviced at all.
