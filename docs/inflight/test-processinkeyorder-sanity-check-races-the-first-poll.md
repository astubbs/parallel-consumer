...
| 2026-09-01 | astubbs/parallel-consumer#334 (the foreign Streams wrappers), while forward-merging astubbs/parallel-consumer#340 into it; macOS, JDK 17 | Red on both full reactor runs of the merged tree - 3 failures in 543 each time, the same `[sanity check input data]` cluster. **This closes out a four-branch day whose value is the control, not the count.** The chain was merged rung by rung and `parallel-consumer-core` is byte-identical at every one of the four (checked with `git diff` against the rung below at each link, empty each time). Tally on one box in one afternoon: rung 1 green 5 of 5; rung 2 red 1 of 2; rung 3 red 2 of 3; rung 4 red 2 of 2. **The failure rate rose through the afternoon while the code under test did not change at all**, which is the cleanest statement yet that this is neither branch-state nor load in any simple sense - and it is the reason none of the four merges was re-run until green, since a green run here would have proved nothing. |
