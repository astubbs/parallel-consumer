...
| astubbs#178 | "Not reproducible by inspection" is too broad - true only under the reporter's stated preconditions - and "nothing in the fork changes it" is now false: the one rebalance-gated route to the symptom was ruled a violation and fixed. The chaos-suite reasoning is stale the other way: the ledger's overlap check now spans epochs | [`a-revoke-sweep-freed-a-key-whose-worker-was-still-running-2026-09-18.md`](../solutions/logic-errors/a-revoke-sweep-freed-a-key-whose-worker-was-still-running-2026-09-18.md); the draft is `issue-response-178.md` |
...
| astubbs#162 | **Misses the headline**: reporters hit the same warning after the fix shipped, proving it partial. Understates what this tree already carries. Cites an unrelated open PR as the thing reworking the area | [`issue-response-162.md`](issue-response-162.md) - the whole section, rewritten, held for the sweep |
