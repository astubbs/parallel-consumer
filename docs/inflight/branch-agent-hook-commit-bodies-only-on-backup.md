...
was said to name them does not exist; it was lost when the stack was rebuilt.

**Read them before changing `.claude/hooks/check-merge-outstanding-work.sh`** - between them they are
the whole design record for that guard, including two decisions that look like oversights from the
code alone:
...
Verified reachable on 2026-08-25: the branch resolves and all five SHAs resolve. **That is the whole
risk here** - they live on one unprotected branch, and deleting it destroys the only copy.
