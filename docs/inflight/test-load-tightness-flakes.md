...
| `ParallelEoSStreamProcessorTest.inFlightMessagesCommittedIfProcessedDuringShutdown[3]` | 1 sighting (2026-09-01), 1 of 2 full-suite runs, 0 of 3 in isolation | `assertCommits` expected `[1]` and got `[]` - the record that completed during shutdown was not committed in time. **Control arm**: the same test on the same tree passed in a full `parallel-consumer-core` unit run minutes earlier, and `parallel-consumer-core` is byte-identical between the branch it failed on and the branch it passed on, so no code difference can explain it |
| `ParallelEoSStreamProcessorTest.processInKeyOrder[3]` | 1 sighting (2026-09-01), the run immediately after the row above, on the same tree | fails its own **preamble** sanity check, `[sanity check input data]`, before the behaviour under test is exercised. **It has a diagnosis, on a branch this one does not carry**: `git show 8a7dc3ec7:docs/inflight/test-processinkeyorder-sanity-check-races-the-first-poll.md`, whose control arm reddened unmodified `master` HARDER than the branch it was seen on - the first evidence the flake is master-state |
