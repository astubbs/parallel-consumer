...
does not touch. Recorded here rather than folded into that PR: these are pre-existing defects in
...
`KafkaConsumer`, which is exactly the `ConcurrentModificationException` astubbs#292 fixed for the
...
This is the orphaned-group-member failure astubbs#292 fixes, relocated to the teardown boundary.
Note that PR's "residual 3" understates it: the consequence is an unclosed, still-polling PC, not a
stranded flag.
...
If a chaos IT throws `ConcurrentModificationException` again after astubbs#292 lands, do **not** read
it as that fix having failed. Look first at whether a drain exceeded the 60s shared join budget, or a
`stopAsync()` close exceeded `settleFleet`'s 15s wait. Both reproduce the original signature through
paths astubbs#292 deliberately left alone.
