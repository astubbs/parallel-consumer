...
| Direct pull (`416f03dec` + tests `4926aec03`) + the scan fix | not cut. One review: `ShardOccupancy` exists to make direct pull's scan viable, they share `getUpperBoundOnSelectableWork` and the abandonment path, and the owner's steer is to combine where the review is one idea |
| Scan fix (`b73f8b97e` + `3c617f4f7`) | **Do not cut alone - attempted 2026-08-25 and abandoned on evidence.** It compiles only on top of THREE merged parents (astubbs#335's `ExecutionState`, astubbs#336's conservation/retirement helpers, astubbs#358's meter), and its own tests additionally need the verdict-free abandonment path and `WorkManager#getUpperBoundOnSelectableWork` - both of which belong to the direct-pull work it was written alongside. Isolating it means hand-rebuilding accounting code that has already produced two bugs. **Ship it WITH direct pull as one review**, on a base that merges the three parents | **Improves the shipped default too** - the 440 -> 1.00 examinations-per-dispatch collapse in `UNORDERED` - so it may deserve promotion ahead of direct pull |
