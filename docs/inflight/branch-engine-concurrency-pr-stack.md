...
| Residence-time metric (`5a0321282` + its clock fix `078ab1c9d`) | next; lands after astubbs#335 because both touch `WorkContainer`. Earns a `Release-Note:` trailer |
| Virtual threads (`b6083f5ef` + CI harness `e337b166a`) | not cut |
| Direct pull (`416f03dec` + tests `4926aec03`) | not cut |
| Scan fix (`b73f8b97e` + `3c617f4f7`) | not cut. **Improves the shipped default too** - the 440 -> 1.00 examinations-per-dispatch collapse in `UNORDERED` - so it may deserve promotion ahead of direct pull |
