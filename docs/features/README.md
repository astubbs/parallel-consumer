...
and record that tag and a concise evidence basis. A version is what a user acts on; a commit hash is not, and it makes
the record harder to read without making it more checkable. If a feature predates retained history, say that
explicitly rather than inventing a later introduction. A capability can
