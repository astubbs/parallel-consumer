 ...
 * always runs first within a section, and the sections run one after another, so no arm meets the machine
 * in the same state as any other. What bounds that is measured rather than argued: the two sections' STOCK
 * arms do byte-identical work - stock Kafka Streams ignores keys, and the per-record cost here is chosen by
 * value - so their difference is this run's own noise floor, printed in the summary. Across five runs on
 * one machine it read between 1.00x and 1.20x, against a head-of-line result of 14x to 19x.
 * <p>
 * The floor is also the reason the negative control can still do its job now that it lands on parity rather
 * than behind. Parity is not "no difference to two decimal places"; it is a difference no larger than the
 * floor, and one run's control read 1.19x on a run whose floor was 1.20x.
 *
...
final class ArmRunner {
    ...
    /*
     * The workload. Chosen so the effect cannot be mistaken for noise and so the run stays short enough
     * that somebody will actually run it: one blocker two orders of magnitude more expensive than the
     * records behind it, and enough of those for a percentile to mean something.
     *
     * There is no benchmark in parallel-consumer-streams for these to be kept in step with on this branch.
     * PcDrivenStreamsDispatchTest asserts that dispatch happens and that the output is unchanged; it does
     * not measure latency. When a benchmark arrives it should share these numbers, and whichever lands
     * second owns saying so - nothing will notice if it does not.
     */
    ...
    /**
     * @param pcDispatch whether the seam is on. Stated explicitly in BOTH arms, and that is not
     *                   belt-and-braces. The switch defaults OFF, so a PC arm that omitted the call would
     *                   silently be a second stock arm and the comparison would be of one thing against
     *                   itself; and the default is documented as moving the day the refusal envelope lands,
     *                   at which point an arm that relied on it would flip without this file changing
