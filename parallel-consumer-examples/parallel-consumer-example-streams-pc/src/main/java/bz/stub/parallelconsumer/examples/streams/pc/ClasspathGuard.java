...
final class ClasspathGuard {
     ...
     * {@link #STREAM_TASK} alone would pass while the rest of the patch set lost, so every one is named.
     * <p>
     * <b>This list must match {@code patched.classes} in {@code parallel-consumer-streams/pom.xml}</b>, and
     * nothing checks that it does. A class dropped from here would simply stop being guarded; a class named
     * here that the pom does not patch fails the run loudly, which is the safer direction to be wrong in.
     */
    private static final String[] PATCHED_CLASSES = {
            ...
            // Added by the execution seam, for the split poll wait that lets a worker completion end the
            // StreamThread's wait rather than the wait running to poll.ms. See PcWorkSignal.
            "org.apache.kafka.streams.processor.internals.StreamThread",
    };

    /**
     * NOT patched, and therefore expected to keep loading from the stock jar. Without this the guard could
     * not tell "our classes won" from "the whole of Kafka Streams got replaced", and the second would mean
     * something has gone very wrong with the build.
     * <p>
     * {@code PartitionGroup} rather than any other unpatched class, because it is the exact thing the seam
     * routes around - {@code nextRecord()} is the one-at-a-time hand-off this demo measures. Seeing it load
     * from the stock jar says the seam replaced the *dispatch* and left Kafka's own queueing alone.
     */
    private static final String UNPATCHED_SIBLING =
    ...
    static void verifyPatchedStreamsIsLoaded() {
        ...
        Class<?> unpatchedSibling = load(UNPATCHED_SIBLING);
        ...
        for (String className : PATCHED_CLASSES) {
            ...
            // package-private access between the patched half and the stock half, and any of them
            // could be the one that landed in the wrong loader.
            if (patched.getClassLoader() != unpatchedSibling.getClassLoader()) {
        ...
        String siblingLocation = codeSourceOf(unpatchedSibling);
    ...
    private static void verifyDispatchSeamIsPresent(final Class<?> streamTask) {
        try {
            // The field's TYPE is printed, not merely its presence. A name can be coincidence; the field
            // being declared as our dispatcher type is the patch and nothing else. It also gives the
            // reflective call a used return value, which -Werror Error Prone requires - and the useful
            // reading was the better way to satisfy it than discarding the result.
            Field seam = streamTask.getDeclaredField(DISPATCH_SEAM_FIELD);
            Console.line("  seam      %-28s <- field '%s' of type %s", simpleName(STREAM_TASK),
                    DISPATCH_SEAM_FIELD, seam.getType().getName());
    ...
    private static void failLoudly(final String className, final String location) {
        ...
        Console.line("  transitively. Do not exclude it - the patch set is only %d classes and needs every",
                PATCHED_CLASSES.length);
        Console.line("  other class from the stock jar.");
        Console.line("");
